...
Hermes uses WAL mode with aggressive retry (15 retries, 20-150ms jitter). Glindt must only open state.db in read-only mode to avoid write contention. Checkpoint every 50 writes. WAL file modification is a good signal for refresh.
