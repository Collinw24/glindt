...
**Workstream:** WS-7 of Glindt v2.8.0
...
- **Live MCP SSE wire-format probing.** WS-7 only writes the YAML + surfaces the field. Hermes owns the runtime connect; Glindt trusts `hermes mcp test <name>` to verify.
...
- **iOS `--no-skills`.** iOS Profiles is read-only (per CLAUDE.md "v0.12 iOS catch-up (Phase H)" and `Glindt iOS/Profiles/ProfilesView.swift`). No new toggles on iOS.
...
#### 1a. `glindt/Packages/GlindtCore/Sources/GlindtCore/Models/HermesMCPServer.swift`
...
#### 1b. `glindt/glindt/Core/Services/HermesFileService.swift`
...
#### 1c. `glindt/glindt/Features/MCPServers/Views/MCPServerAddCustomView.swift`
...
#### 1d. `glindt/glindt/Features/MCPServers/ViewModels/MCPServersViewModel.swift`
...
#### 1e. `glindt/glindt/Features/MCPServers/Views/MCPServersView.swift`
...
#### 1f. `glindt/glindt/Features/MCPServers/Views/MCPServerEditorView.swift`
...
#### 1g. `glindt/Packages/GlindtCore/Tests/GlindtCoreTests/HermesMCPServerYAMLTests.swift` (NEW or extension to existing)
...
- GlindtCore: 4 YAML-parser tests above + 2 model tests (`MCPTransport.allCases.count == 3`, `sseReadTimeout` round-trips through memberwise init).
- GlindtTests (Mac app): `MCPServersViewModelTests.testAddCustomSSE` mock-fileservice test verifying the `--transport sse --sse-read-timeout` flag shape.
...
#### 2a. `glindt/Packages/GlindtCore/Sources/GlindtCore/Models/HermesCronJob.swift`
...
#### 2b. `glindt/glindt/Features/Cron/Views/CronView.swift`
  ...
  if supportsNoAgent {
      ...
      if form.noAgent {
          Text("Watchdog mode — Hermes runs the pre-run script and skips the AI turn. Prompt + skills are ignored.")
              ...
              .padding(.leading, GlindtSpace.s3)
      }
  }
  ```
...
#### 2c. `glindt/glindt/Features/Cron/Views/CronView.swift` — owner site
...
#### 2d. `glindt/glindt/Features/Cron/ViewModels/CronViewModel.swift`
...
#### 2e. `glindt/glindt/Features/Cron/Views/CronView.swift` — detail rendering

**Edits (cosmetic, optional but high-value):** When the selected job has `noAgent == true`, render a small `GlindtBadge("script-only", kind: .info)` in `detailHeader` next to the existing `paused` / `running…` badges so the user can tell at a glance which jobs are watchdogs. Same in the `cronRow` list — append a `GlindtBadge("no-agent", kind: .neutral)` when the flag is on, similar to the existing `paused` badge.
...
#### 3a. `glindt/glindt/Features/Settings/Views/SettingsView.swift`
...
#### 3b. `glindt/glindt/Features/Settings/Views/Tabs/WebToolsTab.swift` (NEW)
...
#### 3c. `glindt/Packages/GlindtCore/Sources/GlindtCore/Models/HermesConfig.swift`
...
#### 3d. `glindt/Packages/GlindtCore/Sources/GlindtCore/Parsing/HermesConfig+YAML.swift`
...
#### 3e. `glindt/glindt/Features/Settings/ViewModels/SettingsViewModel.swift`
...
#### 4a. `glindt/glindt/Features/Profiles/Views/ProfilesView.swift`
...
#### 4b. `glindt/glindt/Features/Profiles/ViewModels/ProfilesViewModel.swift`
...
1. **MCP transport names.** The release notes say "SSE transport" and reference "stale pipe transport failures." Glindt's `MCPTransport` enum has `stdio` and `http`; Hermes internally calls those `stdio` and `streamable-http` (or just `http`), and the "pipe" callsite likely refers to internal stdio process pipes — not a third user-facing transport. We're proceeding on that assumption. **Verify:** read `~/.hermes/hermes-agent/hermes_cli/mcp.py` (or equivalent) during integration to confirm `pipe` is internal-only and not a fourth user-selectable transport.
...
- **iOS surfaces.** All four additions are Mac-only:
  - MCP SSE: Glindt has no iOS MCP servers UI today.
  - Cron `--no-agent`: iOS Cron is read-only (`Glindt iOS/Cron/CronListView.swift`); no editor.
  - Web Tools: iOS Settings doesn't currently surface Web Tools.
  - Profiles `--no-skills`: iOS Profiles is read-only (`Glindt iOS/Profiles/ProfilesView.swift`).
  iOS catch-up is WS-9 territory.
- **Wiki updates.** Per CLAUDE.md, wiki updates land alongside the release once the feature is shipped — not pre-merge. WS-7 PR notes the wiki pages that will need updating in `Glindt-Settings.md`, `Glindt-Cron.md`, `Glindt-MCP-Servers.md`, `Glindt-Profiles.md`, and `Hermes-Version-Compatibility.md`. The wiki PR is its own commit on `gh-pages` after v2.8.0 ships.
