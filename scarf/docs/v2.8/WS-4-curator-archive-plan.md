...
> **Scope.** Catch Glindt's Curator surface up to Hermes v0.13's new write-side
> verbs: `archive <skill>`, `prune`, `list-archived`, and the synchronous flavor
> of `run`. WS-4 owns Mac UX end-to-end + the GlindtCore parser/service work that
...
1. **Wire all four new v0.13 curator verbs** (`archive`, `prune`, `list-archived`,
   synchronous `run`) into GlindtCore through a typed actor surface so the view
...
- **New model:** `HermesCuratorArchivedSkill` in
  `glindt/Packages/GlindtCore/Sources/GlindtCore/Models/HermesCuratorReport.swift`
...
- **Wire shape:** destructive. Removes everything currently archived. Open
  ...
  1. **If `--dry-run` exists:** Glindt's prune confirm sheet calls
  ...
  2. **If no `--dry-run`:** Glindt calls `hermes curator list-archived` to
...
- **`glindt/Packages/GlindtCore/Sources/GlindtCore/Services/CuratorService.swift`**
...
- **`glindt/Packages/GlindtCore/Sources/GlindtCore/Models/HermesCuratorArchive.swift`**
...
- **`glindt/glindt/Features/Curator/Views/CuratorArchivedSection.swift`** — new
  Mac sub-view used by `CuratorView`. Renders a `GlindtCard` containing the
  Archived list. Inputs: `[HermesCuratorArchivedSkill]`,
  `onRestore(name:)`, `onPruneOne(name:)?`, `onPruneAll()`. Empty-state path
  renders an "No archived skills" `GlindtCard` with copy explaining what archive
  does (helpful since Curator hasn't run yet on a fresh install).

- **`glindt/glindt/Features/Curator/Views/CuratorPruneConfirmSheet.swift`** —
...
- **`glindt/Packages/GlindtCore/Sources/GlindtCore/ViewModels/CuratorViewModel.swift`**
...
- **`glindt/glindt/Features/Curator/Views/CuratorView.swift`**
  ...
  - Header toolbar additions (only when `archiveAvailable`):
    - "Prune Archived…" `GlindtDestructiveButton` in the overflow `Menu`,
  ...
  - Inline error banner: render `viewModel.errorMessage` as a yellow
    `GlindtCard` above `statusSummary` with an "x" dismiss. (Use existing
    `GlindtColor.warning` background; inspect the Kanban inline banner for
    pattern.)

- **`glindt/glindt/Features/Curator/Views/CuratorRestoreSheet.swift`**
...
- **`glindt/Glindt iOS/Curator/CuratorView.swift`**
...
- **`glindt/Packages/GlindtCore/Tests/GlindtCoreTests/HermesCuratorParserTests.swift`**
...
Create `glindt/Packages/GlindtCore/Tests/GlindtCoreTests/Fixtures/Curator/`:
...
4. **What's the exact synchronous-`run` timeout?** The release notes say
   ...
   Glindt surface a Cancel button? Probably not in v0.13 — transport-level
...
5. **Confirm UX: typed-name confirmation, multi-tap, or destructive-button
   confirm sheet?** Glindt precedent (see "Constraints"):
   ...
   - **Template uninstall** (`TemplateUninstallSheet.swift:79-96`) uses a
     ...
     be removed, then a `GlindtPrimaryButton` tinted red labeled "Remove".
   ...
   - **Recommendation for prune:** match template-uninstall's shape. Prune is
     ...
     confirm button is `GlindtDestructiveButton` labeled "Prune permanently"
     ...
     Cancel is `GlindtGhostButton`, "Cancel". No typed-name confirmation; the
...
- **`hermes curator pause/resume` on the synchronous run.** The new sync `run`
  ...
  before. No UX change.
- **Telemetry on prune.** No GlindtMon event for prune — measure if a user
...
1. Capture real-world stdout fixtures by running every new v0.13 curator verb
   ...
   `Tests/GlindtCoreTests/Fixtures/Curator/`. _(Resolves Open Q1 + Q2 + Q3.)_
