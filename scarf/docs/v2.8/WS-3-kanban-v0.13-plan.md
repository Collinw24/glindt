...
**Workstream:** WS-3 of Glindt v2.8.0
...
The Hermes v0.13.0 release notes list eight Kanban-shaped items in scope for Glindt:

1. **Hallucination gate + recovery UX** for worker-created cards — workers now emit a "I created a follow-up card" claim that Hermes flags as `hallucination_gate_status=pending` until something verifies the underlying card exists. Glindt needs to render the flag and offer Verify / Reject so the user is the verification gate.
2. **Generic diagnostics engine** for task distress signals — Hermes now emits a structured diagnostics array on a task / run when it observes distress (heartbeat-stalled, repeated tool errors, unbounded retry loop, OOM proxy, etc.). Glindt needs to render those diagnostics in the inspector so the user can act before the auto-block fires.
...
4. **Multiline textarea for inline-create title** — v0.13 server tolerates multi-line titles. The Glindt create sheet's title is currently a single-line `GlindtTextField`; convert to a multi-line input so a long title doesn't get clipped on hover-truncate.
5. **Heartbeat / reclaim / zombie / retry-cap reliability fixes** — mostly server-side, but Glindt's run-row + log-tab phrasing ("stale_lock") becomes user-hostile when v0.13 emits a richer outcome ("zombied — reclaimed by reaper"). Render the new outcome string verbatim and add a glossary tooltip.
6. **Auto-block workers that exit without completing** + `auto_blocked_reason` — currently Glindt renders a generic "Last run: blocked" banner; v0.13 attaches a structured reason ("worker exited without `kanban complete`"). Replace the generic banner with the reason when present.
...
8. **Unify failure counter across spawn / timeout / crash outcomes** — server-side counter rename; Glindt's run-row outcome label rendering may need to absorb a new normalized counter (`failure_count` rather than three separate counters). Verify the run row still renders all outcomes.

The two release-notes items NOT in WS-3 scope:

- **Multi-project boards** — already shipped in v2.7.5 via per-project tenants. Hermes v0.13's "one install, many kanbans" framing is the server's catch-up to what Glindt already solved client-side via the `glindt:<slug>` tenant convention. No change here.
- **Shared board, workspaces, and worker logs across profiles** — entirely server-side; Glindt already shows whichever assignee owns a row.
...
- **Worker task-ownership enforcement on destructive tool calls** — server-side; Glindt observes the failure mode (a run ends with `permission_denied`) but doesn't need new UI.
...
- **Live `watch` streaming.** v2.7.5 polls every 5s. v0.13 hasn't added a stable `watch --json` shape Glindt can rely on; deferred until a future flag (`hasKanbanWatch`).
...
### 1. `glindt/Packages/GlindtCore/Sources/GlindtCore/Models/HermesKanbanTask.swift`
...
### 2. `glindt/Packages/GlindtCore/Sources/GlindtCore/Models/HermesKanbanDiagnostic.swift` (NEW)
...
### 3. `glindt/Packages/GlindtCore/Sources/GlindtCore/Models/HermesKanbanRun.swift`
...
- Optional v0.13 housekeeping: `failure_count: Int?` if v0.13's unified counter is exposed on the run shape (unify failure counter across spawn / timeout / crash). If it appears as a top-level key on the run, decode it; if not, this stays a server-internal field and Glindt doesn't need it.

### 4. `glindt/Packages/GlindtCore/Sources/GlindtCore/Models/HermesKanbanTaskDetail.swift`
...
### 5. `glindt/Packages/GlindtCore/Sources/GlindtCore/Models/KanbanCreateRequest.swift`
...
### 6. `glindt/Packages/GlindtCore/Sources/GlindtCore/Services/KanbanService.swift`
...
### 7. `glindt/glindt/Features/Kanban/Views/KanbanCreateSheet.swift`
...
- Replace the single-line `titleField` (lines 116-122):
  ```swift
  GlindtTextField("What needs doing?", text: $title)
  ```
  with a multi-line variant. Two acceptable approaches:
  - **Preferred:** SwiftUI `TextField` with `axis: .vertical` and `lineLimit(1...4)`. Wraps cleanly inside the existing `GlindtTextField` chrome on macOS 14.6+. Pre-existing `GlindtTextField` is a wrapper — extend the wrapper to take an optional `axis` parameter or add a new `GlindtTextEditor` sibling component to `GlindtDesign`. Touch the design package only if the multi-line variant doesn't already exist there. (Audit `Packages/GlindtDesign/` first; if `GlindtTextEditor` exists, use it.)
...
### 8. `glindt/glindt/Features/Kanban/Views/KanbanInspectorPane.swift`
...
private func hallucinationBanner(for task: HermesKanbanTask) -> some View {
    HStack(alignment: .top, spacing: GlindtSpace.s2) {
        ...
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack(spacing: GlindtSpace.s2) {
                ...
                Button("Reject") { onRejectHallucination() }
                    .buttonStyle(GlindtDestructiveButton())
            }
            .padding(.top, 2)
        }
        Spacer(minLength: 0)
...
Render order in `healthBanner`: hallucination first (the user must resolve this before anything else makes sense), then unassigned, then last-failed-run. Stack vertically inside a `VStack(alignment: .leading, spacing: GlindtSpace.s2)` rather than the current `if/else if`.
...
### 9. `glindt/glindt/Features/Kanban/Views/KanbanCardView.swift`
...
### 10. `glindt/glindt/Features/Kanban/Views/KanbanBoardView.swift`
...
### 11. `glindt/glindt/Features/Kanban/ViewModels/KanbanBoardViewModel.swift`
...
### 12. `glindt/Packages/GlindtCore/Tests/GlindtCoreTests/KanbanModelsTests.swift`
...
These tests pin the tolerant-decode contract on both sides (with new fields, without new fields). Pre-v0.13 hosts running v2.8 Glindt must keep decoding cleanly — without these tests we'd ship a regression that bites every customer not yet on Hermes v0.13.

### 13. `glindt/Packages/GlindtDesign/` — IF a multi-line text component is missing

**Why:** If `GlindtTextField` doesn't already accept an `axis: .vertical` parameter (likely the case in v2.7.5), add one OR add a `GlindtTextEditor` component to the design package so the create sheet can use the design-system token.

**Conservative approach:** Use `TextField` with `axis: .vertical` directly inside `KanbanCreateSheet`, styled to match `GlindtTextField` chrome (background, border, padding from `GlindtColor`/`GlindtRadius`/`GlindtSpace`). Defer adding a new design-system component to a follow-up — design-system additions deserve their own review pass and aren't on this WS's critical path.
...
| Multi-line title input rendering | No | Multi-line input is harmless on v0.12 (the GlindtTextField is just visually taller). |
...
```bash
xcodebuild -project glindt/glindt.xcodeproj \
  -scheme GlindtCore \
...
- **Pre-v0.13 silent regression.** The most damaging failure mode is a v0.12 user upgrading Glindt and seeing the board stop loading. Mitigation: §12 tests pin the v0.12 contract; the gating audit table covers each surface; manual smoke against a v0.12 host is a P0 step.
