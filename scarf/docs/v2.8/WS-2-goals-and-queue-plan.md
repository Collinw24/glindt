...
- A "Goal locked" pill in the chat header (mounted alongside the project / branch chips in [SessionInfoBar](../../glindt/Features/Chat/Views/SessionInfoBar.swift)). Hidden when no active goal.
...
### [glindt/Packages/GlindtCore/Sources/GlindtCore/Models/HermesSlashCommand.swift](../../Packages/GlindtCore/Sources/GlindtCore/Models/HermesSlashCommand.swift)
...
### [glindt/Packages/GlindtCore/Sources/GlindtCore/Models/HermesActiveGoal.swift](../../Packages/GlindtCore/Sources/GlindtCore/Models/HermesActiveGoal.swift) (NEW)
...
### [glindt/Packages/GlindtCore/Sources/GlindtCore/Models/HermesQueuedPrompt.swift](../../Packages/GlindtCore/Sources/GlindtCore/Models/HermesQueuedPrompt.swift) (NEW)
...
Used by `RichChatViewModel.queuedPrompts` and the queue-popover view. The `id` is a Glindt-side UUID minted at queue-time — Hermes' wire protocol doesn't expose a per-queue-entry id (see [Open questions](#open-questions)).
...
**1. Extend `nonInterruptiveCommands` (currently around [RichChatViewModel:251-258](../../Packages/GlindtCore/Sources/GlindtCore/ViewModels/RichChatViewModel.swift)):**

Today the list contains only `/steer`. Add `/goal` and `/queue`. Per the existing contract these are appended unconditionally — capability gating is applied in `availableCommands` (next change). Each entry uses `source: .acpNonInterruptive` so the existing `isNonInterruptiveSlash(_:)` helper at [RichChatViewModel:331-342](../../Packages/GlindtCore/Sources/GlindtCore/ViewModels/RichChatViewModel.swift) auto-recognizes them.
...
`availableCommands` (currently [RichChatViewModel:304-325](../../Packages/GlindtCore/Sources/GlindtCore/ViewModels/RichChatViewModel.swift)) merges the static `nonInterruptiveCommands` unconditionally. Replace that with a filter against a new public `capabilitiesGate` value the controller sets at session-start time:
...
Reset to nil in `reset()` (around [RichChatViewModel:441-478](../../Packages/GlindtCore/Sources/GlindtCore/ViewModels/RichChatViewModel.swift)).
...
`recordQueuedPrompt` is called optimistically when the user sends `/queue ...`. `popQueuedPrompt` runs inside `handlePromptComplete` (currently [RichChatViewModel:763-820](../../Packages/GlindtCore/Sources/GlindtCore/ViewModels/RichChatViewModel.swift)) when the agent finishes a turn — Hermes is responsible for actually running the queued prompt (it lives server-side); the Glindt-side list is purely a UI mirror. Popping is best-effort: if Hermes' server-side queue gets out of sync (deferred prompt aborted, dropped on disconnect), the user sees a stale chip until their next interaction. We accept that v1 trade-off; see [Open questions](#open-questions).
...
### [glindt/glindt/Features/Chat/ViewModels/ChatViewModel.swift](../../glindt/Features/Chat/ViewModels/ChatViewModel.swift) (Mac)
...
**2. Detect non-interruptive commands by name in `sendViaACP` (currently [ChatViewModel:556-635](../../glindt/Features/Chat/ViewModels/ChatViewModel.swift)).**
...
`scheduleHintClear()` extracts the existing 4-second auto-clear pattern (currently inlined for `/steer` at [ChatViewModel:585-591](../../glindt/Features/Chat/ViewModels/ChatViewModel.swift)) into a private helper, so all three commands use the same clear behaviour. The wire send (the existing `client.sendPrompt(...)` call at [ChatViewModel:597](../../glindt/Features/Chat/ViewModels/ChatViewModel.swift)) is unchanged — Hermes parses the slash on the server side.
...
Render block (positioned right after the existing `gitBranch` Label, before the working dot at [SessionInfoBar:65](../../glindt/Features/Chat/Views/SessionInfoBar.swift)):
...
if let activeGoal {
    ...
    .padding(.vertical, 2)
    .background(Capsule().fill(GlindtColor.info.opacity(0.16)))
...
Color choice: `GlindtColor.info` matches the badge intent — informational state, not a warning, not an error. Per CLAUDE.md, accent (rust) is reserved for primary brand surfaces; project / branch already use accent so reusing it would mean three accent chips in a row. `info` differentiates the goal pill visually.
...
Forward the new `SessionInfoBar` parameters at [ChatTranscriptPane:17-25](../../glindt/Features/Chat/Views/ChatTranscriptPane.swift):
...
### [glindt/glindt/Features/Chat/Views/ChatQueueIndicator.swift](../../glindt/Features/Chat/Views/ChatQueueIndicator.swift) (NEW)
    ...
    var body: some View {
        if queuedPrompts.isEmpty { EmptyView() } else {
            Button {
                ...
                .padding(.horizontal, GlindtSpace.s2)
                .padding(.vertical, 2)
                .background(Capsule().fill(GlindtColor.warning.opacity(0.16)))
...
### [glindt/Glindt iOS/Chat/ChatView.swift](../../Glindt%20iOS/Chat/ChatView.swift) — DEFERRED to WS-9

The iOS chat already wires non-interruptive commands at [ChatView:1310-1322](../../Glindt%20iOS/Chat/ChatView.swift) and uses the same `RichChatViewModel`, so the model-side changes are picked up automatically. Surface changes (read-only goal pill, queue chip) belong in WS-9 per the work-stream split. **Do not** add iOS UI changes in this PR — keep the diff scoped.

**Exception:** the iOS controller's `_sendImpl` at [ChatView:1291-1342](../../Glindt%20iOS/Chat/ChatView.swift) needs the same dispatch changes as Mac (record the optimistic goal/queue mutation when the user types `/goal` or `/queue`), otherwise the iOS VM state will diverge from Mac. Mirror change #2 from `ChatViewModel.swift` above into the `_sendImpl` body. iOS just doesn't *render* the goal pill / queue chip yet — that's WS-9.

### [glindt/Packages/GlindtCore/Tests/GlindtCoreTests/M9SlashCommandTests.swift](../../Packages/GlindtCore/Tests/GlindtCoreTests/M9SlashCommandTests.swift)
...
### [glindt/Packages/GlindtCore/Tests/GlindtCoreTests/HermesCapabilitiesTests.swift](../../Packages/GlindtCore/Tests/GlindtCoreTests/HermesCapabilitiesTests.swift)
...
| Type | Where | Purpose |
| --- | --- | --- |
| `HermesActiveGoal` | new GlindtCore model | observable goal-pill state |
| `HermesQueuedPrompt` | new GlindtCore model | one queued-prompt mirror entry |
...
Run `swift test --package-path glindt/Packages/GlindtCore --filter M9SlashCommandTests`. Should be ~12 new tests; existing 23 stay green.
...
3. **Auto-resume after gateway restart — ACP signal.** The release notes say "Auto-resume interrupted sessions after gateway restart" but it's unclear whether that signal:
   - lands as a Glindt-visible ACP event (so we can show an "Auto-resumed" toast),
   - or is purely server-side (Hermes resumes the session transparently and Glindt sees nothing different).
...
6. **Active goal injection into the system prompt.** If Hermes injects the active goal into every turn's system prompt (likely — that's how a "locked" goal would survive across turns server-side), Glindt doesn't need to re-send it on resume. If Hermes uses some other mechanism (e.g. a sidecar tool), that's also Hermes' problem. **No Glindt-side action needed regardless.**
