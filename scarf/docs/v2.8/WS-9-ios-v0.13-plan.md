# WS-9 Plan: GlindtGo iOS catch-up (read-only mirrors of WS-2 / WS-3 / WS-4 / WS-5)

**Workstream:** WS-9 of Glindt v2.8.0
...
**Builds on:**
- v2.7.5 iOS Kanban (`Glindt iOS/Kanban/GlindtGoKanbanView.swift`, `GlindtGoKanbanDetailSheet.swift`).
- v2.7.5 iOS Curator (`Glindt iOS/Curator/CuratorView.swift`).
- v2.7.5 iOS Settings (`Glindt iOS/Settings/SettingsView.swift`) including `platformsSection`.
- v2.5+ iOS Chat (`Glindt iOS/Chat/ChatView.swift`) including `projectContextBar` and `transientHint`.
- WS-1 capability flags + the `.hermesCapabilities(_:)` env injection at `GlindtGoTabRoot.swift:153`.
...
3. **Kanban v0.13 diagnostics on iOS detail sheet.** Extend `GlindtGoKanbanDetailSheet` to render `max_retries`, `auto_blocked_reason`, `hallucination_gate_status`, and the diagnostics array. NO Verify / Reject buttons; the hallucination state is rendered as a badge with the copy "Worker-created — verify on Mac" (since iOS can't verify in v2.8.0).
4. **iOS Curator Archived section.** Append a read-only "Archived" section to the existing `Glindt iOS/Curator/CuratorView.swift`. Per-row: name, kind, archived-date, optional reason (sized small for thumb scrolling). NO Restore / Prune-this / Prune-all buttons. Empty-state copy points the user to the Mac app for restore.
5. **iOS Settings v0.13 features-active badge.** When `caps.semver >= 0.13.0`, surface a small read-only "v0.13 features active" `GlindtBadge` at the top of `SettingsView` with a "Learn more" tap action that opens an action sheet listing the new features.
...
(Verified by walking `Glindt iOS/` at plan time.)

| iOS dir | Files | Mac counterpart |
|---|---|---|
| `App/` | `GlindtIOSApp.swift`, `GlindtGoCoordinator.swift`, `GlindtGoTabRoot.swift`, `Theme/` | `glindtApp.swift`, `AppCoordinator.swift`, `SidebarView.swift` |
...
| **`Kanban/`** | **`GlindtGoKanbanView.swift`, `GlindtGoKanbanDetailSheet.swift` (5-column horizontal-paged Picker, read-only)** | `Features/Kanban/` |
...
| **`Settings/`** | **`SettingsView.swift`, `SettingEditorSheet.swift`, `GlindtMonDiagnosticsView.swift`** | `Features/Settings/` |
...
- No standalone `Glindt iOS/Gateway/` or `Glindt iOS/Platforms/` directory. iOS surfaces gateway / platform configuration through `SettingsView.platformsSection`. WS-9 mirror item 6 extends that section; it does NOT spin up a new feature module.
- No iOS goal / queue surface. WS-2 lays the VM-side scaffolding (`activeGoal`, `queuedPrompts` on the shared `RichChatViewModel` in GlindtCore); WS-9 is what surfaces it on iOS.
...
- `GlindtGoTabRoot.swift:52` constructs a `HermesCapabilitiesStore` per server connection.
- `GlindtGoTabRoot.swift:153` calls `.hermesCapabilities(capabilities)` on the tab view.
...
**Source path read.** The goal text lives on `RichChatViewModel.activeGoal: HermesActiveGoal?` (added in WS-2 — see WS-2 plan §3 "Active goal state"). iOS reads the same VM through `ChatController.vm` (the shared GlindtCore VM). No new GlindtCore field is needed; the WS-2 plumbing flows automatically into iOS.

### File: `Glindt iOS/Chat/ChatView.swift`
...
private var goalChip: some View {
    if let goal = controller.vm.activeGoal {
        Label(truncatedGoalText(goal.text), systemImage: "scope")
            ...
            .foregroundStyle(GlindtColor.info)
            .padding(.horizontal, 8)
            .padding(.vertical, 3)
            .background(GlindtColor.info.opacity(0.16), in: Capsule())
            .lineLimit(1)
...
**Font choice (per CLAUDE.md iOS rules).** Use semantic `.subheadline` because the goal text IS content (the user reads it to recall what they locked the agent on). Per CLAUDE.md "Decision tree per text element: 'is this read for content?' → semantic token. 'Is this chrome / a label / a badge?' → GlindtFont." If the design review pushes back and prefers a fixed-display chip look, switch the inner `Text` to `GlindtFont.captionStrong`; the surrounding pill chrome stays the same.

**Color choice.** `GlindtColor.info` matches Mac's WS-2 plan (informational state, not warning, not error). Keeps the pill visually distinct from the green "success" branch chip and the orange tinted-strip background of `projectContextBar`.
...
The per-run shape adds `run.diagnostics: [HermesKanbanDiagnostic]` (WS-3 plan §3). The typed-mirror enums `KanbanHallucinationGate` and `KanbanDiagnosticKind` are added in GlindtCore and consumable from iOS by `import GlindtCore`.

### File: `Glindt iOS/Kanban/GlindtGoKanbanDetailSheet.swift`
...
if diagnosticsAvailable, let maxRetries = task.maxRetries {
    GlindtBadge("retries: \(maxRetries)", kind: .neutral)
...
if diagnosticsAvailable,
   KanbanHallucinationGate.from(task.hallucinationGateStatus) == .pending {
    ...
    .padding(.vertical, 6)
    .background(GlindtColor.warning.opacity(0.10), in: RoundedRectangle(cornerRadius: GlindtRadius.md, style: .continuous))
    .overlay(
        RoundedRectangle(cornerRadius: GlindtRadius.md, style: .continuous)
            .strokeBorder(GlindtColor.warning.opacity(0.4), lineWidth: 1)
    )
...
if diagnosticsAvailable,
   ...
   let reason = task.autoBlockedReason, !reason.isEmpty {
    ...
    .padding(10)
    .background(GlindtColor.danger.opacity(0.08), in: RoundedRectangle(cornerRadius: GlindtRadius.md, style: .continuous))
}
```
...
private func diagnosticsBlock(_ diags: [HermesKanbanDiagnostic], label: String) -> some View {
    VStack(alignment: .leading, spacing: 6) {
        Text(label)
            ...
            .foregroundStyle(.secondary)
        FlowLayout(spacing: 6) {        // existing primitive at Glindt iOS/Components/FlowLayout.swift
            ForEach(diags) { diag in
                let kind = KanbanDiagnosticKind.from(diag.kind)
                GlindtBadge(diag.kind, kind: kind.badgeKind)
...
**Confirmed:** iOS Curator surface exists at `Glindt iOS/Curator/CuratorView.swift` (read-mostly, with runNow / pause / resume / pin / unpin actions). **In scope.**
...
The shared `CuratorViewModel` lives in GlindtCore — iOS reuses it directly. The iOS `CuratorView` already constructs it at line 18. No iOS-side GlindtCore changes required.

### File: `Glindt iOS/Curator/CuratorView.swift`
                ...
                VStack(alignment: .leading, spacing: 4) {
                    HStack {
                        ...
                        if let category = skill.category, !category.isEmpty {
                            GlindtBadge(category, kind: .neutral)
                        }
                    }
                    HStack(spacing: 6) {
...
**Font choice.** Skill name + reason → semantic `.body` / `.caption` (read for content). Category badge stays `GlindtBadge` (chrome). Date and size → `.caption2` (chrome metadata).
...
### File: `Glindt iOS/Settings/SettingsView.swift`
...
private var v013ActiveBadgeSection: some View {
    Section {
        Button {
            ...
            HStack(spacing: 8) {
                GlindtBadge("v0.13 features active", kind: .success)
                Spacer()
                Text("Learn more")
                    .font(.caption)
    ...
    .listRowBackground(GlindtColor.success.opacity(0.06))
}
```
...
New file `Glindt iOS/Settings/V013FeaturesSheet.swift` (~80 lines):
...
| File | Status | Purpose |
|---|---|---|
| `Glindt iOS/Chat/ChatView.swift` | EDIT | Goal pill + queue chip in `projectContextBar` (§1) |
| `Glindt iOS/Kanban/GlindtGoKanbanDetailSheet.swift` | EDIT | Diagnostics + max_retries + hallucination badge + auto-blocked banner (§2) |
| `Glindt iOS/Kanban/DiagnosticDetailSheet.swift` | NEW | Tap-target sheet showing one diagnostic's full message + timestamp (§2e) |
| `Glindt iOS/Curator/CuratorView.swift` | EDIT | Archived section + capability gate + extra `.task` load (§3) |
| `Glindt iOS/Settings/SettingsView.swift` | EDIT | v0.13 badge section + Platforms section extension (§4, §5) |
| `Glindt iOS/Settings/V013FeaturesSheet.swift` | NEW | "Learn more" sheet for the v0.13-features badge (§5c) |
| `Packages/GlindtCore/Sources/GlindtCore/Services/HermesCapabilities.swift` | EDIT (1 line) | `public var isV013OrLater: Bool` helper if not already present (§5a) |
...
Per CLAUDE.md: iOS clamps Dynamic Type at the scene root (`GlindtIOSApp.swift`: `.dynamicTypeSize(.xSmall ... .accessibility2)`). Verify at both extremes:
...
3. Open Kanban detail: confirm body text + diagnostics chip text scale. Badges (`GlindtBadge`) should NOT scale (they're chrome).
...
- `xcodebuild -project glindt/glindt.xcodeproj -scheme "glindt mobile" -destination 'platform=iOS Simulator,name=iPhone 15' build` must succeed.
...
1. **Does iOS Curator surface exist today?** ✅ Confirmed yes. `Glindt iOS/Curator/CuratorView.swift` exists and is read-mostly with runNow / pause / resume / pin / unpin actions. WS-9 mirror item 4 (Curator Archived list) is in scope. (The user prompt anticipated this might be unknown.)
...
3. **WS-5 plan does not yet exist (`glindt/docs/v2.8/WS-5-gateway-v0.13-plan.md` is missing).** The exact `HermesConfig.gateway.*` and `HermesConfig.googleChat.*` field names are TBD. **Action:** before WS-9 implementation starts, sync with the WS-5 author to align on:
...
5. **`projectContextBar` re-render frequency.** Today it renders only when there's a project. After WS-9, it renders when there's a project OR a goal OR a queued prompt. The added re-render churn during streaming (every diff to `vm.activeGoal` / `vm.queuedPrompts`) may matter for GlindtMon's `chatRender` budget. **Action:** add a GlindtMon counter to the bar's body to measure during dogfooding. If churn becomes a hot-path issue, extract `goalChip` and `queueChip` into separately-scoped subviews so they re-render in isolation.
...
- **Gateway/Platforms iOS feature module from scratch** (separate `Glindt iOS/Gateway/` or `Glindt iOS/Platforms/` dir). v2.8.0 keeps gateway/platform config as an extension to `SettingsView.platformsSection`.
...
- **iOS Goal pill custom font / pill chrome migration to a `GlindtDesign` component** — keep inline. If Mac WS-2 lands a reusable `GlindtGoalPill` component in the design package, swap iOS to use it as a follow-up.
...
- **Telemetry counters** for new iOS surfaces (e.g. GlindtMon counter on goal-pill appearance). Add if dogfooding surfaces a perf signal; otherwise ship without.
...
- **`projectContextBar` re-render churn.** Open Question #5 captures this. Add a GlindtMon counter; revisit if dogfooding shows a hot-path issue.
- **iOS Kanban polling cadence** — the existing 5s poll picks up the new fields automatically. No new polling logic required.
- **No iOS test coverage regression.** WS-9 doesn't add tests but doesn't remove any either. The shared `RichChatViewModel` / `CuratorViewModel` / `KanbanService` tests in GlindtCore (extended by WS-2/3/4) cover the model + state-machine layer; iOS-specific UI is verified manually in v2.8.0.
