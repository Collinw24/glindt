...
3. **`hermes update --yes` plumbing** — purely forward-compatible. v2.7.5 has no in-app "Update Hermes" affordance (Sparkle handles Glindt-self-update, and `hermes update` is invoked by users in their terminal). This WS adds a stub helper on `UpdaterService` (or a new `HermesUpdaterCommandBuilder` static) that the future affordance will call; the helper takes a `HermesCapabilities` and decides whether to append `--yes`. No user-visible change ships in v2.8 from this item alone — see [Out of scope](#out-of-scope).
...
#### [glindt/Packages/GlindtCore/Sources/GlindtCore/Models/ACPMessages.swift](../../Packages/GlindtCore/Sources/GlindtCore/Models/ACPMessages.swift)
...
#### [glindt/Packages/GlindtCore/Sources/GlindtCore/ACP/ACPClient.swift](../../Packages/GlindtCore/Sources/GlindtCore/ACP/ACPClient.swift)
...
#### [glindt/Packages/GlindtCore/Sources/GlindtCore/ViewModels/RichChatViewModel.swift](../../Packages/GlindtCore/Sources/GlindtCore/ViewModels/RichChatViewModel.swift)
...
The `max(...)` guard tolerates pre-v0.13 hosts that return `0` mid-session: if the agent is upgraded server-side without restarting Glindt, the count will resume at the higher value the next time `usage` carries a real number.

#### [glindt/glindt/Features/Chat/Views/SessionInfoBar.swift](../../glindt/Features/Chat/Views/SessionInfoBar.swift)
...
Notes on the visual: stick to existing `Label` + `glindtStyle(.caption)` + `GlindtColor.foregroundMuted` so the chip blends with the other counters. **Don't** invent a new `GlindtBadge` style — the row's already badge-like via the surrounding `.padding(.horizontal, GlindtSpace.s4)` background, and GlindtBadge would visually overpower a passive count. Icon: `arrow.down.right.and.arrow.up.left` (the SF Symbol for compaction). If the symbol doesn't render on macOS 14.6 — which we deploy to — fall back to a Unicode box-drawing glyph or `archivebox.fill`; flag as a follow-up rather than picking now.

#### [glindt/glindt/Features/Chat/Views/ChatTranscriptPane.swift](../../glindt/Features/Chat/Views/ChatTranscriptPane.swift)
...
Pull the capabilities from the existing `@Environment(\.hermesCapabilities)` (declared on the parent view tree per [HermesCapabilities.swift:411](../../Packages/GlindtCore/Sources/GlindtCore/Services/HermesCapabilities.swift)). If the pane doesn't currently observe it, add `@Environment(\.hermesCapabilities) private var capabilities` at the top.

#### iOS

`Glindt iOS` doesn't have a `SessionInfoBar` mirror today; the iOS chat tab uses a different header. Skip iOS in this WS — capture under [Out of scope](#out-of-scope) for follow-up. Reasoning: iOS users are read-only consumers of compression count, the data model already flows through `RichChatViewModel`, and an iOS surface isn't gated on this WS.
...
`/new` already appears in the slash menu — it's advertised by the ACP server via `available_commands_update` (handled in [RichChatViewModel:234](../../Packages/GlindtCore/Sources/GlindtCore/ViewModels/RichChatViewModel.swift) into `acpCommands`). The argumentHint comes from whatever the server sends. That means the v0.13 server will *automatically* surface a hint update because Hermes will send `"argument_hint": "[name]"` (or similar) once the new flag lands. We don't need to hardcode a Glindt-side override.

### What we change

The user-visible work here is mostly verification / smoke-testing. The mechanical changes are minor, mostly defensive:

#### [glindt/glindt/Features/Chat/Views/SlashCommandMenu.swift](../../glindt/Features/Chat/Views/SlashCommandMenu.swift)
...
There is **no in-app `hermes update` affordance** in v2.7.5. `UpdaterService` ([glindt/Core/Services/UpdaterService.swift](../../glindt/Core/Services/UpdaterService.swift)) wraps Sparkle for Glindt-self-update — that's a separate concern from updating the Hermes binary. The `hermes update` subcommand (added in v0.12 with `--check`, extended in v0.13 with `--yes`) is currently invoked by users in their terminal. The comment at [glindtApp.swift:281](../../glindt/glindtApp.swift) ("explicit refresh after `hermes update`") is aspirational — there's no UI that invokes `hermes update`.

### What this WS adds

A small forward-compatible utility so the future "Update Hermes" affordance (queued for a later release) doesn't have to re-derive flag selection. Add a single static helper on either `HermesUpdaterCommandBuilder` (new, in GlindtCore) or as a static on `UpdaterService` (Mac-only). Picking GlindtCore so iOS gets it for free, even though iOS won't ship the affordance soon either:

#### [glindt/Packages/GlindtCore/Sources/GlindtCore/Services/HermesUpdaterCommandBuilder.swift](../../Packages/GlindtCore/Sources/GlindtCore/Services/HermesUpdaterCommandBuilder.swift) (NEW)
...
Test target: a small `M0eUpdaterTests` suite (new file under `GlindtCoreTests`) covering the matrix:
...
The toggle lives in [glindt/Features/Settings/Views/Tabs/AdvancedTab.swift:129–133](../../glindt/Features/Settings/Views/Tabs/AdvancedTab.swift), inside the `Caching & Redaction` section. It's wired through `viewModel.config.redactionEnabled` ↔ `redaction.enabled`. The default for the *Glindt-side* `bool("redaction.enabled", default: false)` at [HermesFileService.swift:315](../../glindt/Core/Services/HermesFileService.swift) is `false` — meaning when the YAML key is absent, Glindt reads the toggle as off. That matches v0.12 server behavior.

In v0.13 the *server-side* default flips to ON (Hermes treats absence-of-key as redaction-enabled). Glindt's read default at the line above stays `false` because that's what we display when the user hasn't explicitly set the key — but the *meaning* of "off-with-no-key" diverges:

- pre-v0.13 host + no key → Glindt shows OFF, server treats as OFF. Honest.
- v0.13 host + no key → Glindt shows OFF, server treats as ON. **Confusing.**
...
#### [glindt/glindt/Features/Settings/Views/Tabs/AdvancedTab.swift](../../glindt/Features/Settings/Views/Tabs/AdvancedTab.swift)
...
#### [glindt/Packages/GlindtCore/Sources/GlindtCore/Models/HermesConfig.swift](../../Packages/GlindtCore/Sources/GlindtCore/Models/HermesConfig.swift)
...
#### [glindt/glindt/Core/Services/HermesFileService.swift](../../glindt/Core/Services/HermesFileService.swift)
...
#### [glindt/glindt/Features/Settings/ViewModels/SettingsViewModel.swift](../../glindt/Features/Settings/ViewModels/SettingsViewModel.swift)
...
#### [glindt/glindt/Features/Settings/Views/Tabs/GeneralTab.swift](../../glindt/Features/Settings/Views/Tabs/GeneralTab.swift)
...
`Glindt iOS` settings are read-mostly (config writes are deferred to the Mac per the existing pattern). Skip iOS for the picker; iOS just shows the value as-is wherever Settings displays it. No iOS work in this WS.
...
The xAI provider is **not in `ttsProviders` today** (verify at [SettingsViewModel.swift:32](../../glindt/Features/Settings/ViewModels/SettingsViewModel.swift) — the array reads `["edge", "elevenlabs", "openai", "minimax", "mistral", "neutts", "piper"]`, no `xai`). Hermes v0.13 adds xAI as a TTS provider (it was added earlier in fact, v0.12 — the v0.13 surface is just the *Custom Voices* / cloning support on top). This WS does both at once: add `xai` to the picker and surface the cloning-supported badge.
...
#### [glindt/glindt/Features/Settings/Views/Tabs/VoiceTab.swift](../../glindt/Features/Settings/Views/Tabs/VoiceTab.swift)
...
The badge view, using `GlindtBadge` (kind `.info`):
...
private var xaiCloningBadge: some View {
    HStack {
        Text("")
            .frame(width: 160, alignment: .trailing)
        GlindtBadge("Cloning supported", kind: .info)
...
The hint text references `hermes voice` because Glindt doesn't manage cloned voices — Hermes does, and v2.7.5 has no in-app voice-cloning UI. Capture under [Out of scope](#out-of-scope) for follow-up.
...
- `glindt/Packages/GlindtCore/Sources/GlindtCore/Services/HermesUpdaterCommandBuilder.swift` (item 3)
- `glindt/Packages/GlindtCore/Tests/GlindtCoreTests/M0eUpdaterTests.swift` (item 3 tests)

Modified:

- `glindt/Packages/GlindtCore/Sources/GlindtCore/Models/ACPMessages.swift` (item 1: `compressionCount` field)
- `glindt/Packages/GlindtCore/Sources/GlindtCore/ACP/ACPClient.swift` (item 1: decode)
- `glindt/Packages/GlindtCore/Sources/GlindtCore/ViewModels/RichChatViewModel.swift` (item 1: counter + reset + `handlePromptComplete`)
- `glindt/Packages/GlindtCore/Sources/GlindtCore/Models/HermesConfig.swift` (items 5 + 6: `display.language`, xAI voice/model fields)
- `glindt/glindt/Features/Chat/Views/SessionInfoBar.swift` (item 1: chip + props)
- `glindt/glindt/Features/Chat/Views/ChatTranscriptPane.swift` (item 1: pass-through)
- `glindt/glindt/Features/Chat/Views/SlashCommandMenu.swift` (item 2: bracket-aware hint)
- `glindt/glindt/Features/Settings/Views/Tabs/AdvancedTab.swift` (item 4: redaction hint)
- `glindt/glindt/Features/Settings/Views/Tabs/GeneralTab.swift` (item 5: language picker)
- `glindt/glindt/Features/Settings/Views/Tabs/VoiceTab.swift` (item 6: xai case + badge)
- `glindt/glindt/Features/Settings/ViewModels/SettingsViewModel.swift` (items 5 + 6: setters + lists)
- `glindt/glindt/Core/Services/HermesFileService.swift` (items 5 + 6: YAML reads)
- (possibly) `glindt/glindt/Features/Settings/Views/Components/PickerRow.swift` — add a `optionLabel:` overload (item 5, if the existing API doesn't carry one)

That's roughly **3 GlindtCore files + 7 Mac app files + 1 new file + 1 test file = ~12 files**, most edits being a few lines each.
...
1. **Pre-v0.13 host**: launch Glindt with a Hermes v0.12 binary on PATH. Verify:
...
2. **v0.13 host**: launch Glindt against the v0.13 dev branch. Verify:
   ...
   - `/new Foo Bar Baz` from the slash menu starts a session named "Foo Bar Baz" (no Glindt-side validation; Hermes handles it).
...
- Compression chip uses `GlindtColor.foregroundMuted` — verify in light + dark; ensure contrast ratio ≥ 4.5:1 against `backgroundSecondary`.
- Picker on Locale section honors keyboard navigation (Tab in / Space to open / Arrows / Return / Esc).
- "Cloning supported" badge uses `GlindtBadge(... kind: .info)` — verify color resolves correctly in both modes; not green (that's `.success`), not yellow (that's `.warning`).
...
**Q4. iOS coverage.** The plan defers iOS for items 1 (compression chip) and 5 (language picker) — iOS doesn't have a `SessionInfoBar` mirror, and iOS Settings is read-mostly. For v2.8 this is acceptable; for v2.9 we should mirror both surfaces in `Glindt iOS/`. Tracking under [Out of scope](#out-of-scope) below.
...
- GlindtCore changes: ~30 LOC across 3 files + 1 new file + 1 test file ≈ **~120 LOC**.
