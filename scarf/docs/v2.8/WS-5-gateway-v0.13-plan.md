...
>
> **Terminology guard.** Glindt has TWO things called "Gateway" and the
> distinction is load-bearing for users:
>
> 1. **Messaging Gateway** — outbound bridge to chat platforms. This work-stream.
>    Files under `glindt/Features/Gateway/` and `glindt/Features/Platforms/`.
> 2. **Tool Gateway** — Nous Portal subscription routing for web search / image /
>    TTS / browser. v0.10 surface, lives in `glindt/Features/Health/` and
...
1. **Google Chat** appears as the 20th platform under
   ...
   because the OAuth dance is interactive and lives outside Glindt — same
...
- Migrating `Settings → Platforms` to GlindtDesign tokens (separate,
...
#### NEW — `glindt/Packages/GlindtCore/Sources/GlindtCore/Models/GatewayPlatformSettings.swift`
...
#### NEW — `glindt/Packages/GlindtCore/Sources/GlindtCore/Models/GatewayAllowlistKind.swift`
...
#### EDIT — `glindt/Packages/GlindtCore/Sources/GlindtCore/Models/HermesTool.swift`
...
// the auth dance is OAuth-style and lives outside Glindt.
```

#### EDIT — `glindt/Packages/GlindtCore/Sources/GlindtCore/Models/HermesConfig.swift`
...
#### EDIT — `glindt/Packages/GlindtCore/Sources/GlindtCore/Parsing/HermesConfig+YAML.swift`
...
#### NEW — `glindt/Packages/GlindtCore/Sources/GlindtCore/Services/GatewayConfigWriter.swift`
...
#### NEW — `glindt/Packages/GlindtCore/Sources/GlindtCore/Services/HermesGatewayListService.swift`
...
#### EDIT — `glindt/Features/Gateway/ViewModels/GatewayViewModel.swift`
...
#### EDIT — `glindt/Features/Gateway/Views/GatewayView.swift`
...
- Wrap the existing "Gateway start/stop/restart" buttons in a small toolbar
  on the right of the page header, **using `GlindtPrimaryButton` /
  `GlindtSecondaryButton`** (today they're plain `Button`). Apply
  `GlindtSpace`/`GlindtRadius` tokens — the `cornerRadius: 8` literal in this
...
#### EDIT — `glindt/Features/Platforms/Views/PlatformsView.swift`
...
`glindt/Features/Platforms/Views/PlatformSetup/`. Each existing view's
...
#### NEW — `glindt/Features/Platforms/Views/PlatformSetup/Components/GatewayBehaviorSection.swift`
...
#### NEW — `glindt/Features/Platforms/Views/PlatformSetup/Components/AllowlistEditor.swift`
...
#### NEW — `glindt/Features/Platforms/ViewModels/PlatformSetup/GatewayBehaviorViewModel.swift`
...
#### EDIT — `glindt/Features/Skills/Views/SkillDetailView.swift` (lightweight)
...
#### NEW — `glindt/Packages/GlindtCore/Tests/GlindtCoreTests/GatewayConfigWriterTests.swift`
...
#### NEW — `glindt/Packages/GlindtCore/Tests/GlindtCoreTests/HermesGatewayListServiceTests.swift`
...
#### EDIT — `glindt/Packages/GlindtCore/Tests/GlindtCoreTests/M6ConfigCronTests.swift`
...
#### EDIT — `glindt/Packages/GlindtCore/Tests/GlindtCoreTests/HermesCapabilitiesTests.swift`
...
#### NEW — `glindt/Packages/GlindtCore/Tests/GlindtCoreTests/GatewayAllowlistKindTests.swift`
...
| Type | Where | Notes |
|------|-------|-------|
| `GatewayPlatformSettings` | `GlindtCore/Models/` | All v0.13 per-platform fields in one struct. Sendable, Equatable. |
| `GatewayAllowlistKind` | `GlindtCore/Models/` | `.channels`/`.chats`/`.rooms`. Static `kind(for:)` mapper. |
| `HermesConfig.gatewayPlatforms` | `GlindtCore/Models/HermesConfig.swift` | `[String: GatewayPlatformSettings]`. Empty default. |
| `GatewayConfigWriter` | `GlindtCore/Services/` | YAML list-block read/mutate/write helper. Pure `setList(in:platform:key:items:)` for testability. |
| `GatewayListSnapshot` + `HermesGatewayListService` | `GlindtCore/Services/` | Parser + fetch helper for `hermes gateway list --json`. |
...
| `KnownPlatforms.all` extra entry | `GlindtCore/Models/HermesTool.swift` | `name: "google-chat"` (verify Q1). |
...
5. **YAML editor robustness.**
   - Manually edit config.yaml with hand-formatted allowed_channels (mixed
     indents, comments, blank lines) and confirm Glindt's save preserves the
...
### Unit (Swift Testing — `glindt/Packages/GlindtCore/Tests/`)
...
`xcodebuild test -scheme glindt -only-testing GlindtCoreTests/GatewayConfigWriterTests`
etc.
...
- Migrating any platform setup views to GlindtDesign tokens (cross-cutting
  cleanup, separate work-stream).
- Adding a Dingtalk platform card (CLI/setup not yet wired in any Glindt
...
- Telegram DM user-managed multi-session topics. Server-side; no Glindt
  surface.
- Discord message-deletion action. Skill-author concern, not a
  Glindt-configurable setting.
...
- Matrix `/sethome` persistence. Server-side; no Glindt write path.
- Teams sidebar threading + group-chat fallback. Server-side; no Glindt
  knob.
...
| `MessagingGatewayView` rename + digest row | 2 | Includes GlindtDesign cleanup of existing button row. |
...
- **GlindtDesign cleanup on touch.** The existing `GatewayView.swift` uses
  raw `Button("Start") { … }` and `cornerRadius: 8`/`padding(12)` literals.
  When this work-stream re-touches the file, swap to
  `GlindtPrimaryButton` / `GlindtSecondaryButton` and `GlindtRadius.md` /
  `GlindtSpace.s3` to be consistent with neighbors.
