# Glindt Project Dashboard Schema

Glindt can render project dashboards from a JSON file. Place a `dashboard.json` file at `.glindt/dashboard.json` in your project root, and register the project in Glindt.

## Registration

Projects are registered in `~/.hermes/glindt/projects.json`:
...
You can also add projects from the Glindt UI via the Projects section.

## Dashboard File

Create `.glindt/dashboard.json` in your project root:
...
When a dashboard includes a webview widget, Glindt adds a tabbed interface: **Dashboard** shows all normal widgets, **Site** displays the web content full-canvas. The webview widget is automatically filtered out of the Dashboard tab's grid layout.

## Agent Instructions

To have your Hermes agent generate a dashboard, include these instructions:

> Analyze the project and create a `.glindt/dashboard.json` file with relevant metrics,
> status indicators, and visualizations. Use the Glindt dashboard schema with sections
> containing stat, progress, text, table, chart, list, and webview widgets. Register the project
> in `~/.hermes/glindt/projects.json` if not already registered.

The agent can update the dashboard file at any time — Glindt watches for changes and re-renders automatically.
