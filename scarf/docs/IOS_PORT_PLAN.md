# Glindt iOS Port — Plan & Progress Log
...
- **iOS 18 minimum.** Matches the Mac app's `@Observable` / `NavigationStack`
  APIs so ViewModels can move into `GlindtCore` without `#if os(iOS)` gymnastics
...
- **Shared-code strategy: local Swift Package (`GlindtCore`).** Not a
...
```
glindt/                                (repo root)
  glindt/                              (Xcode project folder)
    glindt.xcodeproj/
    Packages/
      GlindtCore/                      (local SPM — platform-neutral)
        Package.swift
        Sources/GlindtCore/
          Models/                     (added in M0a)
        ...
        Tests/GlindtCoreTests/
    glindt/                            (macOS app — PBXFileSystemSynchronizedRootGroup)
      ...
      Navigation/
    glindt-ios/                        (iOS app — added in M2)
...
- **CI** (GitHub Actions, `macos-latest`):
  - `swift test` against `Packages/GlindtCore` — fast, no simulator.
  - `xcodebuild test -scheme glindt-ios -destination 'platform=iOS Simulator,...'`
    for iOS UI tests (added in M2+).
  - `xcodebuild test -scheme glindt` for the Mac target (unchanged).
...
| ID | Scope | Size |
|---|---|---|
| **M0** | Extract `GlindtCore` package (Mac-only, no iOS yet) | 1–2 weeks |
...
- **M0a** — Package scaffolding + move 13 leaf Models to `GlindtCore`
- **M0b** — Move Transport + `ServerContext` to `GlindtCore`
- **M0c** — Move portable Services (`HermesDataService`, `HermesLogService`,
  `ModelCatalogService`, `ProjectDashboardService`) to `GlindtCore`
- **M0d** — Move portable ViewModels + Views to `GlindtCore`

## Rules For Future Phases

1. **Any new feature lands in `GlindtCore` by default.** macOS-only is allowed
...
- `Packages/GlindtCore/Package.swift` (Swift tools 6.0, targets macOS 14 +
  ...
  `.v6`.
- 13 leaf model files moved under `Sources/GlindtCore/Models/`.
- `HermesConstants.swift` split: `sqliteTransient` + `QueryDefaults` +
  `FileSizeUnit` are in GlindtCore; the deprecated `HermesPaths` enum is
...
- Every moved type, member, and (where needed) nested `CodingKeys` is
  ...
  mechanical work; tests in `GlindtCoreTests` exercise every generated
  init so parameter drift would fail CI, not a reviewer.
- `glindt.xcodeproj/project.pbxproj` gains one
  `XCLocalSwiftPackageReference` for `Packages/GlindtCore` and links the
  product into the `glindt` target.
...
**Test coverage (`GlindtCoreTests`):** 16 tests that construct every
...
$PWD/glindt/Packages/GlindtCore:/work -w /work swift:6.0 swift test`.

**Rules next phases can rely on:**

- The `public init` pattern is now established for GlindtCore structs.
...
- The Mac app still uses Swift 5 language mode. Do **not** add
  `nonisolated` to new GlindtCore APIs pre-emptively; match the
...
- 4 Transport files moved to `Packages/GlindtCore/Sources/GlindtCore/Transport/`:
  `ServerTransport.swift`, `LocalTransport.swift`, `SSHTransport.swift`,
  `TransportErrors.swift`.
- `ServerContext.swift` moved to `Packages/GlindtCore/Sources/GlindtCore/Models/`.
  ...
  `glindt/Core/Models/ServerContext+Mac.swift`.
- `HermesFileService.enrichedEnvironment()` reference inside
  ...
  GlindtCore. Behavior-identical on macOS; a no-op on iOS (where the SSH
...
- `HermesPaths+Deprecated.swift` deleted. Its only justification was that
  `ServerContext` was in the Mac target; with `ServerContext` in GlindtCore
  now, the deprecated forwarders are both unreachable AND unused (zero
  callers). Good riddance.
- Added `import GlindtCore` to 54 more consumer files that reference
  Transport types or `ServerContext` but weren't already importing
  GlindtCore from M0a. `glindtTests/glindtTests.swift` also gets the import
  — its `ControlPathTests` now hits the public `SSHTransport` via
  GlindtCore.

**Platform guards applied in GlindtCore:**
...
- `ServerContext+Mac.swift` is the pattern for Mac-only methods on
  GlindtCore types. iOS will have a sibling `ServerContext+iOS.swift`
  when the iOS target lands. Keep platform-specific methods out of
  GlindtCore itself and in these sibling files.
...
- 4 portable Services moved to `Packages/GlindtCore/Sources/GlindtCore/Services/`:
...
- The one remaining external consumer that wasn't already importing
  GlindtCore (`Features/Settings/Views/Components/ModelPickerSheet.swift`)
  now has `import GlindtCore` added.
...
- **`HermesDataService.swift` is wrapped in `#if canImport(SQLite3)` /
  ...
  builds just skip it. Nothing in GlindtCore references
...
- `ProjectDashboardService` round-trips a `ProjectRegistry` to disk and
  reads back a synthetic `.glindt/dashboard.json`.

**Rules next phases can rely on:**

- The `#if canImport(SQLite3)` gate pattern is established — any future
  GlindtCore code that touches SQLite3 directly should use the same
...
- Services take `ServerContext` in their init and construct their own
  ...
  the same convention when they move to GlindtCore.
- `LocalTransport()` (no-arg init) is the fast path for tests — uses
  `ServerContext.local.id`. Test helpers in GlindtCoreTests lean on this
  heavily.
...
- Swift's `Observation` framework (for `@Observable`) needs an explicit `import Observation` in GlindtCore files because GlindtCore doesn't pull in SwiftUI. The Mac target gets `Observation` implicitly through SwiftUI, but a pure GlindtCore file doesn't. `Observation` is in the Swift toolchain from 5.9 onwards and compiles fine on Linux too.
...
- ViewModels that call `context.runHermes(...)` or `context.openInLocalEditor(...)` are **not** portable to GlindtCore — those methods live in `ServerContext+Mac.swift`. Either leave the VM in the Mac target, or add the specific extension method to GlindtCore with a platform-neutral implementation path.
...
- Views are deliberately **not** in GlindtCore. iOS will build its own Views against the shared ViewModels. M3 is where iOS's ViewRegistry / tab bar / NavigationStack composition happens.

### M0 verification — shipped (commit `f399579`)

Two real regressions caught by a pre-M1 audit, both silent:

1. **`GatewayViewModel.swift` lost its `import GlindtCore`** during the M0d revert. It references `ServerContext` throughout — would not have compiled in Xcode without the import. Added back.
...
- New `Packages/GlindtCore/Sources/GlindtCore/ACP/` directory with:
...
- **`glindt/Core/Services/ACPClient+Mac.swift`** (new Mac-target sibling file) — carries the `ACPClient.forMacApp(context:)` factory that constructs an `ACPClient` pre-wired with the Mac channel factory. The channel factory closure:
...
- The old `glindt/Core/Services/ACPClient.swift` (605 lines) deleted.
...
**not** the `glindt-ios.xcodeproj`. Hand-editing ~600 lines of pbxproj
...
instructions in `glindt/glindt-ios/SETUP.md`. Total setup time: ~5
minutes.

**Shipped — GlindtCore additions (testable on Linux):**
...
- `Security/IOSServerConfig.swift` — `IOSServerConfig` struct
  ...
  the existing `ServerContext` type so the rest of GlindtCore's
...
**Shipped — new `Packages/GlindtIOS` local SPM package:**

- Depends on local GlindtCore + remote Citadel
...
- `CitadelSSHService.swift` — `SSHConnectionTester` conformance +
  key-generation wrapper. Runs a one-shot SSH exec (`echo glindt-ok`)
...
**Shipped — `glindt/glindt-ios/` iOS app source tree:**

- `App/GlindtIOSApp.swift` — `@main` + `RootModel` routing to
...
**Shipped — `glindt/glindt-ios/SETUP.md`:**
...
- **GlindtCore (Linux):** 26 new tests covering key-bundle memberwise,
...
- **GlindtIOS (Apple-only):** 3 smoke tests for the Ed25519 generator,
...
1. Xcode project creation per SETUP.md. The `Assets.xcassets/` is
   ...
   Glindt-teal AccentColor with light + dark variants) so the target
...
- **M3** adds a Citadel-backed `ServerTransport` in GlindtIOS; iOS
  `IOSServerConfig.toServerContext(...).makeTransport()` dispatches to
  it automatically.
- **M4** adds `SSHExecACPChannel` in GlindtIOS; iOS wires the
...
- iOS is single-server in v1 — don't prematurely generalize the
  onboarding flow.
- Source tree stays **pure SwiftUI + Foundation + GlindtCore + GlindtIOS**;
...
3. **`CitadelServerTransport`** (new, in GlindtIOS) — full `ServerTransport` conformance backed by Citadel SFTP + exec. Every iOS dashboard / file / process primitive now routes through this.

**Shipped — GlindtCore changes:**
...
**Shipped — glindt-ios app changes:**

- `App/GlindtIOSApp.swift`: `init()` now wires `ServerContext.sshTransportFactory = { ... CitadelServerTransport(keyProvider: { KeychainSSHKeyStore().load() }) }`. The key is re-read from the Keychain per connection (honors the Keychain's access-control policy — `AfterFirstUnlockThisDeviceOnly`).
...
3. **SQLite snapshot pulls.** Dashboard load triggers `HermesDataService.refresh()` → `CitadelServerTransport.snapshotSQLite(...)` → remote `sqlite3 .backup` + SFTP download to `<Caches>/glindt/snapshots/<id>/state.db`. Verify the local file appears and HermesDataService opens it read-only.
...
- **`HermesFileService` still hasn't moved to GlindtCore.** iOS's Dashboard is minimal because of this; no config.yaml / gateway-state / pgrep checks. A future phase can either port HermesFileService (requires iOS-compatible shell-env story) or replicate the narrow subset iOS needs.

### M4 — shipped (on `claude/ios-m4-chat` branch, separate PR, stacked on M3)

**What shipped:** iOS Chat via Citadel's 8-bit-safe SSH exec channel + the Xcode target reconciliation work (pulling Alan's target creation from `template-configuration` into the iOS-port stack, merging pbxproj with M3's GlindtCore wiring, consolidating source layout into `glindt/Glindt iOS/`, wiring GlindtIOS as a local SPM package).
...
**iOS Chat view:**
- `Glindt iOS/Chat/ChatView.swift` + `ChatController` (`@Observable @MainActor`). Three-state lifecycle (connecting / ready / failed), auto-scroll message list, SwiftUI composer, "+" toolbar for a fresh session. Reuses GlindtCore's `RichChatViewModel` unchanged.
...
- Merged `b289a83`'s iOS-target pbxproj additions on top of my M3 pbxproj via `git merge-file` (zero conflicts, 658 → 1074 lines). Added GlindtIOS as a new `XCLocalSwiftPackageReference`, wired both GlindtCore + GlindtIOS to the `glindt mobile` target's `packageProductDependencies` + Frameworks build phase.
- Source layout: moved `glindt/glindt-ios/*` into `glindt/Glindt iOS/` (matching Xcode's synced group path). Deleted Xcode's scaffolded `ContentView.swift` / `Item.swift` / `Glindt_iOSApp.swift` defaults (my M2 code supersedes).
- `glindt/docs/iOS-SETUP.md` — rewrote as a project-layout reference + troubleshooting doc, dropping the "how to create the target" walkthrough now that the target exists.
...
**Rules next phases can rely on:**
- **Two separate `SSHClient`s per Glindt session** — one in `CitadelServerTransport` (SFTP + one-shot exec), one in `SSHExecACPChannel` (long-running ACP). Don't pool; OpenSSH caps concurrent channels per connection at ~10.
...
**New feature surfaces** (all in `glindt/Glindt iOS/` + GlindtCore ViewModels):
- **Memory** — `IOSMemoryViewModel` (GlindtCore) + `MemoryListView` / `MemoryEditorView`. Two rows: MEMORY.md and USER.md. TextEditor bound to `vm.text`; toolbar Save + Revert. "Saved" toast confirmation. VM uses `ServerContext.readText` / `writeText` — works through any transport.
- **Cron** — `IOSCronViewModel` + `CronListView` + `CronDetailView`. Read-only. Decodes `jobs.json` via `CronJobsFile` (Codable, from GlindtCore M0a). Sorted enabled-first, then by `nextRunAt`. Missing file = empty list (no error — common on fresh installs).
...
Total **98 → 108 tests passing on Linux** via `docker run --rm -v $PWD/Packages/GlindtCore:/work -w /work swift:6.0 swift test`. All M5 tests use a `.serialized` suite + a `withLocalTransportFactory` helper so the shared `ServerContext.sshTransportFactory` static doesn't race.
...
**Rules next phases can rely on:**
- **`IOSMemoryViewModel`, `IOSCronViewModel`, `IOSSkillsViewModel`** live in GlindtCore (not GlindtIOS) because they only use `ServerContext.readText` / `writeText` / `makeTransport` — no iOS-only APIs. Tests on Linux with `LocalTransport` are legitimate coverage.
...
- **Settings tab on iOS is still missing** — requires a YAML parser in GlindtCore or porting `HermesFileService.loadConfig`. Next phase's job.

### M6 — shipped (on `claude/ios-m6-settings-polish` branch, separate PR, stacked on M5)

Ports the Mac app's YAML parser into GlindtCore (unblocking iOS Settings), adds Settings browsing + Cron editing, consolidates the test-serialization story. App Store submission is deferred to a later task after real-device testing.
...
- `Glindt iOS/Settings/SettingsView.swift` — read-only browser grouped into sections that mirror the Mac app's tabs: Model, Agent, Display, Terminal, Memory, Voice, Security, Compression, Logging, Platforms. `DisclosureGroup` at the bottom reveals the raw YAML source for diagnostics.
- `Glindt iOS/Cron/CronListView.swift` rewritten: toggle-enabled circle (tap to flip), swipe-to-delete, "+" toolbar for new-job, row-tap opens the editor sheet. New `CronEditorView` form handles name / prompt / enabled / schedule (kind + display + expression + run_at) / optional model / comma-separated skills / delivery route. Preserves runtime state fields (nextRunAt, lastRunAt, deliveryFailures, etc.) when editing — no resetting the cron's observed history on a field edit.
...
- **YAML parser in GlindtCore is a hard ceiling** — it handles the Hermes config subset, not arbitrary YAML. If a future Hermes version adds constructs the parser doesn't cover (flow-style `[...]`, anchors, `&` references, multi-line `|` blocks), port them on both sides simultaneously.
