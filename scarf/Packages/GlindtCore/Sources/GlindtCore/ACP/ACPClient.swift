...
/// of the chosen mode — that's a server-side guardrail Glindt doesn't
/// override.
///
...
public actor ACPClient {
    #if canImport(os)
    private let logger = Logger(subsystem: "com.glindt", category: "ACPClient")
    #endif
    ...
    /// prompt server-side regardless of the chosen mode — Glindt doesn't
    ...
    /// without the prefix, per [#97](https://github.com/awizemann/glindt/issues/97)).
    ///
    ...
    ///   sites that worked before [#97](https://github.com/awizemann/glindt/issues/97)
...
public enum ACPErrorHint {
    ...
    public static func classify(errorMessage: String, stderrTail: String) -> Classification? {
        ...
        // --type oauth` is the same code path Glindt already drives via
        ...
        if haystack.range(of: #"No\s+(Anthropic|OpenAI|OpenRouter|Gemini|Google|Groq|Mistral|XAI)?\s*credentials\s+found"#,
            ...
            return Classification(hint: "Hermes can't find your AI provider credentials. Set `ANTHROPIC_API_KEY` (or similar) in `~/.hermes/.env` or your shell profile, then restart Glindt.")
        }
        if let match = haystack.range(of: #"No such file or directory:\s*'([^']+)'"#,
            ...
            if let nameStart = matched.range(of: "'"),
               let nameEnd = matched.range(of: "'", range: nameStart.upperBound..<matched.endIndex) {
                ...
                return Classification(hint: "Hermes couldn't find `\(name)` on PATH. If you use nvm/asdf/mise, make sure it's exported in `~/.zprofile` (not only `~/.zshrc`), then restart Glindt.")
            }
            return Classification(hint: "Hermes couldn't find a required binary on PATH. Check that your shell's PATH is exported in `~/.zprofile`, then restart Glindt.")
        }
