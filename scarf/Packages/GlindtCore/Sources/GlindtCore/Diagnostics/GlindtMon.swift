...
/// Lightweight performance instrumentation for the Glindt app family.
///
...
public enum GlindtMon {
    ...
    public static func install(_ backends: [GlindtMonBackend]) {
        lock.lock()
        defer { lock.unlock() }
        installed = backends
    ...
    /// this in production.
    public static var currentBackends: [GlindtMonBackend] {
        lock.lock()
        defer { lock.unlock() }
        return installed
    }
    ...
    private static let lock = GlindtMonLock()
    nonisolated(unsafe) private static var installed: [GlindtMonBackend] = []
    ...
    private static func snapshotBackends() -> [GlindtMonBackend] {
        lock.lock()
        defer { lock.unlock() }
        return installed
    }
...
public struct GlindtMonSample: Sendable, Hashable {
    public enum Kind: String, Sendable, Codable {
        case interval
        case event
    }
    public let category: GlindtMon.Category
    ...
    public static func == (lhs: GlindtMonSample, rhs: GlindtMonSample) -> Bool {
...
extension GlindtMon {
    public typealias Sample = GlindtMonSample
}
...
public protocol GlindtMonBackend: Sendable {
    func record(_ sample: GlindtMon.Sample)
}
...
@usableFromInline
final class GlindtMonLock: @unchecked Sendable {
