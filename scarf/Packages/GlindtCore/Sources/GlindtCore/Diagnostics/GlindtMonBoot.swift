...
/// Boot-time wiring for GlindtMon. Both app targets call
/// `GlindtMonBoot.configure(...)` at launch and again whenever the user
...
public enum GlindtMonBoot {
    ...
    /// so `defaults read com.glindt.app GlindtMonMode` works on either.
    public static let userDefaultsKey = "GlindtMonMode"
    ...
    @discardableResult
    public static func configure(mode: Mode) -> GlindtMonRingBuffer? {
        switch mode {
        case .off:
            GlindtMon.install([])
            ...
            return nil
        case .signpostOnly:
            GlindtMon.install([GlindtMonSignpostBackend()])
            sharedRingBuffer = nil
            return nil
        case .full:
            let ring = GlindtMonRingBuffer()
            sharedRingBuffer = ring
            GlindtMon.install([
                GlindtMonSignpostBackend(),
                ring,
                GlindtMonLoggerBackend()
            ])
            return ring
        }
    }
    ...
    nonisolated(unsafe) public private(set) static var sharedRingBuffer: GlindtMonRingBuffer?
}
