...
public final class GlindtMonRingBuffer: GlindtMonBackend, @unchecked Sendable {
    public let capacity: Int

    private let lock = GlindtMonLock()
    private var storage: [GlindtMon.Sample?]
    ...
    public func samples() -> [GlindtMon.Sample] {
        lock.lock()
        defer { lock.unlock() }
        if !didWrap {
    ...
    public func summary() -> [GlindtMonStat] {
        ...
        var stats: [GlindtMonStat] = []
        for (key, _) in counts {
            let durations = buckets[key] ?? []
            let kind = kinds[key] ?? .event
            stats.append(GlindtMonStat(
    ...
    private struct BucketKey: Hashable {
        let category: GlindtMon.Category
        let name: String
    }
...
public struct GlindtMonStat: Sendable, Hashable, Codable {
    public let category: GlindtMon.Category
    public let name: String
    public let kind: GlindtMon.Sample.Kind
