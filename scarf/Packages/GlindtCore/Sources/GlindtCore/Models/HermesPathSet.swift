...
public struct HermesPathSet: Sendable, Hashable {
    ...
    /// command. Read-only from Glindt's side (v1); Hermes owns the write
    ...
    public nonisolated var glindtDir: String { home + "/glindt" }
    public nonisolated var projectsRegistry: String { glindtDir + "/projects.json" }

    /// Maps Hermes session IDs to the Glindt project path a chat was
    /// started for. Glindt-owned; Hermes never touches this file.
    public nonisolated var sessionProjectMap: String { glindtDir + "/session_project_map.json" }
    ...
    public nonisolated var nousModelsCache: String { glindtDir + "/nous_models_cache.json" }
    ...
    /// JSON, glindt-owned, survives offline runs so the catalog browser
    /// still has something to render. Wiped by a Hermes home reset.
    public nonisolated var catalogCache: String { glindtDir + "/catalog_cache.json" }
    /// User-saved model presets. Glindt-owned; Hermes never touches this
    /// file. Read by `ModelPresetService`, applied at ACP session boot
    /// via `session/set_model` and at `hermes -z` invocation via
    /// `-m`/`--provider` flags.
    public nonisolated var modelPresetsJSON: String { glindtDir + "/model_presets.json" }
    /// Global Glindt slash commands available in every chat (not just
    ...
    /// `<project>/.glindt/slash-commands/` continue to layer on top.
    public nonisolated var globalSlashCommandsDir: String { glindtDir + "/slash-commands" }
    ...
    /// method doesn't need to relaunch Glindt.
