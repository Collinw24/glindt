...
/// `<project>/.glindt/slash-commands/<name>.md` as a Markdown file with
/// YAML frontmatter — Glindt-side primitive, not a Hermes feature.
///
/// The command is invoked via the chat slash menu like any other command,
/// but Glindt intercepts the invocation client-side: the body is treated as
...
/// it sees the expanded prompt prefixed with a `<!-- glindt-slash:<name> -->`
/// marker so it can correlate.
///
/// **Why client-side expansion.** Hermes has no project-scoped slash
/// command concept. Doing the substitution in Glindt means commands work
...
public extension ProjectSlashCommand {
    ...
    /// authored in Glindt round-trip cleanly through `.glindttemplate`.
