...
public struct SSHConfig: Sendable, Hashable, Codable {
    ...
    /// Override for where Glindt installs new project templates on this host.
...
public struct ServerContext: Sendable, Hashable, Identifiable {
    ...
    /// doesn't exist, so `glindt-ios` wires this factory at launch to
...
///
/// Guarded on `canImport(SwiftUI)` so GlindtCore still compiles on Linux
