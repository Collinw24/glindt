...
/// Top-level manifest for a `.glindtbackup` archive.
///
/// **Archive layout** (`.glindtbackup` is a plain ZIP):
/// ```
/// <name>.glindtbackup
...
/// memories + glindt sidecars + plugins/profiles), each project's full file
...
public struct BackupManifest: Codable, Sendable, Equatable {
    ...
    /// Magic string. Lets a future Glindt reject `.zip` files that aren't
    ...
    public static let kindMagic = "glindt-server-backup"
...
public enum BackupArchiveLayout {
    ...
    public static let archiveExtension = "glindtbackup"
