...
public struct DisplaySettings: Sendable, Equatable {
    ...
    /// formatting and Glindt doesn't render them separately. Persisted
    /// via `hermes config set display.timestamps <bool>`. Pre-v0.14
    /// hosts ignore the key; Glindt hides the toggle when
...
public struct TerminalSettings: Sendable, Equatable {
    ...
    /// Pre-v0.14 hosts ignore the key; Glindt hides the editor row when
...
/// block; Glindt hides the whole Secrets tab when
...
public struct HermesConfig: Sendable {
    ...
    /// platform. Glindt reads for display; edits go through Hermes CLI.
    ...
    /// session. Off by default in Glindt's parser per WS-6 plan
