...
// imports SQLite3) and to keep GlindtCore's compile target narrow.
...
public actor HermesDataService {
    ...
    public func fetchMessagesOutcome(
        ...
        await GlindtMon.measureAsync(.sessionLoad, "mac.fetchMessages") {
            ...
            do {
                ...
                GlindtMon.event(.sessionLoad, "mac.fetchMessages.rows", count: messages.count)
                ...
                GlindtMon.event(.sessionLoad, "mac.fetchMessages.transportError", count: 1)
    ...
    public func fetchSkeletonMessages(
        ...
        await GlindtMon.measureAsync(.sessionLoad, "mac.fetchSkeletonMessages") {
            let sql: String
            let params: [SQLValue]
            if let before {
            ...
            do {
                let rows = try await backend.query(sql, params: params)
                let messages = rows.map { messageFromRow($0) }.reversed() as [HermesMessage]
                GlindtMon.event(.sessionLoad, "mac.fetchSkeletonMessages.rows", count: messages.count)
                return MessageFetchOutcome(messages: messages, transportError: nil)
            } catch let BackendError.transport(reason) {
                GlindtMon.event(.sessionLoad, "mac.fetchSkeletonMessages.transportError", count: 1)
                return MessageFetchOutcome(messages: [], transportError: reason)
            } catch {
                return MessageFetchOutcome(messages: [], transportError: nil)
            }
    ...
    public func hydrateAssistantToolCalls(
        ...
        return await GlindtMon.measureAsync(.sessionLoad, "mac.hydrateToolCalls") {
            ...
            for page in pages {
                ...
                if Task.isCancelled {
                    GlindtMon.event(.sessionLoad, "mac.hydrateToolCalls.cancelled", count: 1)
                    return out
                }
                ...
                do {
                    ...
                    GlindtMon.event(.sessionLoad, "mac.hydrateToolCalls.pageTimeout", count: 1)
                    ...
                    for id in page {
                        ...
                        do {
                            ...
                            GlindtMon.event(.sessionLoad, "mac.hydrateToolCalls.singleTimeout", count: 1)
            ...
            GlindtMon.event(.sessionLoad, "mac.hydrateToolCalls.rows", count: out.count)
            return out
        }
    }
    ...
    public func fetchToolResultsInRange(
        ...
        await GlindtMon.measureAsync(.sessionLoad, "mac.hydrateToolResults") {
            ...
            do {
                ...
                GlindtMon.event(.sessionLoad, "mac.hydrateToolResults.rows", count: messages.count)
                return messages
            } catch {
    ...
    public func fetchRecentToolCallSkeleton(
        ...
        await GlindtMon.measureAsync(.sessionLoad, "mac.fetchToolCallSkeleton") {
            ...
            do {
                let rows = try await backend.query(sql, params: [.integer(Int64(limit))])
                let messages = rows.map { messageFromRow($0) }
                GlindtMon.event(.sessionLoad, "mac.fetchToolCallSkeleton.rows", count: messages.count)
                return MessageFetchOutcome(messages: messages, transportError: nil)
            } catch let BackendError.transport(reason) {
                GlindtMon.event(.sessionLoad, "mac.fetchToolCallSkeleton.transportError", count: 1)
    ...
    public func fetchRecentToolCallsOutcome(
        ...
        await GlindtMon.measureAsync(.sessionLoad, "mac.fetchRecentToolCalls") {
            let sql = """
            ...
            do {
                let rows = try await backend.query(sql, params: [.integer(Int64(limit))])
                let messages = rows.map { messageFromRow($0) }
                GlindtMon.event(.sessionLoad, "mac.fetchRecentToolCalls.rows", count: messages.count)
                return MessageFetchOutcome(messages: messages, transportError: nil)
            } catch let BackendError.transport(reason) {
                GlindtMon.event(.sessionLoad, "mac.fetchRecentToolCalls.transportError", count: 1)
    ...
    public func fetchSessionPreviews(limit: Int = QueryDefaults.sessionPreviewLimit) async -> [String: String] {
        ...
        // GlindtMon instrumentation + transport-error logging for
        ...
        await GlindtMon.measureAsync(.sessionLoad, "mac.fetchSessionPreviews") {
            let sql = """
            ...
            do {
                ...
                GlindtMon.event(.sessionLoad, "mac.fetchSessionPreviews.rows", count: previews.count)
                return previews
            } catch let BackendError.transport(reason) {
                GlindtMon.event(.sessionLoad, "mac.fetchSessionPreviews.transportError", count: 1)
    ...
    /// GlindtMon `mac.loadRecentSessions` traces). Halves the
