...
/// with a `SELECT '__GLINDT_RS_BEGIN__N' AS marker;` synthesised line so
...
public actor RemoteSQLiteBackend: HermesQueryBackend {
    ...
    #if canImport(os)
    private static let logger = Logger(subsystem: "com.glindt", category: "RemoteSQLiteBackend")
    #endif
    ...
    /// or content fragment.
    private static let batchMarkerPrefix = "__GLINDT_RS_BEGIN__"
    ...
    public func query(_ sql: String, params: [SQLValue]) async throws -> [Row] {
        ...
        // of spawning a new SSH subprocess. Surfaces in GlindtMon as
        ...
        if let existing = inFlightQueries[inlined] {
            GlindtMon.event(.sqlite, "query.coalesced", count: 1)
        ...
        let task = Task<[Row], Error> { [self] in
            try await GlindtMon.measureAsync(.sqlite, "query") {
                let dbPath = context.paths.stateDB
                let script = """
                sqlite3 -readonly -json \(quoteForRemoteShell(dbPath)) <<'__GLINDT_SQL__'
                \(inlined)
                __GLINDT_SQL__
                """
                ...
                GlindtMon.event(.sqlite, "query.rows", count: rows.count, bytes: result.stdout.count)
                return rows
            }
        }
    ...
    public func queryBatch(_ statements: [(sql: String, params: [SQLValue])]) async throws -> [[Row]] {
        try await GlindtMon.measureAsync(.sqlite, "queryBatch") {
    ...
    private func _queryBatchImpl(_ statements: [(sql: String, params: [SQLValue])]) async throws -> [[Row]] {
        ...
        // each statement's result set. `SELECT '__GLINDT_RS_BEGIN__N'`
    ...
    private func parseBatchResultSets(_ stdout: String, expectedCount: Int) throws -> [[Row]] {
        ...
        // SELECT emits a one-row array `[{"marker":"__GLINDT_RS_BEGIN__N"}]`;
    ...
    /// If the array's single row is a marker `{"marker":"__GLINDT_RS_BEGIN__N"}`,
