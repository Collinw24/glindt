...
///
/// Pre-v0.11 Glindt hardcoded `~/.hermes` and ignored `active_profile`,
/// which meant `hermes profile use <name>` left Glindt reading the wrong
...
/// that `hermes profile use other` followed by a Glindt operation picks up
...
public enum HermesProfileResolver {
    ...
    private static let logger = Logger(subsystem: "com.glindt.app", category: "HermesProfileResolver")
    ...
    /// failure mode is "Glindt shows what it always showed before."
    ///
    /// **Test override.** Setting `GLINDT_HERMES_HOME` in the environment
    ...
    /// Glindt against an isolated tmpdir Hermes home so the user's real
    ...
    public static func resolveLocalHome() -> String {
        if let override = glindtHermesHomeOverride() {
            return override
        }
    ...
    /// Surfaced in UI chrome so users can see which profile Glindt is
    ...
    public static func activeProfileName() -> String {
        if glindtHermesHomeOverride() != nil {
    ...
    /// `setenv("GLINDT_HERMES_HOME", ...)`.
    public static let testHomeMarkerFilename = ".glindt-test-home-marker"

    /// Read `GLINDT_HERMES_HOME` from the environment. Returns `nil` when
    ...
    ///      accidentally exports `GLINDT_HERMES_HOME` to the wrong value
    ...
    private static func glindtHermesHomeOverride() -> String? {
        guard let raw = ProcessInfo.processInfo.environment["GLINDT_HERMES_HOME"] else {
            return nil
        }
        let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return nil }
        guard trimmed.hasPrefix("/") else {
            logger.warning("GLINDT_HERMES_HOME=\(trimmed, privacy: .public) is not absolute; ignoring.")
            return nil
        }
        let markerPath = trimmed + "/" + testHomeMarkerFilename
        guard FileManager.default.fileExists(atPath: markerPath) else {
            logger.warning("GLINDT_HERMES_HOME=\(trimmed, privacy: .public) lacks sentinel marker (\(testHomeMarkerFilename, privacy: .public)); ignoring to protect real ~/.hermes.")
            return nil
        }
        return trimmed
    }
