...
/// Pure block-splice logic for the Glindt-managed region of a project's
/// `<project>/AGENTS.md`. Shared by Mac (which wraps it in
/// `ProjectAgentContextService` with template-manifest + cron-aware
/// block rendering) and GlindtGo (which renders a simpler block and
/// writes it over SFTP).
///
...
public enum ProjectContextBlock {
    ...
    public static let beginMarker = "<!-- glindt-project:begin -->"
    public static let endMarker = "<!-- glindt-project:end -->"
    ...
    /// identical inputs.
    ///
    /// Called by GlindtGo's ChatController.startNewSession when the
    ...
    /// Render a minimal Glindt-managed block for iOS GlindtGo usage.
    /// Omits the template-manifest + cron-job sections that the Mac
    /// service fills in — GlindtGo v1 doesn't surface those concepts
    ...
    /// do I have available?" and recognise the `<!-- glindt-slash:<name> -->`
    ...
    public static func renderMinimalBlock(
        ...
        lines.append("## Glindt project context")
        lines.append("")
        lines.append("_Auto-generated by Glindt — do not edit between the begin/end markers._")
        lines.append("")
        lines.append("You are operating inside a Glindt project named **\"\(projectName)\"**. This chat session's working directory is the project's directory — path-relative tool calls resolve inside the project.")
        lines.append("")
        lines.append("- **Project directory:** `\(projectPath)`")
        lines.append("- **Dashboard:** `\(projectPath)/.glindt/dashboard.json`")
        if let names = slashCommandNames, !names.isEmpty {
            let formatted = names.sorted().map { "`/\($0)`" }.joined(separator: ", ")
            lines.append("- **Project slash commands:** \(formatted). The user invokes these via the chat slash menu; you'll see the expanded prompt as a normal user message preceded by `<!-- glindt-slash:<name> -->`.")
        }
        lines.append("")
        lines.append("Any content below this block is template- or user-authored; preserve and defer to it for project-specific behavior. Do NOT modify content inside these markers — Glindt rewrites this block on every project-scoped chat start.")
