...
/// Owns the sidecar that attributes Hermes session IDs to Glindt
/// project paths. Promoted to GlindtCore in M9 #4.2 so GlindtGo can
/// write project attributions over SFTP — the whole service is
/// transport-based, so Mac and iOS share the same code path.
///
/// File: `~/.hermes/glindt/session_project_map.json` (resolved via
...
/// disk. Concurrent writers (two Glindt windows on the same
...
public struct SessionAttributionService: Sendable {
    #if canImport(os)
    private static let logger = Logger(subsystem: "com.glindt", category: "SessionAttributionService")
    #endif
    ...
    private func persist(_ map: SessionProjectMap) {
        let path = context.paths.sessionProjectMap
        let transport = context.makeTransport()
        let dir = context.paths.glindtDir
        do {
