...
///
/// Glindt tracks Hermes feature releases by date-version + semver. v0.12 added
...
public struct HermesCapabilities: Sendable, Equatable {
    ...
    /// so diagnostics views can show the exact string Glindt saw.
    ...
    //
    // Add a new flag here when Glindt gains UI that conditionally branches on
    ...
    /// `redaction.enabled` is now off by default in v0.12 — Glindt surfaces
    ...
    /// this flag on, Glindt can surface `/steer` even when the agent isn't
    ...
    /// `hermes gateway list` cross-profile status verb (v0.13+). Lets Glindt
    ...
    /// Context compression count surfaced in the status feed (v0.13+). Glindt
    ...
    /// by Glindt's "Update Hermes" affordance to run unattended.
    ...
    /// xAI Custom Voices — voice cloning support (v0.13+). Exposed in Glindt
    ...
    /// the agent loop; Glindt has no UI surface yet, but the flag lets future
    ...
    /// conversation receives it (v0.13+). Plugin-author concern; Glindt's
    ...
    /// ACP `session/set_model` JSON-RPC method (v0.13+). Lets Glindt
    ...
    // not to a different model), so Glindt doesn't surface it in the ACP
    ...
    /// `clear` (v0.14+). Available in ACP and gateway contexts. Glindt
    ...
    /// from inside an active chat (v0.14+). Glindt already exposes session
    ...
    /// `[auto|codex_app_server]`. Forward-compat flag — Glindt surfaces it
    ...
    /// (default on in v0.14+). Glindt surfaces the toggle so users can
    ...
    /// (v0.14+). Glindt reads the manifest field to render a badge in
    /// `PluginsView`.
    ...
    /// Glindt wraps `hermes proxy start` / `status` / `providers` in a
    ...
    /// existing config keys still work. Glindt mirrors the display
    ...
    /// behavior; Glindt surfaces it as a documentation note in Settings →
    ...
    // Glindt adopts: the chat-scoped Kanban surface, the Kanban maturation
    ...
    public static func parseLine(_ line: String) -> HermesCapabilities {
        ...
        // start — older builds prefix with ANSI color codes Glindt would
        // need to strip.
...
public final class HermesCapabilitiesStore {
    #if canImport(os)
    private let logger = Logger(subsystem: "com.glindt", category: "HermesCapabilities")
    #endif
    ...
    nonisolated private static func detectSync(context: ServerContext) -> HermesCapabilities {
        ...
        do {
            ...
            // `hermes --version` writes to stdout but Glindt's transport
