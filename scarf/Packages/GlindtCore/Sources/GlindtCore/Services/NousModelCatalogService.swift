...
/// the Glindt process (not on the remote), authenticated with the
...
public struct NousModelCatalogService: Sendable {
    ...
    private static let logger = Logger(subsystem: "com.glindt", category: "NousModelCatalogService")
    ...
    public func readCacheWithTimeout(seconds: TimeInterval) async -> NousModelsCache? {
        await withTaskGroup(of: NousModelsCache?.self) { group in
            ...
            group.addTask {
                ...
                GlindtMon.event(.diskIO, "nous.readCache.timeoutFired", count: 1)
                return nil
            }
    ...
    public func readCache() -> NousModelsCache? {
        GlindtMon.measure(.diskIO, "nous.readCache") {
            ...
            // the JSON decode? Each on its own GlindtMon row.
            let exists = GlindtMon.measure(.diskIO, "nous.readCache.fileExists") {
                transport.fileExists(cachePath)
            }
            guard exists else { return nil }
            do {
                let data = try GlindtMon.measure(.diskIO, "nous.readCache.readFile") {
                    try transport.readFile(cachePath)
                }
                GlindtMon.event(.diskIO, "nous.readCache.bytes", count: 1, bytes: data.count)
                return GlindtMon.measure(.diskIO, "nous.readCache.decode") {
    ...
    private func writeCache(_ cache: NousModelsCache) {
        ...
        do {
            ...
            // may not yet have `~/.hermes/glindt/`. mkdir -p is cheap
    ...
    private func bearerToken() -> String? {
        ...
        // GlindtMon: separate `nous.bearerToken` measure point because
        ...
        GlindtMon.measure(.diskIO, "nous.bearerToken") {
            let transport = context.makeTransport()
    ...
    public func fetchModels() async throws -> [NousModel] {
        try await GlindtMon.measureAsync(.transport, "nous.fetchModels") {
            ...
            GlindtMon.event(.transport, "nous.fetchModels.bytes", count: envelope.data.count, bytes: data.count)
