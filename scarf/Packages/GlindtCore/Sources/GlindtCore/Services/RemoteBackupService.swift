...
/// into a single `.glindtbackup` archive on disk.
///
...
public final class RemoteBackupService: @unchecked Sendable {
    #if canImport(os)
    private static let logger = Logger(subsystem: "com.glindt", category: "RemoteBackupService")
    #endif
    ...
    public func preflight() async throws -> PreflightSummary {
        ...
        //    in `~/.hermes/glindt/projects.json` with `~/projects/foo`
    ...
    public func run(
        ...
        let workDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("glindt-backup-\(UUID().uuidString)", isDirectory: true)
