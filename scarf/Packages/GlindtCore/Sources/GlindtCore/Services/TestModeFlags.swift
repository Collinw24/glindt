...
/// a re-scan. The harness sets `--glindt-test-mode` from XCUITest's
/// `XCUIApplication.launchArguments` and pairs it with `GLINDT_HERMES_HOME`
/// (read by `HermesProfileResolver`) to drive Glindt against an isolated
/// Hermes home.
///
...
public struct TestModeFlags: Sendable {
    /// True when the process was launched with `--glindt-test-mode`. Read
    ...
    public init(arguments: [String]) {
        self.isTestMode = arguments.contains("--glindt-test-mode")
    }
}
