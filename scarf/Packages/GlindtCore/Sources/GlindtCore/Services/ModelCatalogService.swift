...
public struct ModelCatalogService: Sendable {
    #if canImport(os)
    private let logger = Logger(subsystem: "com.glindt", category: "ModelCatalogService")
    #endif
    ...
    /// Glindt needs to surface these so the picker matches `hermes model` on
    /// the CLI.
    ...
    public nonisolated func loadProvidersAsync() async -> [HermesProviderInfo] {
        await Task.detached { [self] in
            let providers = GlindtMon.measure(.diskIO, "modelCatalog.loadProviders") {
                self.loadProviders()
            }
            GlindtMon.event(.diskIO, "modelCatalog.providers.count", count: providers.count)
            return providers
        }.value
    }
    ...
    public nonisolated func loadModelsAsync(for providerID: String) async -> [HermesModelInfo] {
        await Task.detached { [self] in
            let models = GlindtMon.measure(.diskIO, "modelCatalog.loadModels") {
                self.loadModels(for: providerID)
            }
            GlindtMon.event(.diskIO, "modelCatalog.models.count", count: models.count)
            return models
        }.value
    }
    ...
    /// Mac's ModelPickerSheet and GlindtGo's scoped settings editor
    ...
    public func validateModel(_ modelID: String, for providerID: String) -> ModelValidation {
        GlindtMon.measure(.diskIO, "modelCatalog.validateModel") {
    ...
    public static let modelAliases: [String: String] = [
        ...
        // the id→id alias is all Glindt needs so a stored retired id still
    ...
    /// override, extra env vars) that Glindt doesn't currently display.
    ///
    ...
    public static let overlayOnlyProviders: [String: HermesProviderOverlay] = [
        ...
        // aliases server-side, but Glindt only registers the canonical so
...
/// Glindt-side mirror of `HermesOverlay` from hermes-agent's
