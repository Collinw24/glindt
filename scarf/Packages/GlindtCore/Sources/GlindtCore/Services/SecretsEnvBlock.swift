...
/// Pure block-splice logic for Glindt's managed regions inside
/// `~/.hermes/.env`. Each registered project that has at least one
/// resolved secret carries one block, bounded by:
///
/// ```
/// # glindt-secrets:begin <slug>
/// GLINDT_<UPPER_SLUG>_<UPPER_FIELDKEY>=<value>
/// ...
/// # glindt-secrets:end <slug>
/// ```
///
...
public enum SecretsEnvBlock {
    ...
    public static let beginMarkerPrefix = "# glindt-secrets:begin "
    public static let endMarkerPrefix = "# glindt-secrets:end "

    // MARK: - Key naming

    /// Build the env-var name for a (slug, fieldKey) pair. Uppercases,
    /// replaces every non-alphanumeric character with `_`, prefixes
    /// `GLINDT_`. Stable: rotating a value writes to the same key.
    public static func envKeyName(slug: String, fieldKey: String) -> String {
        "GLINDT_" + sanitize(slug) + "_" + sanitize(fieldKey)
    }
    ...
    /// 1. `existing` already has a `# glindt-secrets:begin <slug>` /
    ///    `# glindt-secrets:end <slug>` pair → replace the inclusive
