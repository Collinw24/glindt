...
public struct ImageEncoder: Sendable {
    ...
    public nonisolated func encode(
        ...
        GlindtMon.event(.render, "imageEncoder.input.bytes", count: 1, bytes: rawBytes.count)
        return try GlindtMon.measure(.render, "imageEncoder.downsample") {
        #if canImport(AppKit)
        ...
        GlindtMon.event(.render, "imageEncoder.bytes", count: 1, bytes: mainData.count)
        ...
        if rawBytes.starts(with: [0xFF, 0xD8]) {
            GlindtMon.event(.render, "imageEncoder.bytes", count: 1, bytes: rawBytes.count)
            return ChatImageAttachment(
                mimeType: "image/jpeg",
