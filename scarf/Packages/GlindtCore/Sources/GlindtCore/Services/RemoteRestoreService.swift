...
/// Reverses a `.glindtbackup` archive into a target server: validates,
...
/// `~/.hermes/glindt/projects.json` reference the source server's home
/// (e.g. `/root/projects/foo`). After extraction the project lives at
/// `<targetProjectsRoot>/foo`, so the restore rewrites `path` for each
/// entry. Same logic for `<project>/.glindt/manifest.json` if it carries
/// self-references.
///
...
public final class RemoteRestoreService: @unchecked Sendable {
    #if canImport(os)
    private static let logger = Logger(subsystem: "com.glindt", category: "RemoteRestoreService")
    #endif
    ...
    public enum RestoreError: Error, LocalizedError {
        ...
        public var errorDescription: String? {
            ...
            case .unsupportedSchema(let v): return "Backup uses schema v\(v), which this version of Glindt doesn't recognize."
            case .wrongKind(let k): return "This file isn't a Glindt server backup (kind: \(k))."
    ...
    public func inspect(archiveURL: URL) async throws -> InspectionResult {
        let workDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("glindt-restore-\(UUID().uuidString)", isDirectory: true)
    ...
    public func run(
        ...
        // Stage 3: re-anchor `~/.hermes/glindt/projects.json` so the
    ...
    /// Rewrite each entry's `path` in `~/.hermes/glindt/projects.json`
    ...
    private func reanchorProjectsRegistry(
        ...
        let registryPath = targetHome + "/.hermes/glindt/projects.json"
