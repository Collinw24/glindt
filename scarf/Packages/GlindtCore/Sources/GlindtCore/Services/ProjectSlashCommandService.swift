...
/// stored at `<project>/.glindt/slash-commands/<name>.md`.
///
...
/// `GlindtGo` reads over SFTP via Citadel — so the same code path works
...
public struct ProjectSlashCommandService: Sendable {
    ...
    /// List every slash command at `<project>/.glindt/slash-commands/`.
    ...
    /// transport rejects the write. Creates `<project>/.glindt/slash-commands/`
    /// on demand.
    ...
    ///
    /// The expansion also prepends a Glindt-managed marker so the agent
    ...
    public nonisolated func expand(
        ...
        return "<!-- glindt-slash:\(command.name) -->\n\(body)"
    }
    ...
    //
    // Global slash commands live at `~/.hermes/glindt/slash-commands/<name>.md`
    ...
    /// `~/.hermes/glindt/slash-commands/`. Returns `[]` when the directory
    ...
    /// `<project>/.glindt/slash-commands` — same path on Mac + iOS.
    public static func slashCommandsDir(for projectPath: String) -> String {
        let trimmed = projectPath.hasSuffix("/")
            ...
            : projectPath
        return trimmed + "/.glindt/slash-commands"
    }
}
