...
/// `~/.hermes/glindt/model_presets.json`. Glindt-owned — Hermes never
/// touches this file.
///
/// **Concurrency.** Pure-I/O `actor`. Mirrors `KanbanService` and the
/// other GlindtCore services — every public method serializes through
...
public actor ModelPresetService {
    #if canImport(os)
    private static let logger = Logger(subsystem: "com.glindt", category: "ModelPresetService")
    #endif
    ...
    private func persist(_ store: ModelPresetStore) async throws {
        ...
        try await Task.detached(priority: .utility) { [updated] in
            let transport = context.makeTransport()
            let path = context.paths.modelPresetsJSON
            let glindtDir = context.paths.glindtDir
            if !transport.fileExists(glindtDir) {
                try transport.createDirectory(glindtDir)
            }
