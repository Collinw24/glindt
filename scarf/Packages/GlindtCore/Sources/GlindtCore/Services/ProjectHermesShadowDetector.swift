...
/// Glindt then reads the global path on every dashboard tick and shows
...
public struct ProjectHermesShadowDetector: Sendable {
    public struct Shadow: Sendable, Hashable, Identifiable {
        ...
        /// history is invisible to Glindt's global Dashboard for this slice.
    ...
    #if canImport(os)
    private static let logger = Logger(subsystem: "com.glindt", category: "ProjectHermesShadowDetector")
    #endif
    ...
    public func detect(in projects: [ProjectEntry]) async -> [Shadow] {
        ...
        for project in projects where !project.archived {
            ...
            // edge case (someone made `~/.hermes/notes` a Glindt project).
    ...
    ///    `.hermes.glindt-bak.<UTC-stamp>/`. Hermes' CLI stops seeing it
    ...
    public static func consolidationCommand(for shadow: Shadow, hermesHome: String) -> String? {
        ...
        parts.append("mv \(shellQuote(shadow.shadowPath)) \(shellQuote(shadow.shadowPath))\".glindt-bak.$(date -u +%Y%m%d-%H%M%S)\"")
