...
public final class RichChatViewModel {
    ...
    public static let loadHistoricalToolResultsKey = "glindt.chat.loadHistoricalToolResults"
    ...
    /// Project-scoped, Glindt-managed commands at
    /// `<project>/.glindt/slash-commands/<name>.md`. Loaded by
    ...
    /// Global Glindt-managed commands at `~/.hermes/glindt/slash-commands/<name>.md`.
    ...
    public static let nonInterruptiveCommands: [HermesSlashCommand] = [
        ...
        // `/subgoal clear` server-side; Glindt mirrors the active list
    ...
    /// global Glindt > quick_commands** (most specific source wins).
    ...
    public var availableCommands: [HermesSlashCommand] {
        ...
        // Global Glindt commands sit BELOW project-scoped in the
        // precedence chain — a project that authors its own `glindt-help`
        // wins over the bundled one. Surface them with the same
        // `.projectScoped` source for now (no UI distinction between
        // project and global yet); add a dedicated `.globalGlindt`
    ...
    /// was changed elsewhere (a prior Glindt run, the TUI) will display
    ...
    public enum SubgoalCommandArgument: Equatable {
        ...
        /// with usage; Glindt shows a neutral hint and doesn't touch
        /// the local mirror.
        case empty
    }
    ...
    /// authors `/glindt-help` should win over the bundled global one),
    /// then falls back to the global store so `/glindt-*` commands work
    ...
    /// Slash commands Glindt handles entirely on the client and that
    ...
    /// commonly take 3–8 s here, which the GlindtMon `firstThoughtByte`
    ...
    public func refreshSessionFromDB() async {
        await GlindtMon.measureAsync(.sessionLoad, "mac.refreshSessionFromDB") {
    ...
    public func handleACPEvent(_ event: ACPEvent) {
        ...
        if !hasUserSentPromptThisSession {
            ...
            case .messageChunk, .thoughtChunk, .toolCallStart,
                ...
                GlindtMon.event(.chatStream, "mac.handleACPEvent.preEngagementDropped", count: 1)
                return
    ...
    /// `<projectPath>/.glindt/slash-commands/` off the main actor and
    ...
    /// Load the global Glindt slash commands from
    /// `~/.hermes/glindt/slash-commands/`. Populated by
    ...
    /// projection, so we keep the parser minimal and GlindtCore-local.
    ...
    private func appendMessageChunk(text: String) {
        // GlindtMon "first byte" — fires once per turn, on the first
        // visible message chunk. Splits "user tap → first byte"
        // (network + Hermes thinking) from "first byte → turn end"
        // (streaming + Glindt rendering) so we can attribute slow-feel
        ...
        if streamingAssistantText.isEmpty && currentTurnStart != nil {
            GlindtMon.event(.chatStream, "firstByte", count: 1, bytes: text.utf8.count)
        }
    ...
    private func appendThoughtChunk(text: String) {
        if streamingThinkingText.isEmpty && currentTurnStart != nil {
            GlindtMon.event(.chatStream, "firstThoughtByte", count: 1, bytes: text.utf8.count)
        }
    ...
    private func finalizeStreamingMessage() {
        GlindtMon.measure(.chatStream, "finalizeStreamingMessage") {
    ...
    private func _finalizeStreamingMessageImpl() {
        ...
        // GlindtMon — surface turns that finalize with NO visible
        ...
        if hasContent {
            ...
            // GlindtMon ring as a 1–2 ms burst right after every
    ...
    public func loadSessionHistory(sessionId: String, acpSessionId: String? = nil) async {
        await GlindtMon.measureAsync(.sessionLoad, "mac.hydrateMessages") {
        ...
        guard self.sessionId == loadingForSession else {
            GlindtMon.event(.sessionLoad, "mac.hydrateMessages.dropped", count: 1)
            return
        }
        ...
        GlindtMon.event(.sessionLoad, "mac.hydrateMessages.rows", count: messages.count)
        buildMessageGroups()
    ...
    private func startToolHydration(loadingForSession: String) {
        ...
        hydrationTask = Task { @MainActor [weak self] in
            ...
            if Task.isCancelled || self.sessionId != sessionForLoad {
                GlindtMon.event(.sessionLoad, "mac.hydrateTools.dropped", count: 1)
                return
            }
            ...
            guard loadResults else {
                GlindtMon.event(.sessionLoad, "mac.hydrateTools.skippedToolResults", count: 1)
                return
            }
            ...
            GlindtMon.event(.sessionLoad, "mac.hydrateTools.complete", count: 1)
        }
    }
    ...
    public func loadToolResultIfMissing(callId: String) async {
        ...
        buildMessageGroups()
        GlindtMon.event(.sessionLoad, "mac.lazyToolResult.fetched", count: 1)
    }
