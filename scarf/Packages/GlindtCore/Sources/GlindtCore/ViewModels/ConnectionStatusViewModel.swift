...
public final class ConnectionStatusViewModel {
    #if canImport(os)
    private let logger = Logger(subsystem: "com.glindt", category: "ConnectionStatus")
    #endif
    ...
    public enum DegradedCause: Equatable {
        ...
        /// for "Glindt's required state file."
        case configMissing
    ...
    private func probeOnce() async {
        ...
        //                    points at a Hermes profile, suggesting Glindt
    ...
    nonisolated static func describe(cause: DegradedCause, hermesHome: String) -> (reason: String, hint: String) {
        ...
        case .configMissing:
            return (
                ...
                "`\(hermesHome)/state.db` is missing — Hermes creates it on first agent run. Start any session on the remote (e.g. `hermes chat`) and Glindt will go green automatically."
            )
        case .configUnreadable:
            return (
        ...
        case .unknown:
            return (
                ...
                "SSH is fine but Glindt can't reach `\(hermesHome)/state.db`. Run diagnostics for a full breakdown."
            )
        }
    }
}
