...
///
/// Lives in GlindtCore (not GlindtIOS) because it's pure file-I/O on
...
public final class IOSMemoryViewModel {
    ...
    public func load() async {
        ...
        let result: Result<String?, Error> = await GlindtMon.measureAsync(.diskIO, "ios.memory.load") {
            await Task.detached {
                do {
        ...
        if case .success(.some(let loaded)) = result {
            GlindtMon.event(.diskIO, "ios.memory.load.bytes", count: 0, bytes: loaded.utf8.count)
        }
