...
/// transport, parses it into a `HermesConfig` with the GlindtCore
...
public final class IOSSettingsViewModel {
    ...
    /// order, formatting); Glindt just picks the value. Reloads the
...
/// so SettingEditorSheet (GlindtGo) can narrow on commandFailed to
