...
/// Unified Skills viewmodel. Promoted from the Mac target into GlindtCore
...
public final class SkillsViewModel {
    private let logger = Logger(subsystem: "com.glindt", category: "SkillsViewModel")
    ...
    /// GlindtCore unit tests use this directly; Mac call sites wrap in
    ...
    public func load(pinnedNames: Set<String>? = nil) async {
        ...
        let cats: [HermesSkillCategory] = await GlindtMon.measureAsync(.diskIO, "skills.load") {
            await Task.detached {
        ...
        GlindtMon.event(.diskIO, "skills.load.count", count: totalSkills)
        categories = cats
    ...
    /// global list only — Glindt doesn't surface platform overrides
