...
@Suite struct RemoteSQLiteBackendTests {
    ...
    private func makeFixtureStateDB(
        ...
        let testDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("glindt-test-\(UUID().uuidString)", isDirectory: true)
    ...
    private func makeDefaultHomeFixtureContext(dbURL: URL) throws -> DefaultHomeFixture {
        ...
        if FileManager.default.fileExists(atPath: stateLink.path) {
            let bak = hermesDir.appendingPathComponent("state.db.glindt-test-bak-\(UUID().uuidString)")
    ...
    @Test func nonZeroExitThrowsSqliteError() async throws {
        ...
        // the file at all.
        let nonExistentParent = "/var/empty/glindt-test-no-such-dir-\(UUID().uuidString)"
        let ctx = ServerContext(
            id: UUID(),
            displayName: "broken",
