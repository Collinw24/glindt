...
@Suite struct M0cServicesTests {
    ...
    @Test func logServiceParsesHermesLogFormat() async throws {
        ...
        let tmp = FileManager.default.temporaryDirectory.appendingPathComponent("glindt-log-\(UUID().uuidString).log")
        let text = """
    ...
    @Test func modelCatalogLoadsSyntheticJSON() throws {
        ...
        let tmp = FileManager.default.temporaryDirectory.appendingPathComponent("glindt-models-\(UUID().uuidString).json")
        let json = """
        {
          "anthropic": {
            "id": "anthropic",
    ...
    @Test func modelCatalogHandlesMissingAndMalformedFiles() {
        ...
        // path exercised.
        let tmp = FileManager.default.temporaryDirectory.appendingPathComponent("glindt-bad-\(UUID().uuidString).json")
    ...
    @Test func providerDisplayNameOverridesRenamesAlibaba() throws {
        ...
        // without changing the wire ID. Glindt mirrors the rename via
        // `providerDisplayNameOverrides` so `loadProviders()` returns
        // "Qwen Cloud" even when the cached models.dev JSON still says
        // "Alibaba".
        let tmp = FileManager.default.temporaryDirectory.appendingPathComponent("glindt-catalog-\(UUID().uuidString).json")
        defer { try? FileManager.default.removeItem(at: tmp) }
        let json = #"""
        {
          "alibaba": {
    ...
    @Test func vercelNoLongerDemotedSortsAlphabetically() throws {
        // v0.15 removed Vercel AI Gateway from Hermes entirely, so Glindt's
    ...
    @Test func projectDashboardServiceRegistryRoundTrip() throws {
        ...
        // `context.paths.projectsRegistry` which is `home/glindt/projects.json`.
        let fakeHome = FileManager.default.temporaryDirectory
            .appendingPathComponent("glindt-home-\(UUID().uuidString)").path
        ...
        let projectDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("glindt-proj-\(UUID().uuidString)").path
        try localTransport.createDirectory(projectDir + "/.glindt")
        ...
        """
        try localTransport.writeFile(projectDir + "/.glindt/dashboard.json", data: Data(dashJSON.utf8))
    ...
    @Test func projectDashboardServiceReturnsEmptyRegistryOnMissingFile() async {
        ...
        // `$HOME/.hermes/glindt/projects.json`. On CI this probably doesn't
