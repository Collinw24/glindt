...
@Suite struct HermesCuratorParserTests {
    ...
    least recently active (top 5):
      Glindt Dashboard Chart Widget Parse Error Fix  activity=  0  use=  0  view=  0  patches=  0  last_activity=never
      Glindt Project Registry Format Fix         activity=  0  use=  0  view=  0  patches=  0  last_activity=never
    ...
    @Test func parseRealFreshOutput() {
        ...
        #expect(firstRow?.name == "Glindt Dashboard Chart Widget Parse Error Fix")
    ...
    @Test func parsedPausedState() {
        ...
        pinned (2): kanban-orchestrator, glindt-template-author
        """
        ...
        #expect(s.pinnedNames == ["kanban-orchestrator", "glindt-template-author"])
    }
    ...
    @Test func skillRowParserHandlesMultiWordNames() {
        // Names with spaces are common (Glindt Dashboard Chart Widget…)
