...
struct SecretsEnvBlockTests {
    ...
    @Test func envKeyNameLeadingTrailingTrim() {
        ...
        // GLINDT__... or trailing _ in the result.
        let key = SecretsEnvBlock.envKeyName(slug: "-foo-", fieldKey: "-bar-")
        #expect(key == "GLINDT_FOO_BAR")
    ...
    @Test func renderBlockEmitsMarkersAroundEntries() {
        let result = SecretsEnvBlock.renderBlock(
            ...
            entries: [("GLINDT_SITE_STATUS_CHECKER_TOKEN", "abc")]
        )
        #expect(result.hasPrefix("# glindt-secrets:begin site-status-checker"))
        #expect(result.hasSuffix("# glindt-secrets:end site-status-checker"))
        #expect(result.contains("GLINDT_SITE_STATUS_CHECKER_TOKEN=abc"))
    }
    ...
    @Test func removeBlockStripsBlockOnly() {
        ...
        let existing = "USER_VAR=x\n\n" + block + "\n\nMORE_USER=y\n"
        let result = SecretsEnvBlock.removeBlock(forSlug: "foo", from: existing)
        #expect(!result.contains("glindt-secrets"))
    ...
    @Test func slugPrefixCollisionIsolated() {
        ...
        #expect(occurrences(of: "# glindt-secrets:begin foo\n", in: result) == 1)
        #expect(occurrences(of: "# glindt-secrets:begin foo-bar\n", in: result) == 1)
    }
