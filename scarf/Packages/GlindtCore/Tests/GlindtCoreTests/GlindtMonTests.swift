...
@Suite(.serialized) struct GlindtMonTests {
    ...
    @Test func ringBufferWrapsCorrectly() {
        let ring = GlindtMonRingBuffer(capacity: 4)
        ring.record(.fixture(name: "a"))
        ring.record(.fixture(name: "b"))
        ring.record(.fixture(name: "c"))
    ...
    @Test func summaryAggregatesByCategoryAndName() {
        let ring = GlindtMonRingBuffer(capacity: 16)
    ...
    @Test func installEmptyBackendsDeactivates() {
        GlindtMon.install([])
        #expect(GlindtMon.isActive == false)
        GlindtMon.install([GlindtMonRingBuffer(capacity: 4)])
        #expect(GlindtMon.isActive == true)
        GlindtMon.install([])
    }
    ...
    @Test func measureFlowsThroughInstalledBackends() throws {
        let ring = GlindtMonRingBuffer(capacity: 8)
        GlindtMon.install([ring])
        defer { GlindtMon.install([]) }

        let result: Int = GlindtMon.measure(.render, "unit") {
            return 42
        }
        #expect(result == 42)
    ...
    @Test func measureAsyncRecordsDurationEvenOnThrow() async {
        ...
        await #expect(throws: Boom.self) {
            try await GlindtMon.measureAsync(.chatStream, "throws") {
                throw Boom()
            }
        }
        let samples = ring.samples()
    ...
    @Test func eventRecordsCountSample() {
        ...
        GlindtMon.event(.chatStream, "token", count: 1, bytes: 32)
        let samples = ring.samples()
        #expect(samples.count == 1)
    ...
    @Test func bootConfigureModesInstallExpectedBackends() {
        ...
        GlindtMonBoot.configure(mode: .off)
        #expect(GlindtMon.currentBackends.isEmpty)
        #expect(GlindtMonBoot.sharedRingBuffer == nil)

        GlindtMonBoot.configure(mode: .signpostOnly)
        #expect(GlindtMon.currentBackends.count == 1)
        #expect(GlindtMonBoot.sharedRingBuffer == nil)

        let ring = GlindtMonBoot.configure(mode: .full)
        #expect(ring != nil)
        #expect(GlindtMon.currentBackends.count == 3)
        #expect(GlindtMonBoot.sharedRingBuffer === ring)
    }
...
private extension GlindtMon.Sample {
    static func fixture(
        category: GlindtMon.Category = .render,
        name: StaticString,
