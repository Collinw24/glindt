...
/// Exercises the `GLINDT_HERMES_HOME` test-mode override on `HermesProfileResolver`.
...
/// var from ever pivoting Glindt off the user's real `~/.hermes`.
@Suite(.serialized)
struct HermesProfileResolverOverrideTests {

    private static let envKey = "GLINDT_HERMES_HOME"
    ...
    @Test func absoluteOverrideTakesPrecedenceWhenMarkerPresent() throws {
        ...
        let tmp = NSTemporaryDirectory().appending("glindt-test-home-\(UUID().uuidString)")
    ...
    @Test func overrideIsIgnoredWhenMarkerMissing() throws {
        ...
        let tmp = NSTemporaryDirectory().appending("glindt-no-marker-\(UUID().uuidString)")
        try FileManager.default.createDirectory(atPath: tmp, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(atPath: tmp) }
        setenv(Self.envKey, tmp, 1)
    ...
    @Test func overrideBypassesCacheWhenMarkerPresent() throws {
        ...
        let first = NSTemporaryDirectory().appending("glindt-cache-bypass-1-\(UUID().uuidString)")
        let second = NSTemporaryDirectory().appending("glindt-cache-bypass-2-\(UUID().uuidString)")
