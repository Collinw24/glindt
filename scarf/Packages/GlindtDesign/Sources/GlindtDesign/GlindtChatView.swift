...
//  in `ChatViewModel`. Everything visual uses tokens from GlindtTheme + GlindtFont
//  + GlindtComponents — no raw hex, no hardcoded fonts.
//
//  How to integrate:
//    NavigationSplitView column / detail / inspector aren't used here because
//    Glindt uses a custom 3-pane layout (matches the kit). Drop ChatRootView
...
public enum ChatToolKind: String, Hashable, CaseIterable {
    ...
    var color: Color {
        switch self {
        case .read:    return GlindtColor.success         // green
        case .edit:    return GlindtColor.info            // blue
        case .execute: return GlindtColor.warning         // amber
        case .fetch:   return GlindtColor.Tool.web        // purple
        case .browser: return GlindtColor.Tool.search     // indigo
        case .search:  return GlindtColor.accent          // brand rust
        }
    }
...
final class ChatViewModel: ObservableObject {
    ...
    init() {
        sessions = [
            .init(id: "s1", title: "Cron diagnostics",      project: "glindt",        preview: "The daily-summary job ran 14 minutes ago…", time: "14m", model: "sonnet-4.5", pinned: true, status: .live),
            ...
            .init(id: "s5", title: "Memory layout question", project: "glindt",       preview: "The shared memory keys live at…",            time: "yest.", model: "sonnet-4.5"),
            .init(id: "s6", title: "Catalog publish flow",   project: "hermes-blog", preview: "Walked through the .glindttemplate bundle…",  time: "yest.", model: "sonnet-4.5"),
            .init(id: "s7", title: "SSH tunnel debug",       project: "glindt-remote", preview: "Connection drops after ~90s of idle…",      time: "Mon", model: "sonnet-4.5", status: .error),
        ]
        activeSessionID = "s1"

        let calls: [ChatToolCall] = [
            .init(id: "tc-1", kind: .read,    name: "read_file",   arg: "~/.glindt/cron/jobs.json",                 duration: "86 ms", startedAt: "09:42:18.214", tokens: 412),
            .init(id: "tc-2", kind: .execute, name: "execute",     arg: "hermes cron status daily-summary",        duration: "1.4 s", startedAt: "09:42:18.302", tokens: 86, exitCode: 0, cwd: "~/.glindt"),
            .init(id: "tc-3", kind: .read,    name: "read_file",   arg: "~/.glindt/cron/output/daily-summary.md",   duration: "42 ms", startedAt: "09:43:01.190", tokens: 1284),
            .init(id: "tc-4", kind: .edit,    name: "apply_patch", arg: "~/.glindt/cron/jobs.json",                 duration: "120 ms", startedAt: "09:43:03.910", tokens: 88, linesAdded: 1, linesRemoved: 1),
        ]
        focusedToolID = "tc-2"
...
public struct ChatRootView: View {
    ...
    public var body: some View {
        HStack(spacing: 0) {
            ...
            Divider().background(GlindtColor.border)
        ...
        .background(GlindtColor.backgroundPrimary)
    }
}
...
struct ChatSessionListPane: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: GlindtSpace.s2) {
                Text("Chats").glindtStyle(.headline)
                Spacer()
            ...
            HStack(spacing: GlindtSpace.s2) {
                ...
                Spacer()
                Text("1.2 MB · state.db").font(GlindtFont.monoSmall)
            }
            .glindtStyle(.caption)
            .foregroundStyle(GlindtColor.foregroundMuted)
            .padding(.horizontal, GlindtSpace.s3)
            .padding(.vertical, GlindtSpace.s2)
            .overlay(Rectangle().fill(GlindtColor.border).frame(height: 1), alignment: .top)
        }
    ...
    private func sectionHeader(_ text: String) -> some View {
        HStack {
            Text(text).glindtStyle(.captionUppercase)
                .foregroundStyle(GlindtColor.foregroundMuted)
            Spacer()
        }
        .padding(.horizontal, 10).padding(.top, GlindtSpace.s2).padding(.bottom, GlindtSpace.s1)
        .background(GlindtColor.backgroundTertiary)
    }
}
...
struct ChatSessionRow: View {
            ...
            HStack(spacing: 7) {
                ...
                if session.pinned {
                    Image(systemName: "pin.fill")
                        .font(.system(size: 9))
                        .foregroundStyle(GlindtColor.accent)
                }
                Text(session.title)
                    .glindtStyle(.bodyEmph)
                    .lineLimit(1)
                    .foregroundStyle(isActive ? GlindtColor.accentActive : GlindtColor.foregroundPrimary)
                Spacer(minLength: 0)
                Text(session.time)
                    .font(GlindtFont.caption2)
            ...
            HStack(spacing: 6) {
                if session.project != "—" {
                    Text(session.project)
                        ...
                        .background(
                            RoundedRectangle(cornerRadius: GlindtRadius.sm).fill(GlindtColor.backgroundSecondary)
                                .overlay(
                                    RoundedRectangle(cornerRadius: GlindtRadius.sm)
                ...
                if session.unread > 0 {
                    Text("\(session.unread)")
                        ...
                        .padding(.horizontal, 5).padding(.vertical, 1)
                        .background(Capsule().fill(GlindtColor.accent))
                }
            }
            .padding(.leading, 14)
        }
        .padding(.horizontal, 10).padding(.vertical, GlindtSpace.s2)
        .background(
            RoundedRectangle(cornerRadius: 7)
                .fill(isActive ? GlindtColor.accentTint :
                      (hover ? GlindtColor.border : Color.clear))
        )
    ...
    private var statusDot: some View {
        ...
        case .live:
            Circle().fill(GlindtColor.success).frame(width: 7, height: 7)
                .overlay(Circle().stroke(GlindtColor.success.opacity(0.20), lineWidth: 2))
        case .error:
            Circle().fill(GlindtColor.danger).frame(width: 6, height: 6)
        case .idle:
            Circle().fill(GlindtColor.foregroundFaint.opacity(0.4)).frame(width: 6, height: 6)
        }
    }
}
...
struct ChatTranscriptPane: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            transcriptHeader
            ScrollView {
                LazyVStack(alignment: .leading, spacing: GlindtSpace.s4) {
                    ForEach(vm.blocks, id: \.id) { block in
                        renderBlock(block)
                    }
                    suggestedReplies
                }
                .padding(.horizontal, 28).padding(.top, GlindtSpace.s5).padding(.bottom, GlindtSpace.s2)
            }
            ...
            VStack(alignment: .leading, spacing: 3) {
                HStack(spacing: GlindtSpace.s2) {
                    Image(systemName: "pin.fill")
                        .font(.system(size: 11))
                        .foregroundStyle(GlindtColor.accent)
                    Text("Cron diagnostics").glindtStyle(.bodyEmph)
                    GlindtBadge("live", kind: .success)
                }
                HStack(spacing: 8) {
                    Image(systemName: "folder.fill").font(.system(size: 10))
                        .foregroundStyle(GlindtColor.accent)
                    Text("glindt").glindtStyle(.caption).foregroundStyle(GlindtColor.accent).fontWeight(.semibold)
                    metaSeparator
                    Text("claude-sonnet-4.5").font(GlindtFont.monoSmall)
                    metaSeparator
                    Text("14 messages").glindtStyle(.caption)
                    metaSeparator
                    Text("12,847 tok").font(GlindtFont.monoSmall)
                    metaSeparator
                    Text("$0.0421").font(GlindtFont.monoSmall)
                }
                .foregroundStyle(GlindtColor.foregroundMuted)
            }
            Spacer()
            Button("Branch") { }.buttonStyle(GlindtGhostButton())
            Button("Share")  { }.buttonStyle(GlindtSecondaryButton())
        }
        .padding(.horizontal, GlindtSpace.s6).padding(.vertical, GlindtSpace.s3)
        .background(GlindtColor.backgroundSecondary)
        .overlay(Rectangle().fill(GlindtColor.border).frame(height: 1), alignment: .bottom)
    }

    private var metaSeparator: some View {
        Text("·").foregroundStyle(GlindtColor.foregroundFaint)
    }
    ...
    private func renderBlock(_ block: ChatBlock) -> some View {
        ...
        case .dateMarker(_, let label):
            HStack(spacing: 10) {
                Rectangle().fill(GlindtColor.border).frame(height: 1)
                Text(label).glindtStyle(.captionUppercase)
                    .foregroundStyle(GlindtColor.foregroundFaint)
                Rectangle().fill(GlindtColor.border).frame(height: 1)
            }
            ...
            ForEach(["Schedule a dry run", "Show last 5 runs", "Disable daily-summary"], id: \.self) { s in
                HStack(spacing: 4) {
                    Image(systemName: "sparkles").font(.system(size: 10))
                        .foregroundStyle(GlindtColor.accent)
                    Text(s).glindtStyle(.caption)
                }
                .padding(.horizontal, 10).padding(.vertical, 5)
                .background(
                    Capsule().fill(GlindtColor.backgroundSecondary)
                        .overlay(Capsule().strokeBorder(GlindtColor.borderStrong, lineWidth: 1))
                )
            }
        }
            ...
            VStack(alignment: .trailing, spacing: 4) {
                ...
                HStack(spacing: 6) {
                    Image(systemName: "checkmark.circle.fill")
                        .font(.system(size: 10))
                        .foregroundStyle(GlindtColor.success)
                    Text(time).font(GlindtFont.caption2)
                        .foregroundStyle(GlindtColor.foregroundFaint)
                }
            }
        }
...
struct AssistantBubble: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                GlindtCard(padding: GlindtSpace.s3) {
                    Text(markdown).glindtStyle(.body)
                }
                HStack(spacing: 8) {
                    if inProgress {
                        HStack(spacing: 4) {
                            Circle().fill(GlindtColor.accent).frame(width: 7, height: 7)
                                .modifier(PulseModifier())
                            Text("thinking…").foregroundStyle(GlindtColor.accent).fontWeight(.semibold)
                        }
                    }
                    Text(model).font(GlindtFont.monoSmall)
                    Text("·")
                    Text("\(tokens) tok").font(GlindtFont.monoSmall)
                    Text("·")
...
struct ReasoningBlock: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            Button { withAnimation(GlindtAnimation.smooth) { open.toggle() } } label: {
                HStack(spacing: 5) {
            ...
            if open {
                Text(full).glindtStyle(.footnote)
                    .italic()
                    .foregroundStyle(GlindtColor.foregroundMuted)
            } else {
                Text(preview).glindtStyle(.footnote)
                    .italic()
                    .foregroundStyle(GlindtColor.foregroundMuted)
                    .lineLimit(1)
            }
        ...
        .background(
            RoundedRectangle(cornerRadius: 7).fill(GlindtColor.warning.opacity(0.10))
                .overlay(RoundedRectangle(cornerRadius: 7).strokeBorder(GlindtColor.warning.opacity(0.30), lineWidth: 1))
        )
    }
}
...
struct ToolCallCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            Button {
                onTap()
                withAnimation(GlindtAnimation.fast) { open.toggle() }
            } label: {
                HStack(spacing: 9) {
                    HStack(spacing: 5) {
                    ...
                    Text(call.name).font(GlindtFont.monoSmall).fontWeight(.semibold)
                    Text(call.arg).font(GlindtFont.monoSmall)
                        .foregroundStyle(GlindtColor.foregroundMuted)
                        .lineLimit(1).truncationMode(.middle)
                    Spacer(minLength: 8)
                    Text(call.duration).font(GlindtFont.caption2)
                        .foregroundStyle(GlindtColor.foregroundFaint)
                    Image(systemName: "checkmark.circle.fill")
                        .font(.system(size: 11)).foregroundStyle(GlindtColor.success)
                    Image(systemName: open ? "chevron.down" : "chevron.right")
                        .font(.system(size: 9)).foregroundStyle(GlindtColor.foregroundFaint)
                }
                .padding(.horizontal, 10).padding(.vertical, 6)
                .background(
                    RoundedRectangle(cornerRadius: 7)
                        .fill(isFocused ? call.kind.tint : GlindtColor.border.opacity(0.5))
                        .overlay(
                            RoundedRectangle(cornerRadius: 7)
                                .strokeBorder(isFocused ? call.kind.color : GlindtColor.border, lineWidth: isFocused ? 1.4 : 1)
                        )
                )
            }
            .buttonStyle(.plain)
...
private struct TerminalOutput: View {
    ...
    private func line(_ s: String, muted: Bool = false) -> some View {
        Text(s).font(GlindtFont.monoSmall)
...
private struct JSONPreview: View {
    var body: some View {
        ...
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(
            RoundedRectangle(cornerRadius: 7).fill(GlindtColor.backgroundSecondary)
                .overlay(RoundedRectangle(cornerRadius: 7).strokeBorder(GlindtColor.border, lineWidth: 1))
        )
    }
    private func row(_ n: Int, _ s: String) -> some View {
        HStack(spacing: 0) {
            Text(String(format: "%2d", n))
                .foregroundStyle(GlindtColor.foregroundFaint)
            Text("  ").font(GlindtFont.monoSmall)
            Text(s).foregroundStyle(GlindtColor.foregroundMuted)
        }
        .font(GlindtFont.monoSmall)
    }
}
...
private struct DiffPreview: View {
    var body: some View {
        VStack(alignment: .leading, spacing: 1) {
            row("3", "  \"schedule\": \"0 9 * * *\",", bg: nil, color: GlindtColor.foregroundPrimary)
            row("-", "  \"timezone\": \"UTC\",", bg: GlindtColor.danger.opacity(0.10), color: GlindtColor.danger)
            row("+", "  \"timezone\": \"America/New_York\",", bg: GlindtColor.success.opacity(0.10), color: GlindtColor.success)
            row("5", "  \"enabled\": true", bg: nil, color: GlindtColor.foregroundPrimary)
        }
        .padding(8)
        .background(
            RoundedRectangle(cornerRadius: 7).fill(GlindtColor.backgroundSecondary)
    ...
    private func row(_ marker: String, _ text: String, bg: Color?, color: Color) -> some View {
        HStack(spacing: 0) {
            ...
            Text(text).foregroundStyle(GlindtColor.foregroundPrimary)
            Spacer(minLength: 0)
        }
        .font(GlindtFont.monoSmall)
        .padding(.vertical, 1)
...
struct ChatComposer: View {
    ...
    var body: some View {
        VStack(spacing: GlindtSpace.s2) {
            // Context chips row
            HStack(spacing: 6) {
                contextChip("folder", "glindt", brand: true)
            ...
            HStack(spacing: 6) {
                ...
                Button {} label: {
                    Image(systemName: "arrow.up")
                        ...
                        .background(
                            RoundedRectangle(cornerRadius: 8).fill(GlindtColor.accent)
                        )
                }
                .buttonStyle(.plain)
            }
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(slashOpen ? GlindtColor.accent : GlindtColor.borderStrong,
                                      lineWidth: 1)
                )
        )
        .glindtShadow(slashOpen ? .md : .sm)
        .padding(.horizontal, GlindtSpace.s6)
        .padding(.vertical, GlindtSpace.s3)
        .background(GlindtColor.backgroundSecondary)
        .overlay(Rectangle().fill(GlindtColor.border).frame(height: 1), alignment: .top)
        .overlay(alignment: .bottomLeading) {
            if slashOpen { SlashMenu().padding(.horizontal, GlindtSpace.s6).offset(y: -56) }
        }
    }
    ...
    private func contextChip(_ icon: String, _ label: String, brand: Bool = false, muted: Bool = false) -> some View {
        HStack(spacing: 5) {
            ...
            Text(label).font(brand ? GlindtFont.caption : GlindtFont.monoSmall)
        }
        .padding(.horizontal, 8).padding(.vertical, 2)
        .foregroundStyle(brand ? GlindtColor.accentActive
                         : muted ? GlindtColor.foregroundMuted : GlindtColor.foregroundPrimary)
        .background(
            Capsule().fill(brand ? GlindtColor.accentTint : GlindtColor.border.opacity(0.5))
                .overlay(
                    Capsule().strokeBorder(
                        muted ? GlindtColor.borderStrong : Color.clear,
    ...
    private func composerChip(_ icon: String, _ label: String) -> some View {
        HStack(spacing: 4) {
            ...
            Text(label).font(GlindtFont.monoSmall)
        }
        .foregroundStyle(GlindtColor.foregroundMuted)
        .padding(.horizontal, 7).padding(.vertical, 3)
        .background(RoundedRectangle(cornerRadius: 6).fill(GlindtColor.border.opacity(0.4)))
    }
}
...
private struct SlashMenu: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            Text("SLASH COMMANDS").glindtStyle(.captionUppercase)
                .foregroundStyle(GlindtColor.foregroundMuted)
                .padding(.horizontal, 8).padding(.top, 6).padding(.bottom, 4)
            ForEach(Array(items.enumerated()), id: \.offset) { idx, item in
                HStack(spacing: 9) {
                    Image(systemName: item.icon).font(.system(size: 13))
                    Text("/\(item.cmd)").font(GlindtFont.mono).fontWeight(.semibold)
                    Text(item.desc).glindtStyle(.footnote)
                        .foregroundStyle(GlindtColor.foregroundMuted)
                    Spacer()
                    if idx == 0 {
                        Text("↵").font(GlindtFont.monoSmall)
                            .padding(.horizontal, 4).padding(.vertical, 1)
                            .background(RoundedRectangle(cornerRadius: 3).fill(GlindtColor.border))
                    }
                }
                .padding(.horizontal, 8).padding(.vertical, 6)
                .background(idx == 0 ? GlindtColor.accentTint : Color.clear)
                .foregroundStyle(idx == 0 ? GlindtColor.accentActive : GlindtColor.foregroundPrimary)
        ...
        .background(
            RoundedRectangle(cornerRadius: 9, style: .continuous)
                .fill(GlindtColor.backgroundSecondary)
                .overlay(RoundedRectangle(cornerRadius: 9).strokeBorder(GlindtColor.border, lineWidth: 1))
        )
        .glindtShadow(.lg)
    }
}
...
struct ChatInspectorPane: View {
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
                ...
                HStack(spacing: GlindtSpace.s2) {
                    if let call {
                        ...
                        VStack(alignment: .leading, spacing: 1) {
                            Text("\(call.kind.label) CALL").tracking(0.5)
                                .glindtStyle(.captionStrong)
                                .foregroundStyle(call.kind.color)
                            Text(call.name).font(GlindtFont.mono).fontWeight(.semibold)
                                .lineLimit(1)
                        }
                    }
                    Spacer()
                    Button { } label: {
                        Image(systemName: "xmark").font(.system(size: 11))
                            .foregroundStyle(GlindtColor.foregroundMuted).padding(4)
                    }.buttonStyle(.plain)
                }
            ...
            .padding(.horizontal, GlindtSpace.s4).padding(.vertical, GlindtSpace.s3)
            .overlay(Rectangle().fill(GlindtColor.border).frame(height: 1), alignment: .bottom)
            ...
            // Footer
            HStack(spacing: 6) {
                Button("Re-run") {}.buttonStyle(GlindtSecondaryButton()).frame(maxWidth: .infinity)
                Button("Copy")   {}.buttonStyle(GlindtGhostButton())
            }
            .padding(.horizontal, GlindtSpace.s4).padding(.vertical, GlindtSpace.s2)
            .overlay(Rectangle().fill(GlindtColor.border).frame(height: 1), alignment: .top)
        }
        .background(GlindtColor.backgroundSecondary)
    }
...
private struct InspectorDetails: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: GlindtSpace.s5) {
            // Status banner
            section("STATUS") {
                HStack(spacing: GlindtSpace.s2) {
                    Image(systemName: "checkmark.circle.fill")
                        .foregroundStyle(GlindtColor.success).font(.system(size: 14))
                    VStack(alignment: .leading, spacing: 1) {
                        Text("Completed").glindtStyle(.captionStrong)
                            .foregroundStyle(GlindtColor.success)
                        Text("Exit 0 · No errors").glindtStyle(.caption)
                            .foregroundStyle(GlindtColor.foregroundMuted)
                    }
                    Spacer()
                }
                .padding(GlindtSpace.s2)
                .background(
                    RoundedRectangle(cornerRadius: 7).fill(GlindtColor.success.opacity(0.12))
                        .overlay(RoundedRectangle(cornerRadius: 7).strokeBorder(GlindtColor.success.opacity(0.25), lineWidth: 1))
                )
            }
            section("ARGUMENTS") {
                Text(call.arg).font(GlindtFont.monoSmall)
                    .padding(GlindtSpace.s2)
                    .frame(maxWidth: .infinity, alignment: .leading)
                    .background(RoundedRectangle(cornerRadius: 7).fill(GlindtColor.border.opacity(0.4)))
            }
            section("TELEMETRY") {
                VStack(spacing: 0) {
                    ...
                    if let exit = call.exitCode {
                        kv("Exit code", "\(exit)", mono: true, color: GlindtColor.success)
                    }
                ...
                VStack(alignment: .leading, spacing: 6) {
                    HStack(spacing: 6) {
                        Image(systemName: "checkmark.shield.fill").font(.system(size: 11))
                            .foregroundStyle(GlindtColor.success)
                        (Text("Allowed by ")
                         + Text("glindt-default").font(GlindtFont.monoSmall)
                         + Text(" profile"))
                            .glindtStyle(.caption)
                    }
                    HStack(spacing: 6) {
                        Image(systemName: "checkmark").font(.system(size: 11))
                            .foregroundStyle(GlindtColor.success)
                        Text("No human approval required").glindtStyle(.caption)
                    }
                }
                .padding(GlindtSpace.s2)
                .frame(maxWidth: .infinity, alignment: .leading)
    ...
    private func section<Content: View>(_ title: String,
        ...
        VStack(alignment: .leading, spacing: GlindtSpace.s2) {
            HStack {
                Text(title).glindtStyle(.captionUppercase)
                    .foregroundStyle(GlindtColor.foregroundMuted)
                Spacer()
            }
            content()
            if let hint {
                Text(hint).glindtStyle(.caption)
                    .foregroundStyle(GlindtColor.foregroundFaint)
            }
        }
    }
    ...
    private func kv(_ k: String, _ v: String, mono: Bool, color: Color? = nil) -> some View {
        HStack {
            Text(k).glindtStyle(.caption)
                .foregroundStyle(GlindtColor.foregroundMuted)
            Spacer()
            Text(v)
                .font(mono ? GlindtFont.monoSmall : GlindtFont.caption)
                .foregroundStyle(color ?? GlindtColor.foregroundPrimary)
        }
        .padding(.vertical, 5)
        .overlay(Rectangle().fill(GlindtColor.border).frame(height: 1), alignment: .bottom)
    }
...
private struct InspectorOutput: View {
    var body: some View {
        VStack(alignment: .leading, spacing: GlindtSpace.s4) {
            Text("STDOUT").glindtStyle(.captionUppercase)
                .foregroundStyle(GlindtColor.foregroundMuted)
            VStack(alignment: .leading, spacing: 4) {
                Group {
                    Text("$ ").foregroundStyle(Color(red: 0.48, green: 0.45, blue: 0.40))
