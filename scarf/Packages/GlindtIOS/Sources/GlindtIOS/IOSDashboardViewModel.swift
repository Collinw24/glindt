...
// either moved to GlindtCore or replicated in an iOS-compatible form.
...
/// (via the transport factory wired in `GlindtIOSApp.init`) routes all
...
public final class IOSDashboardViewModel {
    ...
    /// a registered Glindt project. Populated in `load()` by a single
    ...
    public func load() async {
        ...
        await GlindtMon.measureAsync(.sessionLoad, "ios.loadDashboard") {
        ...
        GlindtMon.event(.sessionLoad, "ios.allSessions.count", count: allSessions.count)
    ...
    public func refresh() async {
        GlindtMon.event(.sessionLoad, "ios.dashboardRefresh.trigger", count: 1)
        await load()
    }
}
