...
/// UNUserNotificationCenter delegate for GlindtGo. M9 #4.4 skeleton —
...
///   the GlindtGoCoordinator. "Approve" / "Deny" categories send the
...
final class NotificationRouter: NSObject, UNUserNotificationCenterDelegate {
    ...
    ///
    /// - The `GLINDT_PENDING_PERMISSION` category (Approve / Deny actions)
    ...
    private let logger = Logger(subsystem: "com.glindt", category: "NotificationRouter")

    /// Coordinator reference set by GlindtGoTabRoot on appear so the
    /// router can route notification-taps to the correct tab. Weak to
    /// avoid a retain cycle with the view's state.
    weak var coordinator: GlindtGoCoordinator?
    ...
    func userNotificationCenter(
        ...
        guard let glindtPayload = info["glindt"] as? [String: Any] else {
            logger.warning("notification missing glindt payload; ignoring")
            return
        }
        let sessionID = glindtPayload["sessionId"] as? String
    ...
    func registerCategories() {
        ...
        let pendingPermission = UNNotificationCategory(
            identifier: "GLINDT_PENDING_PERMISSION",
