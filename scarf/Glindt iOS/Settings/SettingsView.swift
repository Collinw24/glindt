...
struct SettingsView: View {
    ...
    /// v2.7 — Glindt-local opt-in to bulk-fetch tool result CONTENT
    ...
    /// v2.7 — Glindt-local chat preferences. Mirrors the Mac Settings
    ...
    @ViewBuilder
    private var chatGlindtSection: some View {
        Section {
    ...
    /// — it controls the in-process GlindtMon backend set instead. Off
    ...
    private var diagnosticsSection: some View {
        Section {
            NavigationLink {
                GlindtMonDiagnosticsView()
            } label: {
                Label("Performance", systemImage: "speedometer")
            }
            // Show the share affordance only when MetricKit has actually
            // persisted a payload to Documents/GlindtDiagnostics/. Apple
