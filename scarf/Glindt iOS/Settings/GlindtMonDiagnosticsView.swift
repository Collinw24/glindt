...
/// GlindtMon backend mode, watch live aggregated stats from the ring
...
/// panel; both sides read `GlindtMonBoot.sharedRingBuffer`.
struct GlindtMonDiagnosticsView: View {
    @State private var mode: GlindtMonBoot.Mode = GlindtMonBoot.currentMode()
    @State private var stats: [GlindtMonStat] = []
    ...
    private var modeSection: some View {
        Section {
            Picker("Mode", selection: $mode) {
                Text("Off").tag(GlindtMonBoot.Mode.off)
                Text("Signpost only").tag(GlindtMonBoot.Mode.signpostOnly)
                Text("Full").tag(GlindtMonBoot.Mode.full)
            }
            .pickerStyle(.segmented)
            .onChange(of: mode) { _, newValue in
                GlindtMonBoot.setMode(newValue)
            }
        } header: {
            Text("Recording mode")
        } footer: {
    ...
    private var summarySection: some View {
        Section {
            ...
            Text("Sorted by 95th-percentile duration. Counts include events; intervals are everything wrapped in `GlindtMon.measure`.")
                .font(.caption)
        }
    }
    ...
    private var actionsSection: some View {
        Section {
            ...
            Button(role: .destructive) {
                GlindtMonBoot.sharedRingBuffer?.reset()
                refresh()
            } label: {
    ...
    private func refresh() {
        stats = GlindtMonBoot.sharedRingBuffer?.summary() ?? []
    }

    private func copyJSON() {
        guard let json = GlindtMonBoot.sharedRingBuffer?.exportJSON() else { return }
