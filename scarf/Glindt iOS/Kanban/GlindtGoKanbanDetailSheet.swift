...
/// drag-drop).
struct GlindtGoKanbanDetailSheet: View {
    let taskId: String
    ...
    private func headerCard(_ task: HermesKanbanTask) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            FlowLayout(spacing: 6) {
                GlindtBadge(task.status.lowercased(), kind: badgeKind(for: task.status))
                if let assignee = task.assignee, !assignee.isEmpty {
                    GlindtBadge(assignee, kind: .neutral)
                }
                if let workspace = task.workspaceKind {
                    GlindtBadge(workspace, kind: .neutral)
                }
                if let tenant = task.tenant, !tenant.isEmpty {
                    GlindtBadge(tenant, kind: .brand)
                }
    ...
    private func hallucinationBadge(_ task: HermesKanbanTask) -> some View {
        if diagnosticsAvailable,
           KanbanHallucinationGate.from(task.hallucinationGateStatus) == .pending {
            ...
            .padding(.vertical, 6)
            .background(
                GlindtColor.warning.opacity(0.10),
                in: RoundedRectangle(cornerRadius: GlindtRadius.md, style: .continuous)
            )
            .overlay(
    ...
    private func autoBlockedBanner(_ task: HermesKanbanTask) -> some View {
        if diagnosticsAvailable,
           ...
           let reason = task.autoBlockedReason, !reason.isEmpty {
            ...
            .background(
                GlindtColor.danger.opacity(0.08),
                in: RoundedRectangle(cornerRadius: GlindtRadius.md, style: .continuous)
            )
        }
    }
            ...
            FlowLayout(spacing: 6) {
                ForEach(diags) { diag in
                    Button {
                        ...
                        GlindtBadge(diag.kind, kind: diagnosticBadgeKind(diag))
                    }
                    .buttonStyle(.plain)
    ...
    /// `GlindtBadgeKind` palette. Mirrors the Mac inspector's
    /// `diagnosticBadge` helper so the two surfaces tint identically.
    private func diagnosticBadgeKind(_ diag: HermesKanbanDiagnostic) -> GlindtBadgeKind {
        ...
        VStack(alignment: .leading, spacing: 8) {
            if comments.isEmpty {
                ...
                ForEach(comments) { comment in
                    ...
                    .padding(8)
                    .background(GlindtColor.backgroundSecondary.opacity(0.5))
                    .clipShape(RoundedRectangle(cornerRadius: GlindtRadius.md, style: .continuous))
                }
            }
        }
    }
        ...
        VStack(alignment: .leading, spacing: 8) {
            if runs.isEmpty {
                ...
                ForEach(runs) { run in
                    VStack(alignment: .leading, spacing: 2) {
                        HStack {
                            GlindtBadge(run.outcome ?? run.status, kind: outcomeKind(run.outcome ?? run.status))
                    ...
                    .padding(8)
                    .background(GlindtColor.backgroundSecondary.opacity(0.4))
                    .clipShape(RoundedRectangle(cornerRadius: GlindtRadius.md, style: .continuous))
                }
            }
        }
    ...
    private func badgeKind(for status: String) -> GlindtBadgeKind {
    ...
    private func outcomeKind(_ outcome: String) -> GlindtBadgeKind {
