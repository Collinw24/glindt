...
// so GlindtCore-agnostic static analysis doesn't choke.
#if canImport(SQLite3)

/// M4 iOS Chat: streams JSON-RPC over a Citadel SSH exec channel to a
/// remote `hermes acp` process. Reuses GlindtCore's `RichChatViewModel`
...
struct ChatView: View {
    ...
    @Environment(\.glindtGoCoordinator) private var coordinator
    ...
    var body: some View {
        // GlindtMon body-evaluation counter. Re-render churn during
    ...
    /// in glindt/glindt/Features/Chat/Views/ChatView.swift:errorBanner;
    ...
    /// current session is scoped to a Glindt project. Sits full-width
...
final class ChatController {
    ...
    /// Display name of the Glindt project this session is scoped to,
    ...
    // glindt/Features/Chat/ViewModels/ChatViewModel.swift:563-693.
    ...
    /// aware) and parses with the GlindtCore YAML parser — same path
    ...
    /// GlindtGo context that's a blocking SSH round-trip that, before
    ...
    /// (glindt/Features/Chat/ViewModels/ChatViewModel.swift:563).
    ...
    func resetAndStartNewSession() async {
        ...
        // global Glindt commands too so `/glindt-*` still surfaces.
    ...
    /// with the project's path as cwd. Writes the Glindt-managed
    ...
    func resetAndStartInProject(_ project: ProjectEntry) async {
        ...
        // <project.path>/.glindt/slash-commands/ into the chat menu.
        // Async + non-fatal — degrades cleanly on SFTP failures (logged).
        vm.loadProjectScopedCommands(at: project.path)
        // Refresh global Glindt commands so any version bump applied
    ...
    func startResuming(sessionID: String) async {
        await GlindtMon.measureAsync(.sessionLoad, "ios.startResuming") {
    ...
    private func _startResumingImpl(sessionID: String) async {
        ...
        // slash commands at the project's `.glindt/slash-commands/` dir.
        ...
        do {
            ...
            // (glindt/glindt/Features/Chat/ViewModels/ChatViewModel.swift:376).
    ...
    private var bubbleContent: some View {
        ...
        if message.isUser {
            ...
            HStack(alignment: .top, spacing: 8) {
                ...
                // matches the Mac side and the GlindtChatView reference.
...
private struct ToolCallCard: View {
    ...
    private var toolColor: Color {
        switch call.toolKind {
        case .read:    return GlindtColor.success
        case .edit:    return GlindtColor.info
        case .execute: return GlindtColor.warning
        case .fetch:   return GlindtColor.Tool.web
        case .browser: return GlindtColor.Tool.search
        case .other:   return GlindtColor.foregroundMuted
        }
    }
}
...
private struct IOSModelPreflightSheet: View {
    ...
    private var reasonLine: String {
        let suffix = "Glindt will save these to `config.yaml` on \(serverDisplayName) and start the chat."
