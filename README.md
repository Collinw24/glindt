...
- **Vercel removed** — Vercel AI Gateway (provider) and Vercel Sandbox (terminal backend) were deleted upstream in v0.15 and are dropped from Glindt's pickers.
...
See the full [v2.10.0 release notes](https://github.com/awizemann/glindt/releases/tag/v2.10.0) for the complete list, including the pre-release review fixes and the v2.9 highlights (Hermes v0.14 catch-up: `/subgoal` + `/yolo` + `/sessions` + `/codex-runtime` slash commands, xAI Grok OAuth + NovitaAI providers, LINE + SimpleX Chat platforms, Brave Search + DuckDuckGo backends, the Hermes Proxy local server) which are all still in play.

**Previous releases:** see the [Release Notes Index](https://github.com/awizemann/glindt/wiki/Release-Notes-Index) on the wiki for v2.7, v2.6, v2.5, v2.3, v2.2, v2.0, v1.6, and earlier.

## GlindtGo — the iPhone companion

Same Hermes server you've been running on your Mac — reachable from your phone over SSH. Multi-server, project-scoped chat, session resume, memory editor, cron list, skills tree, settings (read), all native iOS. Pure-Swift SSH (Citadel under the hood — no `ssh` binary needed on iOS). Per-project chat writes the same Glindt-managed `AGENTS.md` block the Mac app does, so the agent boots with the same project context regardless of which client opened the session.
...
<p align="center">
  <a href="assets/screenshots/glindtgo-servers.png"><img src="assets/screenshots/glindtgo-servers.png" alt="GlindtGo — Servers list" width="140"></a>
  <a href="assets/screenshots/glindtgo-chat.png"><img src="assets/screenshots/glindtgo-chat.png" alt="GlindtGo — Chat with Hermes" width="140"></a>
  <a href="assets/screenshots/glindtgo-project-dashboard.png"><img src="assets/screenshots/glindtgo-project-dashboard.png" alt="GlindtGo — Project dashboard" width="140"></a>
  <a href="assets/screenshots/glindtgo-skills.png"><img src="assets/screenshots/glindtgo-skills.png" alt="GlindtGo — Skills browser" width="140"></a>
  <a href="assets/screenshots/glindtgo-system.png"><img src="assets/screenshots/glindtgo-system.png" alt="GlindtGo — System tab" width="140"></a>
</p>
...
See the [GlindtGo wiki page](https://github.com/awizemann/glindt/wiki/GlindtGo) for the full feature tour, [GlindtGo Onboarding](https://github.com/awizemann/glindt/wiki/GlindtGo-Onboarding) for the SSH-key setup walkthrough, and [Platform Differences](https://github.com/awizemann/glindt/wiki/Platform-Differences) for what is and isn't shared between Mac and iOS.

## Connect GlindtGo to your Hermes server

GlindtGo speaks SSH directly — no companion service, no developer-controlled server in between. Onboarding takes about a minute:

1. **Install via TestFlight.** Open the [public TestFlight link](https://testflight.apple.com/join/qCrRpcTz) on your phone, accept the invite, install GlindtGo from TestFlight (just like any other beta).
...
3. **Generate Key.** GlindtGo creates a fresh Ed25519 keypair on the device. The private half lives in the iOS Keychain (`kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly`) and is excluded from iCloud sync — it never leaves the phone.
4. **Add the public key to your Hermes host.** Tap **Copy public key**, then on the host run:
   ```bash
   cat >> ~/.ssh/authorized_keys <<'EOF'
   <paste the line GlindtGo showed you>
   EOF
   chmod 600 ~/.ssh/authorized_keys
   ```
   This is its own line per device — the convention any second SSH client uses. Mac (Glindt) keeps using your existing ssh-agent / `~/.ssh/config` and is unaffected.
5. **Tap Test connection.** GlindtGo opens an SSH session, probes for the `hermes` binary, and saves the server on success. If it can't find `hermes`, see the [troubleshooting section](https://github.com/awizemann/glindt/wiki/GlindtGo-Onboarding#troubleshooting) — it's almost always a `PATH` quirk on non-interactive SSH.
...
Glindt 2.0 is a multi-window app. Each window is bound to exactly one Hermes server — your local `~/.hermes/` is synthesized automatically, and you can add remotes via **File → Open Server…** → **Add Server** (host, user, port, optional identity file). Open a second window for a different server and the two run side-by-side with independent state.

Remote Hermes is reached over system SSH — the same `~/.ssh/config`, ssh-agent, ProxyJump, and ControlMaster pooling your terminal uses. File I/O flows through `scp`/`sftp`; SQLite is served from atomic `sqlite3 .backup` snapshots cached under `~/Library/Caches/glindt/snapshots/<server-id>/`; chat (ACP) tunnels as `ssh -T host -- hermes acp` with JSON-RPC over stdio end-to-end. Everything in the feature list below works against remote identically to local.

### Remote setup requirements

The remote host must have:

1. **SSH access** — key-based auth via your local ssh-agent. Glindt never prompts for passphrases; run `ssh-add` once in Terminal before connecting.
...
Glindt mirrors Hermes's surface area through a sidebar-based UI. Sections below map 1:1 to the app's sidebar.
...
- **Profiles** — Switch between multiple isolated Hermes instances. Create, rename, delete, export (zip), import. Safe-switch warning reminds users to restart Glindt after activating a different profile
...
Custom, agent-generated dashboards for any project. Define stat boxes, charts, tables, progress bars, checklists, rich text, and embedded web views in a simple JSON file — Glindt renders them with live refresh. Let your Hermes agent build and maintain project-specific visualizations automatically. See [Project Dashboards](#project-dashboards-1) below for the full schema.

### System

- **Hermes Process Control** — Start, stop, and restart the Hermes agent directly from Glindt
- **Menu Bar** — Status icon showing Hermes running state with quick actions

## Requirements

- macOS 14.6+ (Sonoma) for Glindt
- iOS 18.0+ for [GlindtGo](https://github.com/awizemann/glindt/wiki/GlindtGo) (the iPhone companion, public TestFlight from v2.5)
...
- For remote servers: SSH access (key-based), `sqlite3` on the remote (for atomic DB snapshots), and the `hermes` CLI resolvable from the remote user's `PATH` or at a path you specify per server. GlindtGo requires the same on every Hermes host it connects to.

### Compatibility

Glindt reads Hermes's SQLite database and parses CLI output from `hermes status`, `hermes doctor`, `hermes tools`, `hermes sessions`, `hermes gateway`, and `hermes pairing`. Automatic schema detection provides backward compatibility with older databases while supporting new features in newer Hermes versions.
...
xattr -d com.apple.quarantine /Applications/Glindt.app

# Or extract with ditto instead of double-clicking the zip:
ditto -xk ~/Downloads/Glindt-vX.X.X-Universal.zip ~/Downloads/
```

**Do not run `xattr -rc /Applications/Glindt.app`** — it strips codesign-related extended attributes and can break the bundle's seal. **Do not run `codesign --force --deep --sign - /Applications/Glindt.app`** — `--deep` ad-hoc re-signing is incompatible with Sparkle.framework's nested XPC services and `Updater.app` sub-bundle, and will corrupt the framework signature even if the outer app appears intact afterward. If a clean re-download + `xattr -d com.apple.quarantine` doesn't resolve the issue, please open an issue with `codesign --verify --verbose=4 --strict /Applications/Glindt.app` output captured **before** any mitigation attempts.
...
cd glindt/glindt
open glindt.xcodeproj
```

Or from the command line:

```bash
xcodebuild -project glindt/glindt.xcodeproj -scheme glindt -configuration Release -arch arm64 -arch x86_64 ONLY_ACTIVE_ARCH=NO build
```
...
Glindt follows the **MVVM-Feature** pattern with zero external dependencies beyond SwiftTerm:
...
Glindt reads Hermes data directly from `~/.hermes/`:
...
| `.glindt/dashboard.json` | JSON (per-project) | Read-only |
| `glindt/projects.json` | JSON (registry) | Read/Write |
...
Glindt watches `~/.hermes/` for file changes and queries the SQLite database for sessions, messages, and analytics. Views refresh automatically when Hermes writes new data.
...
Management actions (renaming sessions, toggling tools, editing memory) call the Hermes CLI or write directly to the appropriate files, keeping Glindt and Hermes in sync.

The app sandbox is disabled because Glindt needs direct access to `~/.hermes/` and the ability to spawn the Hermes binary.

## Project Dashboards

Project Dashboards turn Glindt into a customizable monitoring hub for all your projects. You define a simple JSON file in your project folder describing what to display — stat boxes, charts, tables, progress bars, checklists, rich text, and embedded web views — and Glindt renders it as a live-updating dashboard. Your Hermes agent can generate and maintain these dashboards automatically.
...
- **Any project status** — if your agent can measure it, Glindt can display it

### Quick Start

**1. Create the dashboard file**

Create `.glindt/dashboard.json` in any project folder:
...
In Glindt, go to **Projects** in the sidebar and click the **+** button to add your project folder. Or have your agent add it directly to the registry at `~/.hermes/glindt/projects.json`:
...
Select your project in the Projects sidebar — the dashboard renders immediately. Glindt watches the file for changes and refreshes automatically whenever the JSON is updated.
...
> Analyze this project and create a `.glindt/dashboard.json` dashboard with relevant metrics and status. Use stat widgets for key numbers, charts for trends, tables for structured data, lists for task tracking, and a webview widget if the project has a local web server or HTML reports. Register the project in `~/.hermes/glindt/projects.json` if not already registered.

Your agent can update the dashboard as part of cron jobs, after builds, or whenever project state changes. Since Glindt watches the file, updates appear in real-time.
...
Each section defines a grid with 1–4 columns. Widgets flow left-to-right, wrapping to new rows. See [DASHBOARD_SCHEMA.md](glindt/docs/DASHBOARD_SCHEMA.md) for the full schema reference with examples of every widget type.

## Releases

Glindt ships through GitHub releases — the App Store is not supported because Glindt spawns the user-installed `hermes` binary and reads `~/.hermes/` directly, both of which App Sandbox forbids.
...
The Sparkle appcast is served from [awizemann.github.io/glindt/appcast.xml](https://awizemann.github.io/glindt/appcast.xml).

Signing prerequisites (one-time):

- `Developer ID Application` certificate in the login Keychain
- `glindt-notary` keychain profile registered via `xcrun notarytool store-credentials`
...
Community-contributed Glindt project templates live under [`templates/`](templates/) in this repo and are browsable at **[awizemann.github.io/glindt/templates/](https://awizemann.github.io/glindt/templates/)** with live dashboard previews and one-click `glindt://install?url=…` links.

- **Install from the web** — click "Install with Glindt" on any template's detail page; the app takes over from there.
- **Install from a local file** — Glindt → Projects → Templates → Install from File…, or double-click any `.glindttemplate` in Finder.
...
If you find Glindt useful, consider buying me a coffee.
