...
A major release tracking **Hermes v2026.4.30 (v0.12.0)** — the largest single Hermes update Glindt has had to follow since v0.10's Tool Gateway. Headline additions: the autonomous **Curator**, **multimodal image input** in chat, **5 new inference providers**, **Microsoft Teams + Yuanbao** gateway platforms, a **read-only Kanban** view, and GlindtGo gains read-only Webhooks/Plugins/Profiles plus a Hermes-version banner.
...
Hermes v0.12's autonomous skill curator prunes / consolidates / archives agent-created skills on a 7-day schedule. Glindt adds a dedicated **Curator** sidebar item under Interact (Mac) and a Curator nav row under the System tab (iOS).
...
Hermes v0.12 advertises `prompt_capabilities.image = true` on ACP and accepts image content blocks in `session/prompt`. Glindt wires the producer side on both targets:
...
Hermes routes the resulting prompt to a vision-capable model automatically — no extra Glindt-side work to pick the right aux model.
...
Three new capabilities Glindt can now reach:
...
Hermes v0.12 cron jobs accept `--workdir <absolute-path>` to inject AGENTS.md / CLAUDE.md / .cursorrules from that directory and pin cwd for terminal/file/code_exec tools. Glindt's CronJobEditor now has a Workdir field; both create and edit paths forward the flag. Existing v0.11 jobs keep the no-cwd behaviour by leaving the field blank.

The `context_from` chaining field is read-only from Glindt this round (Hermes hasn't exposed a `--context-from` CLI flag yet, only YAML).

### Microsoft Teams + Yuanbao (Mac)

Two new gateway platforms. Microsoft Teams (the 19th platform) ships as a plugin; Yuanbao 元宝 (the 18th) is a native gateway adapter. Both surface in the Platforms tab with read-only setup panels — the OAuth dance for Yuanbao and the plugin install for Teams happen outside Glindt.
...
Three new System-tab nav rows in GlindtGo, all read-only:
...
- `HermesCapabilitiesStore` (`@Observable @MainActor`) caches per-server capabilities. Injected on `ContextBoundRoot` (Mac) and `GlindtGoTabRoot` (iOS) via `.environment(_:)` and `.hermesCapabilities(_:)`.
...
- **Chat font-size slider had no visible effect (#68)** — `RichChatView` only set `\.dynamicTypeSize`, but `GlindtFont` tokens are fixed-point (`Font.system(size: 14, …)`) so dynamic type didn't reach bubble text, reasoning, tool chips, code blocks, or markdown headings. New `\.chatFontScale` env value plumbed through `RichMessageBubble`, `MarkdownContentView`, and `CodeBlockView`; `ChatFontScale.{body, caption, captionStrong, caption2, mono, monoSmall, codeBlock, codeInline}(_:)` helpers mirror the GlindtFont base sizes so 100% is byte-for-byte identical to today's UI.
...
- **No completion notification (#64)** — sending a long prompt and switching to other work required polling the chat to know when the response landed. New `ChatNotificationService` fires a local `UNUserNotificationCenter` banner on prompt completion when Glindt isn't the foreground app. Settings → Display → Feedback → "Notify when Hermes finishes" toggle, default on.
...
Targets Hermes **v2026.4.30 (v0.12.0)**. v2026.4.23 (v0.11.0) hosts continue to work — every v0.12 surface is gated on capability detection, so Glindt v2.6 against v0.11 looks identical to Glindt v2.5.2 against v0.11. Update Hermes (`hermes update`) to unlock the new surfaces.
