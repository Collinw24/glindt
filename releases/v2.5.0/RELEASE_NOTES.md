...
The big one for 2.5: **GlindtGo, the iPhone companion**, ships in public TestFlight. Same Hermes server you've been running on your Mac — now reachable from your phone over SSH. Dashboard, chat, memory, cron, skills, settings (read), all of it. On the Mac side, the global Sessions list grows up alongside the iOS work — project filter, project badges on each row. **Plus**: full Hermes v2026.4.23 chat parity (`/steer`, per-turn stopwatch, numbered approvals, git branch chip), portable project-scoped slash commands that ship with `.glindttemplate` bundles, in-app Spotify OAuth, and design-md prereq checks.

### GlindtGo iOS companion (public TestFlight)

GlindtGo is a fully native iOS app — not a web view, not a remote desktop. It speaks SSH (Citadel under the hood), reads your Hermes state directly from your Mac (or wherever Hermes is running), and lets you tap a session to resume it from where you left off. Per-project chat works end-to-end: pick a project on your phone, the agent gets the same Glindt-managed `AGENTS.md` context block the Mac writes, and the resulting session shows up correctly attributed in the Dashboard's Sessions tab.
...
- **Project-scoped chat.** "+ In project…" sheet picks from your project registry over SFTP. Writes the Glindt-managed `AGENTS.md` block before spawning `hermes acp` so the agent boots with project context. Records the resulting session ID in the attribution sidecar so the Mac picks it up.
...
**TestFlight invite:** see the [GlindtGo wiki page](https://github.com/awizemann/glindt/wiki/GlindtGo) for the public link + onboarding walkthrough.

### Portable project-scoped slash commands

A net-new Glindt primitive (Hermes has no project-scoped slash command concept — Glindt invents the format and intercepts the chat menu client-side). Author reusable prompt templates as Markdown files at `<project>/.glindt/slash-commands/<name>.md` with YAML frontmatter (name, description, argumentHint, optional model override, tags). Invoke as `/<name> [args]` from chat — Glindt substitutes `{{argument}}` (and `{{argument | default: "..."}}`) in the body and sends the expanded prompt to Hermes; the agent never sees the slash. Works uniformly on Mac + iOS, local + remote SSH, against any Hermes version.
...
- **iOS read-only browser.** GlindtGo's chat project context bar grows a `<N> slash` chip when the project has slash commands; tap to browse them in a sheet. Multi-line markdown editing is a phone keyboard's nightmare, so v2.5 keeps Mac as the canonical editor; iOS catches up in v2.6+.
- **AGENTS.md block extension.** The Glindt-managed project context block now lists available commands so the agent can answer "what slash commands does this project have?" and recognise the `<!-- glindt-slash:<name> -->` marker prepended to expanded prompts.
- **`.glindttemplate` format extension** (schemaVersion 3). Templates ship slash commands by including `slash-commands/<name>.md` files at the bundle root and listing them in `manifest.contents.slashCommands`. The installer copies them to the project's `.glindt/slash-commands/` dir; the lock file tracks them for clean uninstall (user-authored commands in the same dir survive uninstall).
...
Glindt 2.5 mirrors the chat-surface features Hermes's TUI rewrite shipped this week:
...
Hermes v2026.4.23 added two new skills. Glindt surfaces them properly:
...
- **Project badge** on each row — small tinted folder chip with the project name. Same visual language GlindtGo uses on its Dashboard.
- Logic comes from the same `SessionAttributionService` + `ProjectDashboardService` GlindtGo consumes, both in GlindtCore. Single source of truth across platforms.

### Human-readable cron schedules everywhere

Pre-2.5, both Mac and iOS rendered cron jobs as `0 */6 * * *` raw. The new `CronScheduleFormatter` in GlindtCore translates the common shapes to plain English (every-N-minutes, every-N-hours, daily-at-H, weekdays-at-H, the `@hourly`/`@daily`/`@weekly`/`@monthly` macros) and falls back to the raw expression for anything custom. Both apps consume it.

### Under the hood

- **Shared services.** `SessionAttributionService`, `ProjectContextBlock`, and `CronScheduleFormatter` moved into GlindtCore; both apps consume them via their respective transports (`SSHTransport` on Mac, `CitadelServerTransport` on iOS).
...
- **Logger conversion.** Remaining `print("[Glindt] …")` debug statements in `HermesDataService`, `HermesLogService`, and `ProjectDashboardService` swap to `os.Logger` calls (subsystem `com.glindt`), matching the global rule that production code uses `Logger` and `print()` is reserved for previews + test helpers.

### Notes for users running 2.3

No data migrations needed. Server configs, Keychain entries, project registries, session attribution sidecar — all forward-compatible. The only invariant change is iOS-only: `GlindtGo.servers.v1` UserDefaults key migrates to `com.glindt.ios.servers.v2` on first launch, and Keychain accounts move from `"primary"` to `"server-key:<UUID>"`. One-shot, idempotent — re-running 2.3 after 2.5 ran would just see the v2 data.
