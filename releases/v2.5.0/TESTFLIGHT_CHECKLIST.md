...
Pre-flight steps to take GlindtGo to public TestFlight. Order matters — Apple review wants the privacy URL to resolve at submission time, and the build needs to upload before review can start.
...
- [ ] Copy `glindt/docs/PRIVACY_POLICY.md` content into `.gh-pages-worktree/privacy/index.html` (wrap in minimal HTML, or leave as Markdown if GitHub Pages renders Markdown — GitHub Pages with Jekyll does).
- [ ] `cd .gh-pages-worktree && git add privacy/index.html && git commit -m "docs(privacy): publish v2.5 policy" && git push`
- [ ] Verify https://awizemann.github.io/glindt/privacy/ resolves (give it ~1 min after push).
...
Open `glindt/glindt.xcodeproj`, select the `glindt mobile` target.
...
- [ ] Xcode → Product → Scheme → `glindt mobile`.
...
- [ ] **App information** (one-time, persists across builds):
  ...
  - Privacy policy URL: https://awizemann.github.io/glindt/privacy/
...
- [ ] **Test information** (per-build is fine, persists if not changed):
  ...
  - Marketing URL (optional): https://github.com/awizemann/glindt
- [ ] **What to test** (per-build):
  ```
  v2.5.0 — first public TestFlight build of GlindtGo. Try connecting to a
...
> GlindtGo is the iOS companion to Glindt, the Mac client for the Hermes AI agent. Connect to a Hermes server you operate (Mac, Linux, or any SSH-reachable host) and run sessions, browse memory, manage cron jobs, and resume conversations from your phone. All data stays between your device and your Hermes host — no developer servers in between.
...
- [ ] Record the URL — needed in Phases E (wiki GlindtGo page) and F (README v2.5 section).
