# GlindtGo — App Store Connect submission copy

Single source of truth for every field you paste into App Store Connect → My Apps → GlindtGo. TestFlight-specific fields (Beta App Description, "What to test") live in [TESTFLIGHT_CHECKLIST.md](TESTFLIGHT_CHECKLIST.md). This file covers the full App Store listing for when GlindtGo graduates from TestFlight to the public store.
...
```
https://github.com/awizemann/glindt/wiki/Support
```

### Marketing URL (optional)

```
https://github.com/awizemann/glindt
```

### Privacy Policy URL

```
https://awizemann.github.io/glindt/privacy/
```
...
```
GlindtGo is the iPhone companion to Glindt, the open-source macOS GUI for the Hermes AI agent. It connects from your phone to a Hermes server you operate — your Mac, a home Linux box, a cloud VM, anything reachable over SSH — and lets you run sessions, browse memory, manage cron jobs, and resume conversations on the go.

A fully native iOS app, not a web view or a remote desktop. GlindtGo speaks SSH directly using a pure-Swift implementation, reads Hermes state via SFTP and SQLite snapshots, and streams real-time agent output over the Agent Client Protocol on a long-lived SSH exec channel. Every byte stays between your device and the Hermes host you configured.
...
• Project-scoped chat. Pick a project from your registry and GlindtGo writes the same Glindt-managed AGENTS.md context block the Mac app does, so the agent boots with the right project context. The resulting session is attributed correctly across both clients.
...
Privacy. GlindtGo does not collect, transmit, or store your data on any server controlled by the developer. There are no analytics, no telemetry, no ad identifiers. SSH keys are generated on-device and stored in the iOS Keychain with the ThisDeviceOnly attribute, so they are unreachable while the device is locked and never sync to iCloud. The complete privacy policy lives at awizemann.github.io/glindt/privacy.

Open-source under the MIT license. Source, issue tracker, and contributor docs at github.com/awizemann/glindt. Bug reports tagged component:glindtgo go straight to the developer.
...
For v2.5.0 — first public App Store release. Trimmed from `RELEASE_NOTES.md`'s GlindtGo section to fit the iOS audience.

```
First public release of GlindtGo, the iPhone companion to the Glindt macOS app.
...
Privacy. No analytics, no telemetry, no developer-controlled servers. Read the full policy at awizemann.github.io/glindt/privacy.
```
_1150 / 4000 chars._
...
Marketing version: `2.5.0` — the same number `release.sh` will write to `MARKETING_VERSION` for the macOS Glindt release. Keeping the iOS + Mac versions in lockstep is the convention this project uses.
...
```
glindt/Glindt iOS/Assets.xcassets/AppIcon.appiconset/AW Mac OS Applications-macOS-Default-1024x1024@1x.png
```
...
1. App Store Connect → My Apps → GlindtGo → App Store tab → iOS App.
