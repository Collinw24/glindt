...
- The `/model <X>` slash command typed in Glindt chat — Glindt passes the literal text through; the parsing bug lives entirely in Hermes's ACP `_cmd_model` handler (per the RCA in [#97](https://github.com/awizemann/glindt/issues/97)). Workaround: use the new chat-header model badge in v2.9 instead, which now resolves correctly via this fix.
