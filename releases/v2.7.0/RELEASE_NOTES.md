...
The throughline: Glindt got materially faster and more honest on slow remote SSH links, where 30-second sqlite timeouts and silently-empty UI used to be common. The skeleton-then-hydrate pattern, SSH cancellation propagation, and GlindtMon-driven diagnosis are the shape of how that work gets done now.
...
v2.7 wires `withTaskCancellationHandler` through [`SSHScriptRunner.run`](https://github.com/awizemann/glindt/blob/main/glindt/Packages/GlindtCore/Sources/GlindtCore/Transport/SSHScriptRunner.swift) and [`RemoteSQLiteBackend.query`](https://github.com/awizemann/glindt/blob/main/glindt/Packages/GlindtCore/Sources/GlindtCore/Services/Backends/RemoteSQLiteBackend.swift) so parent cancellation reaches the `Process` and calls `proc.terminate()` within 100ms. New `ssh.cancelled` GlindtMon event surfaces this.
...
- **Model/provider mismatch banner.** [`ModelPreflight.detectMismatch`](https://github.com/awizemann/glindt/blob/main/glindt/Packages/GlindtCore/Sources/GlindtCore/Services/ModelPreflight.swift) recognizes when `model.default` carries a `<provider>/...` prefix that disagrees with `model.provider` (e.g. `anthropic/claude-sonnet-4.6` + `provider: nous` after switching OAuth via Credential Pools). Banner offers one-click fix in either direction.
...
A **third project entry point** alongside Browse Catalog and Add Existing Project: a wizard that scaffolds a Glindt-standard project skeleton (`<project>/.glindt/dashboard.json` + AGENTS.md marker block), registers it, and hands off to a chat session that auto-activates the bundled `glindt-template-author` skill. The skill drives the rest conversationally — widgets, optional config schema, optional cron — and writes the final files itself. Wizard stays minimal because the agent does configuration better than a multi-step form. The skill ships bundled inside `Glindt.app/Contents/Resources/BuiltinSkills.bundle/` and copies into `~/.hermes/skills/` on launch (idempotent + version-gated).

**Cron + Keychain — `$GLINDT_<SLUG>_<FIELD>` env vars.** Cron prompts that referenced `secret`-typed config fields used to get the literal `keychain://...` URI back when reading `config.json`, producing 401s. v2.7 mirrors resolved Keychain values into `~/.hermes/.env` under a marker-bounded block keyed by template slug:

```sh
# glindt-secrets:begin local-news-aggregator
GLINDT_LOCAL_NEWS_AGGREGATOR_API_TOKEN=actual-value
GLINDT_LOCAL_NEWS_AGGREGATOR_RSS_URL=https://example.com/feed
# glindt-secrets:end local-news-aggregator
```
...
```json
{
  "prompt": "Use the terminal: curl -sS -H \"Authorization: Bearer $GLINDT_LOCAL_NEWS_AGGREGATOR_API_TOKEN\" \"$GLINDT_LOCAL_NEWS_AGGREGATOR_RSS_URL\" -o {{PROJECT_DIR}}/.glindt/feed.xml"
}
```

**Migration.** First launch of v2.7 walks the project registry and writes the managed block per schemaful project — automatic. Existing cron prompts you wrote against the old (broken) `config.json` pattern still need updating: open the cron job in Glindt's Cron sidebar and edit the prompt, or ask the agent in chat ("Update my Local News cron job's prompt to use the new env var convention") — the bundled `glindt-template-author` skill (now v1.1.0) documents the convention with worked examples.

Also fixes [#75](https://github.com/awizemann/glindt/issues/75) — `_NSDetectedLayoutRecursion` on the Configuration form for projects whose form transitioned between stages with different intrinsic heights.
...
- **Project-wide auto-refresh.** [`HermesFileWatcher`](https://github.com/awizemann/glindt/blob/main/glindt/glindt/Core/Services/HermesFileWatcher.swift) used to watch each project's `dashboard.json` specifically. v2.7 promotes that to a watch on the entire `<project>/.glindt/` directory. A `markdown_file` or `log_tail` widget pointing at `<project>/.glindt/reports/foo.md` refreshes the moment a cron job rewrites the file. **By convention, place files the dashboard reads inside `.glindt/`** so the watch picks them up.
...
- **Schema mirror.** The widget vocabulary lives once at [`tools/widget-schema.json`](https://github.com/awizemann/glindt/blob/main/tools/widget-schema.json); the catalog validator reads from it and enforces per-type required fields.

---

### OAuth resilience + Credential Pools

- **Daily OAuth keepalive cron.** Prevents Anthropic OAuth refresh tokens from expiring after weeks of inactivity. New cron job `[glindt:oauth-keepalive]` (managed by Glindt) pings Hermes on a daily cadence; the in-app Refresh All Sessions action mirrors the same path on demand.
...
- **Aux Tab unknown-task surface.** When `config.yaml` has an `auxiliary.<task>` block for a task Glindt doesn't know about (newer Hermes added it; Glindt hasn't caught up), render it as a plain row with the raw provider/model values instead of dropping it silently.
...
### GlindtMon — performance instrumentation harness

The diagnostic surface that drove the bulk of the v2.7 perf work. Off by default; signpost-only mode (Instruments-friendly) is free; Full mode (4096-entry in-memory ring buffer + os.Logger) is a click away in Settings → Diagnostics → Performance. Wiki: https://github.com/awizemann/glindt/wiki/Performance-Monitoring
...
321 GlindtCore tests pass (302 prior + 19 new ModelPreflight). New GlindtMon events documented in the [Performance-Monitoring wiki](https://github.com/awizemann/glindt/wiki/Performance-Monitoring).
...
- Existing `.glindttemplate` bundles install unchanged. Catalog manifest schemaVersion stays at 1/2/3 — no bump.
- Existing `~/.hermes/.env` content is preserved byte-identically — Glindt only writes inside its `# glindt-secrets:begin <slug>` / `# glindt-secrets:end <slug>` regions.
- The skeleton-then-hydrate chat loader and SSH cancellation propagation are **Mac-only** in this release; GlindtGo (iOS) keeps its existing chat path.
...
- **iOS New Project wizard + iOS Keychain-env mirror.** GlindtGo's project surface is read-only; the wizard's chat-handoff pattern depends on Mac-only ACP plumbing.
