...
### Remote Chat no longer blocks on a bare `hermes` binary ([#100](https://github.com/awizemann/glindt/issues/100))
...
### Session resume no longer silently drops context ([#99](https://github.com/awizemann/glindt/issues/99))
...
`ACPClient.loadSession` assumed any non-throwing response meant the session loaded. But Hermes's `load_session` returns a JSON-RPC **`result: null`** (not an error) when a session can't be restored into the ACP runtime. Glindt treated that null as success and proceeded against a phantom session id, so prompts landed in fresh context.
...
3 new GlindtCore tests: `loadSession` throws on a null result + succeeds on a dict result (mock round-trip), and `hermesBinaryProbablyResolvable` returns true for a bare remote name without a transport round-trip.
