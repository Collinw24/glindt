...
- **Per-turn file-mutation verifier** — v0.14 Hermes appends a verifier footer to assistant turns that wrote files (configurable via `file_mutation_verifier`). The footer flows through Glindt chat as plain agent output; bespoke card-style rendering is deferred until the upstream format stabilizes.
...
### Providers
- **xAI Grok OAuth (SuperGrok)** — sign in with your xAI account, talk to Grok models from Glindt. Subscription-gated; surfaces in the Models picker with a "Subscription" pill.
...
- **MetricKit crash + hang diagnostics** — Glindt-iOS persists MetricKit reports across launches so post-mortems work after a TestFlight crash.
...
Note: the new `/handoff` slash command in Hermes v0.14 is messaging-platform handoff (CLI-only). Mid-chat model switching in Glindt continues to use the existing `session/set_model` RPC path via the chat-header model badge.

## Upgrade notes

- The Sparkle appcast at `https://awizemann.github.io/glindt/appcast.xml` will offer this update automatically on next launch.
- macOS 14.6+ (Sonoma) deployment target unchanged.
- No breaking changes to `~/.hermes/` state. The Glindt sidecar at `~/.hermes/glindt/` gains a new optional file for per-project Hermes proxy bindings (created lazily on first use).
