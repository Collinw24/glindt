...
A patch release covering Template Configuration rendering fixes reported against v2.2.0, plus a new catalog template that packages a Hermes skill for scaffolding new Glindt projects.
...
A `.glindttemplate` whose only content is a Hermes skill (`glindt-template-author`) plus a minimal dashboard that points users at it. Installing the template drops the skill at `~/.hermes/skills/templates/awizemann-template-author/glindt-template-author/SKILL.md`, discoverable by Claude Code, Cursor, Codex, Aider, and every other agent that reads the standard `~/.hermes/skills/` directory.

The skill teaches agents how to scaffold a new Glindt-compatible project through a short interview — purpose, data source, cadence, widgets, config, secrets — then write `<project>/.glindt/dashboard.json`, `<project>/.glindt/manifest.json`, `<project>/AGENTS.md`, and `<project>/README.md`. Scaffolded projects are usable locally and cleanly exportable as `.glindttemplate` bundles via Glindt's Export flow later. [Catalog detail page →](https://awizemann.github.io/glindt/templates/awizemann-template-author/)
...
- [Catalog site](https://awizemann.github.io/glindt/templates/) — two templates live: `awizemann/site-status-checker` and `awizemann/template-author`.
