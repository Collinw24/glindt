...
PROJECT="$REPO_ROOT/scarf/glindt.xcodeproj"
SCHEME="${SCHEME:-glindt}"
CONFIG="${CONFIG:-Debug}"
DERIVED_DATA="$REPO_ROOT/build/DerivedData"
PACKAGE_RESOLVED_REL="scarf/glindt.xcodeproj/project.xcworkspace/xcshareddata/swiftpm/Package.resolved"
...
cleanup_generated_files() {
  if [[ "${REMOVE_GENERATED_PACKAGE_RESOLVED:-0}" == "1" && -f "$PACKAGE_RESOLVED" ]]; then
    ...
    rmdir "$REPO_ROOT/scarf/glindt.xcodeproj/project.xcworkspace/xcshareddata/swiftpm" 2>/dev/null || true
    rmdir "$REPO_ROOT/scarf/glindt.xcodeproj/project.xcworkspace/xcshareddata" 2>/dev/null || true
  fi
}
...
APP_PATH="$DERIVED_DATA/Build/Products/$CONFIG/glindt.app"
...
if [[ -t 0 && -z "${CI:-}" ]]; then
  ...
  if [[ "$reply" =~ ^[Yy]$ ]]; then
    rm -rf "/Applications/glindt.app"
    ditto "$APP_PATH" "/Applications/glindt.app"
    echo "Installed to /Applications/glindt.app"
  fi
fi
