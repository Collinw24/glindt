#!/usr/bin/env bash
#
# Glindt templates catalog helper — runs the Python validator, renders the
...
#   ./scripts/catalog.sh preview [DIR]        # render self-contained preview; DIR defaults to /tmp/glindt-catalog-preview
...
cmd_preview() {
  need_builder
  local dir="${1:-/tmp/glindt-catalog-preview}"
  rm -rf "$dir"
  mkdir -p "$dir"
