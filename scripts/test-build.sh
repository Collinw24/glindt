#!/usr/bin/env bash
#
# Glindt test-build pipeline — produces a signed (and optionally notarized)
...
#                                             #   xattr -dr com.apple.quarantine /Applications/Glindt.app
...
#   2. If using --notarize: `xcrun notarytool` profile "glindt-notary" set up.
...
TEAM_ID="3Q6X2L86C4"
SCHEME="glindt"
PROJECT="scarf/glindt.xcodeproj"
NOTARY_PROFILE="glindt-notary"
...
DMG_NAME="Glindt-test-v${MARKETING_VERSION}-${TIMESTAMP}-${GIT_HASH}${GIT_DIRTY}-${VARIANT_LABEL}.dmg"
...
WORK_DIR="$(mktemp -d -t glindt-test-build)"
...
ARCHIVE_PATH="$VARIANT_DIR/glindt.xcarchive"
EXPORT_DIR="$VARIANT_DIR/export"
APP_PATH="$EXPORT_DIR/Glindt.app"
...
# Xcode exports as glindt.app — rename to Glindt.app (matches release flow).
if [[ -d "$EXPORT_DIR/glindt.app" && ! -d "$APP_PATH" ]]; then
  mv "$EXPORT_DIR/glindt.app" "$APP_PATH"
fi
...
# ---------- optional notarize ----------
if [[ $NOTARIZE -eq 1 ]]; then
  NOTARIZE_ZIP="$VARIANT_DIR/Glindt-notarize.zip"
...
mkdir -p "$STAGE_DIR"
ditto "$APP_PATH" "$STAGE_DIR/Glindt.app"
ln -s /Applications "$STAGE_DIR/Applications"

log "Create $DMG_NAME"
hdiutil create \
  -volname "Glindt $MARKETING_VERSION test" \
  -srcfolder "$STAGE_DIR" \
  -ov \
  -format UDZO \
  "$DMG_PATH"
