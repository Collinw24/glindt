#!/usr/bin/env bash
#
# Glindt release pipeline — local, manual, repeatable.
#
# Usage:
...
#   2. App Store Connect API key stored for notarytool as profile "glindt-notary":
#        xcrun notarytool store-credentials "glindt-notary" \
...
# ---------- config ----------
TEAM_ID="3Q6X2L86C4"
BUNDLE_ID="com.glindt.app"
SCHEME="glindt"
PROJECT="scarf/glindt.xcodeproj"
NOTARY_PROFILE="glindt-notary"
SIGNING_IDENTITY="Developer ID Application"
APPCAST_URL="https://awizemann.github.io/glindt.appcast.xml"
...
build_variant() {
  ...
  local archive_path="$variant_dir/glindt.xcarchive"
  local export_dir="$variant_dir/export"
  local app_path="$export_dir/Glindt.app"
  local notarize_zip="$variant_dir/Glindt-notarize.zip"
  ...
  # Xcode exports as glindt.app (PRODUCT_NAME = $TARGET_NAME = "glindt"). Rename so
  # users see properly-cased Glindt.app in /Applications. Renaming the bundle
  ...
  codesign --verify --strict --deep --verbose=4 "$verify_dir/Glindt.app" \
    || die "[$label] codesign --verify failed on packaged zip"
  spctl --assess --type execute --verbose "$verify_dir/Glindt.app" \
    || die "[$label] spctl --assess failed on packaged zip"
  rm -rf "$verify_dir"
}

UNIVERSAL_ZIP="$RELEASE_DIR/Glindt-v${VERSION}-Universal.zip"
ARM64_ZIP="$RELEASE_DIR/Glindt-v${VERSION}-ARM64.zip"
...
DOWNLOAD_URL="$DOWNLOAD_URL_BASE/v${VERSION}/Glindt-v${VERSION}-Universal.zip"
