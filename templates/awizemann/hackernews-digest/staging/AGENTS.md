...
- `.glindt/config.json` — **the source of truth for filter settings.** Written by Glindt's install/configure UI. Holds:
...
- `.glindt/manifest.json` — cached copy of `template.json`. Don't modify.
...
- `.glindt/dashboard.json` — Glindt dashboard. **Only the `value` fields of the three stat widgets and the `items` array of the "Top Stories" list widget should be updated.** The section titles, widget types, and structure must stay intact.

## How configuration works

The user configures this project through Glindt's UI — not by editing files directly. On install, a form asked them for the score threshold, item cap, and any topic keywords; those values landed in `.glindt/config.json`. They can edit those values any time via the **Configuration** button on the project dashboard header.
...
**Never** edit `.glindt/config.json` yourself. If the user asks "raise the score threshold" or "add a topic" in chat, tell them to open the Configuration button on the dashboard.
...
1. Read `.glindt/config.json`. Extract `values.min_score` (number), `values.max_items` (number), and `values.topics` (array). Apply defaults (100 / 15 / `[]`) for any missing field.
...
8. Update `.glindt/dashboard.json`:
