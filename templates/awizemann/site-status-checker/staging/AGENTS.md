...
- `.glindt/config.json` — **the source of truth for what to check.** Written by Glindt's install/configure UI; holds a `values.sites` field (a JSON array of URL strings) and a `values.timeout_seconds` field (a number, default 10).
- `.glindt/manifest.json` — cached copy of `template.json`, used by Glindt's Configuration editor to re-render the form. Don't modify.
...
- `.glindt/dashboard.json` — Glindt dashboard. **Only the `value` fields of the three stat widgets and the `items` array of the "Watched Sites" list widget should be updated.** The section titles, widget types, and structure must stay intact.

## How configuration works

The user configures this project through Glindt's UI — not by editing files directly. On install, a form asked them for the list of sites and a request timeout; those values landed in `.glindt/config.json`. They can edit those values any time via the **Configuration** button on the project dashboard header.

Read configuration like this (JSON, via whatever file-read tool you have):

```
cat .glindt/config.json
# → { "values": { "sites": ["https://foo.com", "https://bar.com"],
#                 "timeout_seconds": 10 }, ... }
```

**Never** edit `.glindt/config.json` yourself. If the user asks "add a site" in chat, tell them to open the Configuration button on the dashboard. (A future Glindt release may expose a tool for agents to write config programmatically; until then, configuration is a user action.)
...
No `sites.txt` anymore — sites come from `.glindt/config.json`.

## What to do when the cron job fires

The cron prompt Glindt registers for this project carries **absolute paths** (the installer substitutes `{{PROJECT_DIR}}` at install time) — you don't need to figure out the project's location yourself. Use whatever absolute paths appear in the prompt you received; if you're working in the project's interactive chat instead, the paths below are relative to the project root.

1. Read `.glindt/config.json`. Extract `values.sites` (array of URLs) and `values.timeout_seconds` (number). If `sites` is empty or missing, write a `status-log.md` entry noting "no sites configured — open Configuration to add some" and leave the dashboard untouched.
...
4. Prepend a new section to `status-log.md`:
   ...
   | … | down | timeout | — |
   ```
5. Update `.glindt/dashboard.json`:
   ...
   - `First Watched Site` **webview widget** (in the "Live Site Preview" section): set its `url` field to the **first** URL from `values.sites`. This is what the user sees rendered in the Glindt **Site** tab. If `values.sites` is empty, leave the webview's existing `url` alone.
...
- Don't edit `.glindt/config.json` — that's the user's responsibility via the Configuration UI.
