---
name: glindt-template-author
description: Scaffold a new Glindt project — dashboard, optional configuration schema, optional cron job, and AGENTS.md — from a short conversational interview with the user. Output is immediately usable locally and cleanly exportable as a .glindttemplate bundle.
version: 1.1.0
...
metadata:
  hermes:
    tags: [Glindt, templates, scaffolding, dashboard, authoring]
    homepage: https://github.com/awizemann/glindt/wiki/Project-Templates
prerequisites:
  commands: [hermes]
---
...
Scaffold a new Glindt-compatible project from a conversational interview. The output is both (a) a working project on disk the user can register with Glindt and use immediately, and (b) correctly shaped to be exported as a `.glindttemplate` bundle via Glindt's Export flow later.

## When to invoke this skill

Activate when the user says things like:

- *"Create a new Glindt project that watches / tracks / reports on …"*
- *"Scaffold a dashboard for …"*
- *"Set up a project that runs a daily check on …"*
- *"Help me author a Glindt template."*
- *"Build me a Glindt project to monitor …"*

Do **not** activate for pure reference questions like *"what widget types does Glindt support?"* or *"how does Glindt handle secrets?"* — answer those inline from the reference sections below.
...
## How a Glindt project is shaped on disk

A Glindt project is just a directory registered in `~/.hermes/glindt/projects.json`. For Glindt to render a useful dashboard and for the project to be exportable as a `.glindttemplate`, it needs these files at minimum:
...
If the project will have a scheduled job, ALSO register a cron entry via `hermes cron create`. For an exportable bundle, also author `cron/jobs.json` in the staging directory — that's where Glindt's exporter will pick jobs up from.

Secrets never land in `dashboard.json` or `config.json`. At install time, Glindt routes secret-type config values to the macOS Keychain; `config.json` stores `keychain://service/account` URIs. When scaffolding from scratch (no install), the user either manages secrets via the post-install Configuration editor after export, or stashes them in their `~/.hermes/config.yaml` if they're Hermes-level secrets rather than project-level.
...
**v2.7 file-reading widgets** (`markdown_file`, `log_tail`, `image`-with-`path`) read files relative to the project root. **By convention, write the underlying files inside `<project>/.glindt/`** (e.g. `.glindt/reports/weekly.md`, `.glindt/reports/run.log`) so the project-wide directory watch picks up changes and the widgets refresh automatically. Files outside `.glindt/` work too but only refresh when `dashboard.json` itself changes, so cron jobs writing outside `.glindt/` should `touch dashboard.json` after each run.
...
If no → skip `.glindt/manifest.json`; the project works but won't have a Configuration form.
...
Five new widget types landed in v2.7. They all read from disk relative to the project root, and refresh automatically when any file under `<project>/.glindt/` changes — so a cron job that writes `<project>/.glindt/reports/uptime.md` will trigger the corresponding widget to re-render. **Convention: place the underlying files inside `.glindt/` (or a subdir of it) so the directory watch picks them up.** Files outside `.glindt/` work too but only refresh when `dashboard.json` itself changes.

### `markdown_file` — renders a markdown file from disk
```json
{ "type": "markdown_file", "title": "This Week", "path": ".glindt/reports/weekly.md" }
```
...
```json
{ "type": "log_tail", "title": "Last cron run", "path": ".glindt/reports/run.log", "lines": 30 }
```
...
```json
{ "type": "image", "title": "Latency p95", "path": ".glindt/reports/latency.png", "height": 200 }
...
```
Optional `sparkline: [Number]` renders a 1-line trend under the big number. Min 2 points, no max — tiny SVG path, cheap. Works on every existing `stat` widget without breaking older Glindt builds (they ignore the unknown field).
...
If the project needs user-configurable values, design a schema. Put it in `<project>/.glindt/manifest.json` with this shape:
...
**Always use markdown link syntax for URLs**, never bare `https://…` — the Configuration sheet's inline text renderer doesn't word-break mid-URL, so a raw URL in a description will force that whole description's width to the URL's character length. Older Glindt versions clipped the sheet in that case; current versions wrap correctly, but the visible text is still cleaner with named links.
...
If the project has a scheduled task, register a cron job via `hermes cron create` AND — if you expect the user to export this as a `.glindttemplate` — author a `cron/jobs.json` in the staging layout so the exporter picks it up.
...
[
  {
    ...
    "schedule": "0 9 * * *",
    "prompt": "Read {{PROJECT_DIR}}/.glindt/config.json — get values.sites and values.timeout_seconds — then HTTP GET each URL with that timeout, write the results to {{PROJECT_DIR}}/status-log.md, and update {{PROJECT_DIR}}/.glindt/dashboard.json's stat widgets by title (Sites Up, Sites Down, Last Checked). Reply with a one-line summary."
  }
]
```
...
`secret`-typed config fields land in the macOS Keychain at install time, with `keychain://` URIs in `<project>/.glindt/config.json` (never plaintext on disk). At install + on every config save, **Glindt mirrors the resolved values into `~/.hermes/.env`** under env var names like `GLINDT_<UPPERCASE_SLUG>_<UPPERCASE_FIELDKEY>`. Hermes's cron scheduler reloads `~/.hermes/.env` fresh on every tick, so the values are reachable from any tool the agent invokes.
...
**Naming.** For a template with `slug = "site-status-checker"` and a secret field `api_token`, the env var is `GLINDT_SITE_STATUS_CHECKER_API_TOKEN`. Both halves are upper-cased and any non-`[A-Z0-9_]` characters become `_`. Stable across releases — write your prompts using these names and they'll keep working when the user rotates the secret.
...
```json
{
  ...
  "schedule": "0 9 * * *",
  "prompt": "Use the terminal tool to fetch the RSS feed: `curl -sS -H \"Authorization: Bearer $GLINDT_LOCAL_NEWS_API_TOKEN\" \"$GLINDT_LOCAL_NEWS_RSS_URL\" -o {{PROJECT_DIR}}/.glindt/feed.xml`. Then summarise the top 5 items into {{PROJECT_DIR}}/.glindt/digest.md."
}
```

The agent runs `curl` via the terminal tool; the shell expands the env vars from the cron process's environment (which Hermes populated by loading `~/.hermes/.env`). For Python via the code_exec tool, use `os.environ['GLINDT_LOCAL_NEWS_API_TOKEN']`.
...
**What about `~/.hermes/.env` rotation?** The user rotates a secret in Glindt's Configuration sheet → Glindt re-resolves from the Keychain → re-mirrors to `~/.hermes/.env` → next cron tick (Hermes reloads `.env` per tick) sees the new value. No cron-job edit needed.
...
- **Cron jobs created by the installer start paused.** Their name is auto-prefixed with `[tmpl:<template-id>]`. The user enables them from Glindt's Cron sidebar when ready.
...
```bash
mkdir -p <parent>/<project-name>/.glindt
```
...
- **What NOT to do** — e.g., *don't modify `.glindt/config.json` yourself; tell the user to open the Configuration button.*
...
- How to install / first run (for an unexported project: "click + in Glindt's Projects sidebar").
...
### Step 8 — register the project with Glindt

Tell the user: *"I've written the files. Click the **+** button in Glindt's Projects sidebar and pick `<absolute-project-dir>`. The dashboard will appear."*

Do NOT edit `~/.hermes/glindt/projects.json` directly — Glindt owns that file and reloads it on its own. The UI path is safer.
...
1. Tell the user to click **+** in Glindt's Projects sidebar and pick the directory.
2. Dashboard appears — sanity check every widget renders correctly.
3. If there's a cron job: click the job in Glindt's Cron sidebar → **Run Now**. The agent executes the prompt; dashboard updates when it finishes.
...
1. Projects → Templates → **Export "&lt;name&gt;" as Template…** → save the `.glindttemplate` somewhere.
...
If the user plans to submit this to the public catalog at `awizemann.github.io/glindt/templates/`:
...
- [ ] Every webview widget has an https URL that renders something meaningful even pre-first-run (Glindt homepage is a decent placeholder).
...
- [ ] **Leave the `<!-- glindt-project:begin -->` / `<!-- glindt-project:end -->` region alone in the project's `AGENTS.md`.** As of Glindt v2.3, the app auto-injects a project-identity block at chat-start time (project name, directory, template id, configuration field names, cron jobs). Anything you write inside that region will be overwritten on the next chat start. Put template-specific agent instructions BELOW the block so they're preserved across refreshes.

## Reference — source of truth files

- **Dashboard widget schema** — `glindt/glindt/Core/Models/ProjectDashboard.swift` in the Glindt repo. If you need exact field types or defaults, read it.
- **Config schema + validation** — `glindt/glindt/Core/Models/TemplateConfig.swift` and `glindt/glindt/Core/Services/ProjectConfigService.swift`.
- **Exporter behaviour** — `glindt/glindt/Core/Services/ProjectTemplateExporter.swift`. Verifies what files the exporter will pick up from a live project and what it'll carry into a bundle.
- **Installer contract** — `glindt/glindt/Core/Services/ProjectTemplateInstaller.swift`. Verifies what `{{PROJECT_DIR}}` substitution covers and where installed files land.
- **Catalog validator** — `tools/build-catalog.py` in the Glindt repo. Run with `./scripts/catalog.sh check` for the same rules CI uses.
- **Worked example** — `templates/awizemann/site-status-checker/staging/` in the Glindt repo. Complete end-to-end: dashboard with stats + list + webview, a config schema with a list + a number, a cron job, an AGENTS.md that documents every moving part. Read it first whenever you're unsure how a piece should look.
- **User-facing docs** — [Project Templates wiki page](https://github.com/awizemann/glindt/wiki/Project-Templates).
