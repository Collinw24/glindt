// Glindt dashboard widget renderer — the dogfood piece.
//
// Takes the SAME `dashboard.json` shape the Glindt macOS app renders
// (see glindt/glindt/Core/Models/ProjectDashboard.swift) and produces an
...
// glindt/glindt/Features/Projects/Views/Widgets/ AND add an entry to
// widget-schema.json.
//
...
(function (global) {
  ...
  function renderSection(section) {
    ...
    // Webview widgets render in a dedicated tab in the Glindt app but
  ...
  function renderLogTail(widget) {
    ...
    card.appendChild(elt("div", "widget-cron-hint",
      "Live tail appears in Glindt after install."));
    return card;
  }
  ...
  function renderMarkdownFile(widget) {
    ...
    card.appendChild(elt("div", "widget-cron-hint",
      "File contents appear in Glindt after install."));
    return card;
  }
  ...
  function renderImage(widget) {
    ...
    if (widget.url) {
      ...
      card.appendChild(elt("div", "widget-cron-hint",
        "Local files render in Glindt after install."));
    } else {
      card.appendChild(renderWidgetError(
  ...
  function renderKanbanSummary(widget) {
    ...
    card.appendChild(head);
    card.appendChild(elt("div", "widget-cron-meta",
      "Live Kanban summary appears in Glindt after install."));
  ...
  function renderCronStatus(widget) {
    ...
    if (!widget.jobId) {
      ...
      card.appendChild(elt("div", "widget-cron-hint",
        "Live status (last run, next run, output tail) appears in Glindt after install."));
    }
    return card;
  }
  ...
  function renderUnknown(widget) {
    return renderWidgetError(
      ...
      "This catalog renderer doesn't know about this widget type. The Glindt app may render it correctly if it's been added in a newer release."
    );
  }
  ...
  // form UI lives inside the Glindt app. This is purely informational
  ...
  function renderConfigSchema(container, config) {
    ...
    const desc = elt("p", "config-schema-desc",
      "Fields you'll fill in during install. Secrets are stored in the macOS Keychain; non-secret values live at <project>/.glindt/config.json.");
