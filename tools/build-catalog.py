#!/usr/bin/env python3
"""Glindt template catalog builder + validator.

Walks every `templates/<author>/<name>/` in this repo, validates the
`.glindttemplate` bundle against its manifest claim (same invariants the
...
class TemplateRecord:
    ...
    def to_catalog_entry(self) -> dict:
        ...
        return {
            ...
            "minGlindtVersion": m.get("minGlindtVersion"),
...
def _iter_templates(repo_root: Path) -> Iterable[Path]:
    ...
    a `template.json` or a built `.glindttemplate`). Authors whose dirs
...
def validate_template(template_dir: Path) -> tuple[TemplateRecord | None, list[ValidationError]]:
    ...
    # Find the bundle. By convention it's `<dir>/<dir-basename>.glindttemplate`
    # or any single .glindttemplate in the dir.
    bundles = sorted(template_dir.glob("*.glindttemplate"))
    if not bundles:
        errors.append(ValidationError(template_dir, "no .glindttemplate found in template directory"))
        return None, errors
    if len(bundles) > 1:
        errors.append(ValidationError(
            template_dir,
            f"more than one .glindttemplate present: {[b.name for b in bundles]}"
        ))
    ...
    install_url = (
        "https://raw.githubusercontent.com/awizemann/glindt/main/"
...
def _check_staging_matches_bundle(record: TemplateRecord) -> list[ValidationError]:
    ...
    if missing_in_bundle:
        errors.append(ValidationError(
            ...
            "(rebuild with `zip -qq -r <name>.glindttemplate .` from staging/)"
        ))
...
def render_detail(tmpl: str, record: TemplateRecord) -> str:
    ...
    tokens = {
        ...
        "GLINDT_INSTALL_URL": f"glindt://install?url={install_url}",
    }
    out = tmpl
