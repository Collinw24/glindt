...
def make_template_dir(
    ...
    bundle_name = bundle_name or f"{name}.glindttemplate"
...
class ValidationTests(unittest.TestCase):
    ...
    def test_rejects_oversized_bundle(self):
        ...
        with zipfile.ZipFile(template_dir / "huge.glindttemplate", "w", zipfile.ZIP_STORED) as zf:
    ...
    def test_rejects_missing_bundle(self):
        ...
        (template_dir / "staging").mkdir(parents=True)
        # No .glindttemplate in the dir.
        _, errors = self._validate_all()
        self.assertTrue(any("no .glindttemplate found" in str(e) for e in errors), errors)
    ...
    def test_render_site_end_to_end(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (site_src / "template.html.tmpl").write_text(
                "<h1>{{NAME}}</h1><p>{{DESC}}</p>"
                "<a href=\"{{GLINDT_INSTALL_URL}}\">install</a>"
            ...
            # Install URL wires through the glindt:// scheme + raw GH URL.
            self.assertIn("glindt://install?url=https://raw.githubusercontent.com/", alpha_html)
