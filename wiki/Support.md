...
title: Support
type: note
permalink: glindt-wiki/support
---

# Support

How to get help with Glindt (macOS) or GlindtGo (iOS), report a bug, request a feature, or disclose a security issue. This page is the **Support URL** Apple's App Store reviewer follows — it's intentionally simple and link-heavy so anyone landing here from the App Store finds what they need without scrolling.
...
|---|---|
| GlindtGo can't connect to my Hermes host | [GlindtGo Onboarding](GlindtGo-Onboarding) → Troubleshooting |
| Mac Glindt can't reach a remote server | [Servers & Remote](Servers-and-Remote) |
...
| TestFlight says "this beta isn't accepting any new testers" | Apple's Beta Review queue is processing the latest build. See [GlindtGo](GlindtGo) — bookmark the page and try again in 24–48h. |
| Hermes "command not found" over iOS SSH | [GlindtGo Onboarding](GlindtGo-Onboarding) — set the **Hermes binary hint** in the server-edit screen |

## Bug reports

GitHub Issues is the canonical bug tracker:

**[github.com/awizemann/glindt/issues](https://github.com/awizemann/glindt/issues)**

Tag your issue with the right component so it gets routed:

- `component: glindt` — Mac app bugs.
- `component: glindtgo` — iOS app bugs.
- `component: glindtcore` — shared package bugs (services, models, transport).
- `component: design-system` — GlindtDesign tokens / components.
- `component: templates` — `.glindttemplate` install / uninstall / catalog issues.
- `component: docs` — wiki / README / release-notes corrections.

Include in every report:

- Glindt version: **Settings → General → About** (Mac) / **System tab → Server section** (iOS).
...
- `feature: glindt` / `feature: glindtgo` / etc.
...
## TestFlight feedback (GlindtGo only)

Open GlindtGo → take a screenshot → use the **Send Beta Feedback** button TestFlight overlays on your screenshot. The screenshot + your text go straight to the developer along with device + iOS version metadata. This is the right channel for TestFlight build issues (crashes, layout glitches on a specific device, missing strings).
...
- Or use GitHub's private security advisories: <https://github.com/awizemann/glindt/security/advisories/new>.
...
Both Glindt and GlindtGo are MIT-licensed. Source: <https://github.com/awizemann/glindt>.
...
- [GlindtGo](GlindtGo) — feature tour, FAQs.
- [GlindtGo Onboarding](GlindtGo-Onboarding) — SSH key setup walkthrough.
