...
type: note
permalink: glindt-wiki/slash-commands
---

# Slash Commands (project-scoped)

A project can ship its own slash commands — reusable prompt templates as Markdown files at `<project>/.glindt/slash-commands/<name>.md` with YAML frontmatter. Invoke as `/<name> [args]` from chat; Glindt substitutes `{{argument}}` placeholders in the body and sends the expanded prompt to Hermes. The agent never sees the slash itself, just the rendered prompt with a `<!-- glindt-slash:<name> -->` marker so it can recognize the command in transcripts.

Project-scoped slash commands are a Glindt primitive — Hermes has no project-scoped slash command concept of its own. Glindt intercepts the chat menu client-side, expands the prompt, and forwards. Works uniformly on Mac + iOS, local + remote SSH, against any Hermes version.
...
4. **Save** writes the file to `<project>/.glindt/slash-commands/<name>.md`.

Files are plain Markdown, so you can also author them outside Glindt — any editor works. Glindt watches the directory and refreshes the menu live.
...
Pick one with ↑/↓ + Enter or Tab. Commands with `argumentHint` insert a trailing space so you can start typing the argument immediately. Hit Enter again to send. Glindt expands the body, prepends the `<!-- glindt-slash:<name> -->` marker, and sends the result to Hermes.
...
## GlindtGo (iOS) — read-only browser
...
The Glindt-managed project context block written to `<project>/AGENTS.md` (between `<!-- glindt-project:begin -->` and `:end -->`) lists every available slash command so the agent can answer *"what slash commands does this project have?"* and recognise the `<!-- glindt-slash:<name> -->` marker prepended to expanded prompts.
...
## Packaging via `.glindttemplate` (schemaVersion 3)
...
The installer copies the files to the project's `.glindt/slash-commands/` directory and tracks them in the lock file. **User-authored slash commands in the same directory survive uninstall** — only the template-shipped ones are removed.
...
## Why a Glindt primitive (not a Hermes one)
...
- [GlindtGo](GlindtGo) — iOS read-only browser.
