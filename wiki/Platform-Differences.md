...
type: note
permalink: glindt-wiki/platform-differences
---

# Platform differences — Glindt (Mac) vs GlindtGo (iOS)
...
| Feature | Mac (Glindt) | iOS (GlindtGo) | Why the gap |
|---|:---:|:---:|---|
...
| **Chat (ACP)** | Yes | Yes | — |
| **Project-scoped chat** | Yes | Yes | Shared `ProjectContextBlock` / `SessionAttributionService` in GlindtCore. |
...
| **Messaging Gateway (Discord, etc.)** | Yes (status + config) | No | Mac-only filesystem checks (`gateway_state.json`, pgrep). Could be lifted into GlindtCore with effort. |
...
GlindtGo is a **monitor + steer** client. It's optimised for:
...
| AppleScript / URL scheme `glindt://` | iOS apps don't get the same routing surface; can be added if needed. |
