...
type: note
permalink: glindt-wiki/sidebar-and-navigation
---
...
[`AppCoordinator.swift`](https://github.com/awizemann/glindt/blob/main/glindt/glindt/Navigation/AppCoordinator.swift) holds three pieces of state:
...
[`SidebarView.swift`](https://github.com/awizemann/glindt/blob/main/glindt/glindt/Navigation/SidebarView.swift) is a `List` with hardcoded `Section` headers. Selection is two-way bound to `coordinator.selectedSection`:
...
## GlindtGo (iOS) navigation

GlindtGo uses a different model — a 5-tab `TabView` rather than a sidebar. The tabs (Dashboard | Projects | Chat | Skills | System) are wrapped in their own `NavigationStack`s so push navigation (Cron editor, Memory detail, Project detail, Settings) stays scoped to the tab. Cross-tab signalling (Dashboard row → Chat tab resume, Project Detail → in-project chat handoff, notification deep-link → Chat) flows through `GlindtGoCoordinator`.

`.tabViewStyle(.sidebarAdaptable)` is wired so the system can switch to a sidebar layout on larger devices — but as of v2.5 the iOS target ships **iPhone-only** (`TARGETED_DEVICE_FAMILY = 1`, `SUPPORTS_MACCATALYST = NO`, `SUPPORTS_MAC_DESIGNED_FOR_IPHONE_IPAD = NO`). iPad polish is on the [GlindtGo Roadmap](GlindtGo-Roadmap) but not in scope for v2.5.
...
---
_Last updated: 2026-04-25 — Glindt v2.5.0 (audit pass: 5 sidebar groups not 4, 22 cases not 23, Projects own group, Messaging Gateway display name, iPhone-only iOS target)_