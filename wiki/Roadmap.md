...
title: Roadmap
type: note
permalink: glindt-wiki/roadmap
---

# Roadmap

What's next for Glindt. Public, opinionated, subject to change. The internal version of this lives in [`glindt/docs/ROADMAP.md`](https://github.com/awizemann/glindt/blob/main/glindt/docs/ROADMAP.md) — the public wiki version is a distillation.

## Now (2.5)

- **[GlindtGo](GlindtGo) public TestFlight.** First public iPhone companion build. Pulse the beta tester pool; iterate on feedback over 2.5.x patches.
...
- **Documentation pass.** Wiki reorganized to surface GlindtGo as a first-class section. [Platform Differences](Platform-Differences) is the new canonical reference for "what's different on iOS".
...
- **A web version of Glindt.** The whole point is being native — macOS app on the desktop, iPhone app on mobile, both close to the metal.
- **Background sync.** Glindt is a viewer; Hermes runs the agent. Pull happens when you open a tab, not in the background. (Push notifications, when Hermes ships a sender, are an *event* surface — they alert; they don't sync.)
...
- **Local Hermes runtime on iOS.** Hermes is Python; iOS doesn't sandbox Python runtimes practically. GlindtGo will always be a thin client over SSH.

## Suggesting features

Open an issue at <https://github.com/awizemann/glindt/issues> with what you want and why. Star the repo if you'd use it (signal helps prioritization).

---
_Last updated: 2026-04-25 — Glindt v2.5.0 + GlindtGo public TestFlight_