...
- **[Design System](Design-System)** — GlindtColor / GlindtFont / components reference
- [Architecture Overview](Architecture-Overview) — MVVM-F, services, transport, GlindtCore
- [Performance Monitoring](Performance-Monitoring) — GlindtMon: opt-in perf instrumentation, how to capture a baseline
...
Glindt mirrors Hermes's surface area through a sidebar-based UI grouped into four sections:
...
Glindt 2.0 is a multi-window app — one window per Hermes server, local or remote. Remote hosts are reached over plain SSH using your existing `~/.ssh/config`, agent, ProxyJump, and ControlMaster.
