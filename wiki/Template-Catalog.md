...
type: note
permalink: glindt-wiki/template-catalog
---

# Template Catalog (v2.8+)

The in-app **Template Catalog** browses every official `.glindttemplate` published at [awizemann.github.io/glindt/templates/](https://awizemann.github.io/glindt/templates/) without leaving Glindt. One click installs a template; the existing [Project Templates](Project-Templates) flow takes over from there.

## Opening the catalog

Sidebar → **Projects** → toolbar **Templates** menu → **Browse Catalog…**.

The sheet fetches the catalog from awizemann.github.io once on first open and again whenever you click the refresh icon (top-right of the sheet). Between refreshes it serves a 24-hour cache from `~/.hermes/glindt/catalog_cache.json`, so opening the sheet repeatedly doesn't re-hit the network. If both the network and the cache are unavailable, Glindt falls back to a hardcoded list of the official templates so the sheet is never empty on a fresh install.
...
Update detection compares the catalog's `version` against `<project>/.glindt/template.lock.json`'s `template_version` for every project in `~/.hermes/glindt/projects.json`. The actual update flow is "uninstall the old version, install the new one" — Glindt doesn't yet ship a single-click in-place upgrade. Your config values survive the round-trip because they live in `<project>/.glindt/config.json` separately from the lock file.
...
- **Recommended model** — if the template author specified one. Glindt does NOT auto-switch your active model; this is informational only.
...
The catalog is a single JSON file regenerated by `tools/build-catalog.py` on every release that adds or updates a template. The file format is documented in [`templates/CONTRIBUTING.md`](https://github.com/awizemann/glindt/blob/main/templates/CONTRIBUTING.md). Glindt reads it from a hardcoded URL — there's no Settings field to change the source today (deliberate; one canonical catalog keeps the namespace honest). If you want to test against a forked or local catalog, point your dev Glindt build at it by editing `CatalogService.baseURL` in source.

## Privacy

Opening the sheet hits `https://awizemann.github.io/glindt/templates/catalog.json` once per refresh window (24 hours). The request is anonymous — no token, no identifier. Same shape as Sparkle's update check. See [Privacy Policy](Privacy-Policy) for the full picture of what Glindt accesses.
...
- **No README rendering.** The detail page uses the description from `catalog.json` only. Full READMEs ship inside each `.glindttemplate` and become visible when you start the install flow.