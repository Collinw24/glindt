...
type: note
permalink: glindt-wiki/release-process
---

# Release Process

> **Two release tracks as of v2.5.** The Mac app ships through GitHub Releases + Sparkle (this page). GlindtGo (iOS) ships through TestFlight / App Store Connect — see [`releases/v<version>/TESTFLIGHT_CHECKLIST.md`](https://github.com/awizemann/glindt/blob/main/releases/v2.5.0/TESTFLIGHT_CHECKLIST.md) and [`APP_STORE_METADATA.md`](https://github.com/awizemann/glindt/blob/main/releases/v2.5.0/APP_STORE_METADATA.md). The two tracks are independent and don't share a single command — they share the version number by convention.

Mac releases are produced by a single local script: [`scripts/release.sh`](https://github.com/awizemann/glindt/blob/main/scripts/release.sh) in the main repo. **The script is the source of truth** — this page is a public-facing summary; do not duplicate prerequisites or step-by-step internals here.
...
Releases are EdDSA-signed by Sparkle. The private key lives in the user's macOS Keychain under `https://sparkle-project.org`; the public key is embedded in `Info.plist` as `SUPublicEDKey`. **If the private key is lost, no installed Glindt can ever update again.** There is no recovery — every existing user would have to manually download a new build.

## Where things live

- Release script: [`scripts/release.sh`](https://github.com/awizemann/glindt/blob/main/scripts/release.sh) (full prerequisites in the file header)
- Per-version notes + appcast entry: `releases/v<version>/`
- Appcast feed: `https://awizemann.github.io/glindt/appcast.xml`
- Releases page: <https://github.com/awizemann/glindt/releases>
...
- If the version includes GlindtGo changes: separately archive + upload via Xcode Organizer per [`TESTFLIGHT_CHECKLIST.md`](https://github.com/awizemann/glindt/blob/main/releases/v2.5.0/TESTFLIGHT_CHECKLIST.md). The Mac release script doesn't touch iOS.

## iOS release flow (separate from `release.sh`)

GlindtGo ships through:

1. **Xcode → Product → Archive** for the `glindt mobile` scheme (Any iOS Device destination).
...
5. Public App Store submission is a separate review (24–72h) using the same processed build — see [`APP_STORE_METADATA.md`](https://github.com/awizemann/glindt/blob/main/releases/v2.5.0/APP_STORE_METADATA.md) for the description, keywords, support URL, and privacy URL fields.
...
---
_Last updated: 2026-04-27 — Glindt v2.5.1 (post-package verification gate)_