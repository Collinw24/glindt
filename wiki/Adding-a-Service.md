...
type: note
permalink: glindt-wiki/adding-a-service
---
...
|---|---|---|
| **`GlindtCore` package** | The service does pure I/O against Hermes (transport reads, parsing, attribution, formatting) — **default choice**, since iOS likely needs it too. | [`glindt/Packages/GlindtCore/Sources/GlindtCore/Services/`](https://github.com/awizemann/glindt/tree/main/glindt/Packages/GlindtCore/Sources/GlindtCore/Services) |
| **Mac target** (`glindt/Core/Services/`) | The service depends on AppKit, Sparkle, NSWorkspace, AppleScript, or anything else iOS can't link. | [`glindt/glindt/glindt/Core/Services/`](https://github.com/awizemann/glindt/tree/main/glindt/glindt/glindt/Core/Services) |
| **`GlindtIOS` package** | iOS-only — needs Citadel, iOS Keychain, UIKit. | [`glindt/Packages/GlindtIOS/Sources/GlindtIOS/`](https://github.com/awizemann/glindt/tree/main/glindt/Packages/GlindtIOS/Sources/GlindtIOS) |

When in doubt, **start in GlindtCore.** Promote to a target only when you hit a framework restriction. See [GlindtCore Package](GlindtCore-Package) for the package boundary rationale.
...
In `glindtApp.swift`'s `ContextBoundRoot`:
...
Service code that uses transport is testable with a mock transport. GlindtCore ships a `MockTransport` and the test suite exercises the contract — see [Testing](Testing). Add tests in `glindt/Packages/GlindtCore/Tests/GlindtCoreTests/` for GlindtCore services; in `glindt/glindtTests/` for Mac-only ones.

---
_Last updated: 2026-04-25 — Glindt v2.5.0 (GlindtCore / Mac / GlindtIOS placement matrix; logger sweep note)_