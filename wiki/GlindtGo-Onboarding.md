---
title: GlindtGo-Onboarding
type: note
permalink: glindt-wiki/glindt-go-onboarding
---

# GlindtGo Onboarding & SSH Keys

GlindtGo connects to a Hermes server you operate over SSH. There's no Glindt-controlled cloud account — your iPhone holds an SSH key, your Hermes host trusts that key, and that's the entire trust relationship. This page walks through the onboarding flow step by step and explains what to do if the connection test fails.
...
- A way to paste a single line of text into a file on that host. Usually `ssh user@host` from another machine and editing `~/.ssh/authorized_keys`. If you're already running [Glindt](Home) on Mac, you have this.

GlindtGo never asks for your account password. It also never holds an Apple-side cloud token — there's no "sign in with Glindt" anywhere.
...
5. Test connection — GlindtGo SSHes in, looks for the `hermes` binary, saves on success
...
- **Nickname (optional)** — display name in GlindtGo. Defaults to `user@host`.
- **Hermes binary hint (optional)** — leave empty unless you know `hermes` is at an unusual path. GlindtGo prepends `~/.local/bin`, `/opt/homebrew/bin`, and `/usr/local/bin` to PATH automatically — those three cover ~95% of pipx + Homebrew installs. If your `hermes` lives elsewhere (custom virtualenv, system-managed install dir like `~/.hermes/bin`), set the hint to its absolute path.

Tap **Next**.

### 2. Choose: generate or import

- **Generate a new key** — recommended for most users. GlindtGo creates a fresh Ed25519 keypair on-device. This is the right choice unless you have a specific reason to reuse an existing key.
...
If generating: GlindtGo runs `CitadelSSHService.generateEd25519Key()` (pure-Swift, no `ssh-keygen`). About a second.

If importing: paste the private-key PEM in the top box and the matching public-key line in the bottom box. GlindtGo validates the pair before accepting.

Either way, the private half is stored in the iOS Keychain with these attributes:

- **Service:** `com.glindt.ssh-key`
...
- **Accessibility (opt-in, v2.5.1+):** `kSecAttrAccessibleAfterFirstUnlock` + `kSecAttrSynchronizable=true`. Toggle from System → Security → "Sync SSH key with iCloud Keychain". The Keychain entry now syncs across signed-in Apple devices, end-to-end encrypted by iCloud Keychain (with Advanced Data Protection enabled, the encryption keys never leave your devices). Adding a second device no longer requires generating a fresh key — install GlindtGo, sign in to the same Apple ID with iCloud Keychain enabled, the same key shows up.
...
GlindtGo displays the public-key line in a monospaced selectable box, with a Copy button.

Tap **Copy public key**. Now go to your Hermes host (in another shell) and append it:

```bash
cat >> ~/.ssh/authorized_keys <<'EOF'
ssh-ed25519 AAAAC3...the-line-GlindtGo-showed-you... glindt-ios-<device-name>
EOF
chmod 600 ~/.ssh/authorized_keys
chmod 700 ~/.ssh
```

This is its own line per device — the convention any second SSH client uses. Mac's Glindt keeps using your existing ssh-agent / `~/.ssh/config` and is unaffected.

If `~/.ssh` doesn't exist yet on the host, create it: `mkdir -p ~/.ssh && chmod 700 ~/.ssh` first.

Back in GlindtGo, tap **I've added this key**.

### 5. Test connection

GlindtGo opens an SSH session, runs a single probe (`echo $HOME && which hermes`), and if both succeed, saves the server. Expect a 2–4 second wait.
...
2. In GlindtGo: **Servers → tap the server → Edit → Hermes binary hint** → paste the absolute path (e.g. `/opt/glindt-tools/bin/hermes`).
...
- Custom SSH port? Confirm in GlindtGo's server-edit screen.

If `ssh user@host` from another device on the same network also fails, fix that first — GlindtGo can't connect to a host you can't ssh to.

### "Authentication failed" / "Permission denied (publickey)"

The public key you copied into `authorized_keys` doesn't match what GlindtGo is offering. Common causes:

- **You pasted the wrong line.** Tap **Show public key** again in GlindtGo and re-copy. Compare line-for-line against what's in `~/.ssh/authorized_keys`.
...
GlindtGo strict-checks SSH host keys. If the host's key changed (new install, MITM unlikely but possible), use **Servers → Forget this server** and re-onboard — Citadel will accept the new host key on first connect.

### Onboarding succeeds but Dashboard shows zero sessions

GlindtGo downloads a snapshot of `~/.hermes/state.db` over SFTP. If your Hermes install hasn't yet written the DB (no sessions ever started), the snapshot is empty. Start a session via the Mac app or `hermes chat` first, then pull-to-refresh the Dashboard.
...
- **No cloud accounts.** Glindt has no developer-controlled server. Your iPhone connects directly to your Hermes host over SSH.
- **No analytics.** GlindtGo doesn't transmit any data to any third party.
...
- [GlindtGo](GlindtGo) — feature tour, FAQs, limitations.
...
---
_Last updated: 2026-04-27 — Glindt v2.5.1 (opt-in iCloud Keychain sync for SSH keys)_