...
type: note
permalink: glindt-wiki/project-templates
---

# Project Templates

`.glindttemplate` bundles — shareable projects for Glindt and any agent that reads the [agents.md](https://agents.md/) standard. Introduced in v2.2.0.
...
1. **Install from File…** — pick a `.glindttemplate` from disk.
2. **Install from URL…** — paste an `https://` URL pointing at a `.glindttemplate`.
3. **`glindt://install?url=…`** — click a deep link in a browser. macOS hands it to Glindt via the registered URL scheme. Only `https://` targets are accepted; `http://`, `file://`, and `javascript:` are refused. The install sheet always opens for user confirmation — a deep link never installs silently.
...
Templates can declare a typed configuration schema. When present, Glindt inserts a **Configure** step between the parent-directory pick and the preview sheet.
...
Non-secret values are written to `<project>/.glindt/config.json` as plain JSON. Secrets are stored in the login Keychain under service name `com.glindt.template.<slug>` with an account keyed to a hash of the project directory path — two installs of the same template in different directories don't collide.
...
Glindt displays this in the Configure sheet without auto-switching your active model — always your call. Change it in Settings if you agree.
...
Uninstall is driven by `<project>/.glindt/template.lock.json`, which records everything the installer wrote. The preview sheet lists what will be removed and what will be preserved:

- **Removed:** every file listed in the lock, the skills namespace directory (wholesale — it's isolated), every Keychain ref, every tagged cron job via `hermes cron remove`, the memory block between its `<!-- glindt-template:<id>:begin/end -->` markers, and the projects-registry entry.
...
- **Remove from List (keep files)…** — *registry only*. Glindt forgets the project; nothing on disk is touched, cron jobs stay, Keychain secrets stay, skills stay. A confirmation dialog spells this out. Useful for hand-installed projects (not template-installed) or if you want to hide a project from the sidebar without destroying it.
- **Uninstall Template (remove installed files)…** — *full cleanup*, driven by the lock file as above. Only shown for projects that were installed from a `.glindttemplate`.
...
The exporter carries the configuration *schema* from `<project>/.glindt/manifest.json` into the bundle but **never** the user's values from `<project>/.glindt/config.json`. Exporting is safe on projects with live config — your secrets and personal settings stay local.

The output is a single `.glindttemplate` file you can hand to anyone, upload to a share, or submit to the public catalog.
...
└── dashboard.json          # copied to <project>/.glindt/dashboard.json
```
...
Glindt v2.7 added five new widget types — `markdown_file`, `log_tail`, `cron_status`, `image`, `status_grid` — plus a `sparkline` field on `stat` and a typed status enum on `list` items. **None of these require a `schemaVersion` bump.** They're additive within `dashboard.json` itself, so:

- v1, v2, v3 bundles that use only the original 7 widget types keep working byte-identically on v2.7+.
- Bundles that adopt new widget types still validate against the existing manifest schema — only the catalog validator's vocabulary list ([`tools/widget-schema.json`](https://github.com/awizemann/glindt/blob/main/tools/widget-schema.json)) was extended.
- A v2.7-authored dashboard installed into a pre-v2.7 Glindt renders unknown widgets as a clearly-labeled error card (not a crash), so forward-incompatibility degrades gracefully.
...
- **`kanban_summary`** renders the top three `running` / `blocked` / `todo` tasks for the current project's tenant by priority, plus a glance footer (`"12 todo · 3 running · 5 blocked"`) sourced from `hermes kanban stats`. Polls every 10s while the dashboard is foregrounded. Drop `{ kind: kanban_summary, max_rows: 3 }` into a dashboard.json section to include it. Catalog validator ([`tools/build-catalog.py`](https://github.com/awizemann/glindt/blob/main/tools/build-catalog.py)) and site renderer ([`site/widgets.js`](https://github.com/awizemann/glindt/blob/main/site/widgets.js)) recognize it; bundles using it keep validating against the existing schema.
- **`kanbanTenant`** is a Glindt-minted `glindt:<slug>` string written to `<project>/.glindt/manifest.json` on first kanban interaction inside a project, so per-project boards filter by it automatically. **Templates do not ship `kanbanTenant`** — it's user-machine-scoped state. The exporter strips it out of bundles so two installs of the same template don't collide on tenants. Catalog validator skips the field entirely.
...
- Document every `~/.glindt/config.json` field the cron job reads.
...
Published templates live at [awizemann.github.io/glindt/templates/](https://awizemann.github.io/glindt/templates/), generated from `templates/<author>/<name>/` in the [Glindt repo](https://github.com/awizemann/glindt/tree/main/templates).
...
- A one-click install button that opens `glindt://install?url=…` in the user's browser.

Submissions go through [`templates/CONTRIBUTING.md`](https://github.com/awizemann/glindt/blob/main/templates/CONTRIBUTING.md). A CI-enforced Python validator ([`tools/build-catalog.py`](https://github.com/awizemann/glindt/blob/main/tools/build-catalog.py)) mirrors the Swift-side invariants (supported field types, widget types, contents-claim verification, secret-with-default rejection, 5 MB bundle-size cap, high-confidence secret patterns). Bundles are raw-served from `main` at `https://raw.githubusercontent.com/awizemann/glindt/main/templates/<author>/<name>/<name>.glindttemplate` — no per-template GitHub Releases ceremony.
...
When you click **New Chat** on a project's Sessions tab, Glindt does two things:

1. Starts `hermes acp` with the project directory as the session's working directory.
2. Writes a Glindt-managed block into `<project>/AGENTS.md` between `<!-- glindt-project:begin -->` and `<!-- glindt-project:end -->` markers, so Hermes's automatic context-file load picks up the project's identity.

Hermes reads a context file from the session's cwd at startup, picking the first match in this priority order: `.hermes.md` → `HERMES.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. Each is capped at 20 KB. Glindt's managed block contains:
...
**Bare projects.** If you add a plain directory to Glindt (via the `+` button) and start a chat in it, Glindt creates a minimal `AGENTS.md` containing only the managed block. The agent still has identity-level project awareness even without a template.
...
The service backing all of this is `ProjectAgentContextService` in the main Glindt repo. Template authors: leave the `<!-- glindt-project -->` region alone in your bundled `AGENTS.md`, put template-specific instructions below it.
...
- [Installation](Installation) — getting Glindt set up in the first place.
...
---
_Last updated: 2026-05-08 — Glindt v2.7.5 (`kanban_summary` widget + per-project `kanbanTenant` manifest field; templates never ship the tenant — exporter strips it)_