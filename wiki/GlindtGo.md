...
title: GlindtGo
type: note
permalink: glindt-wiki/glindt-go
---

# GlindtGo — iOS companion for Hermes

GlindtGo is the iPhone companion to [Glindt](Home), the macOS GUI for the [Hermes AI agent](https://github.com/awizemann/hermes-agent). It connects from your phone to a Hermes server you operate (your Mac, a home Linux box, a cloud VM — anywhere reachable over SSH), and lets you run sessions, review memory, manage cron jobs, and resume conversations on the go.

> **Status:** Public beta in TestFlight. See **[Installation](#installation)** below.

## What GlindtGo is, in one paragraph

GlindtGo is a fully native iOS app — not a web view, not a remote desktop. It speaks SSH (Citadel under the hood, no `ssh` binary needed on iOS), reads your Hermes state directly via SFTP + SQLite snapshots, and streams real-time agent output over the [Agent Client Protocol](ACP-Subprocess) on a long-lived SSH exec channel. Every byte stays between your device and the Hermes host you configured. There are no developer-controlled servers in between.
...
GlindtGo is in **public TestFlight**. Apple-provided test environment, free to join, no payment, no Apple ID needed for beta installs.
...
3. **Tap "Accept" and "Install"** — TestFlight installs GlindtGo alongside your other apps.
4. **Open GlindtGo** — onboarding walks you through host details, generates a new SSH keypair, and gives you the public-key snippet to paste into your Hermes host's `~/.ssh/authorized_keys`. Step-by-step walkthrough: [GlindtGo Onboarding](GlindtGo-Onboarding).

Onboarding details:

- GlindtGo generates a fresh Ed25519 keypair on first run. The private half lives in the iOS Keychain. **Default:** device-local (`kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly`, not iCloud-synced). **v2.5.1+:** an opt-in toggle in System tab → Security flips it to sync via iCloud Keychain (`kSecAttrAccessibleAfterFirstUnlock` + `kSecAttrSynchronizable=true`) so iPhone + iPad + Mac see the same key without onboarding each device. End-to-end encrypted by iCloud Keychain; with Advanced Data Protection enabled, the encryption keys never leave your devices.
...
| **Chat** | Streamed agent responses, tool-call disclosure groups, code blocks with horizontal scroll. Project-scoped chat picks a project from your registry, writes the same Glindt-managed `AGENTS.md` block as the Mac app, and spawns `hermes acp` with the project as the working directory. |
...
Picking a project at the start of a chat tells the agent exactly which directory it's operating in. GlindtGo does the same handshake the Mac app does:

1. SFTP-reads `~/.hermes/glindt/projects.json` for the project registry.
2. On selection, SFTP-writes a managed block into `<project>/AGENTS.md` (between `<!-- glindt-project:begin -->` and `:end -->` markers — preserves anything outside).
3. Spawns `hermes acp` with `cwd = <project.path>`.
4. After the session ID returns, records the attribution in `~/.hermes/glindt/session_project_map.json`.
...
If the SFTP write fails (permissions, disk full, network drop), GlindtGo surfaces a banner — "Project context not written — agent will proceed without it" — and starts the session anyway. The session works; it just doesn't have the augmented context.

## Limitations in v1

- **No local mode.** GlindtGo only operates against an SSH-reachable Hermes host. There's no local Hermes runtime on iOS.
...
- **No template install UI.** `.glindttemplate` install + uninstall is Mac-only in v1.
- **No terminal mode.** Rich-chat (ACP) only.
- **English only.** The Mac app ships in 7 languages; GlindtGo is English-only for v1.
...
**Onboarding can't connect.** First check that `ssh user@host` works from your Mac with the same hostname/port. If that fails, GlindtGo can't connect either — fix SSH first. If the Mac connection works:

- Make sure you appended the GlindtGo-shown public-key to `~/.ssh/authorized_keys` on the host.
- Make sure `hermes` is in the SSH user's PATH on the host. GlindtGo prepends common pipx / Homebrew install paths to the exec command, but if `hermes` lives somewhere unusual, run `which hermes` over SSH and ensure the path is one of: `~/.local/bin`, `/opt/homebrew/bin`, `/usr/local/bin`, or `~/.hermes/bin`.

**Dashboard shows zero sessions but I know there are some.** GlindtGo downloads a snapshot of `~/.hermes/state.db` over SFTP. If your Hermes install hasn't yet written the DB (no sessions ever started), the snapshot is empty. Start a session via the Mac app or CLI first.
...
A: No. See the [privacy policy](https://awizemann.github.io/glindt/privacy/). The apps make exactly three kinds of network connections: (1) SSH to your Hermes hosts, (2) Sparkle update checks (Mac only), (3) HTTPS to GitHub Pages for the public template catalog. Zero analytics.

**Q: Where can I see what's planned next?**
A: [GlindtGo Roadmap](GlindtGo-Roadmap) tracks shipped milestones (M6 / M7 / M8 / M9) and remaining work. The [main Roadmap](Roadmap) covers cross-platform plans.

## Reporting issues

- **Bugs:** [github.com/awizemann/glindt/issues](https://github.com/awizemann/glindt/issues) — tag `component: glindtgo`.
- **Feature requests:** same, tag `feature: glindtgo`.
...
_Last updated: 2026-04-29 — Glindt v2.5.2 (Auto-reconnect + Draft persistence + Load earlier messages rows added to features matrix)._