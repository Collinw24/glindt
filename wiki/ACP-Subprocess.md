...
type: note
permalink: glindt-wiki/acp-subprocess
---

# ACP Subprocess

ACP — Agent Client Protocol — is Hermes's chat protocol: JSON-RPC 2.0 over stdio (or its bidirectional equivalent). Glindt's Rich Chat surface speaks ACP end-to-end. There is no SQLite polling involved in chat; tokens, thoughts, tool calls, and permission prompts all stream live from the channel.

## ACPClient + ACPChannel

[`ACPClient`](https://github.com/awizemann/glindt/blob/main/glindt/Packages/GlindtCore/Sources/GlindtCore/ACP/ACPClient.swift) is an `actor` that owns whatever-it-is that ferries JSON-RPC bytes back and forth, and exposes an `AsyncStream<ACPEvent>`. The "whatever-it-is" is abstracted behind the [`ACPChannel`](https://github.com/awizemann/glindt/blob/main/glindt/Packages/GlindtCore/Sources/GlindtCore/ACP/ACPChannel.swift) protocol so Mac and iOS share the client without `#if os(...)`:

| Channel | Where it lives | What it wraps |
|---|---|---|
| [`ProcessACPChannel`](https://github.com/awizemann/glindt/blob/main/glindt/Packages/GlindtCore/Sources/GlindtCore/ACP/ProcessACPChannel.swift) | GlindtCore | A Foundation `Process` running `hermes acp` (local) or `ssh -T host -- hermes acp` (Mac remote). |
| [`SSHExecACPChannel`](https://github.com/awizemann/glindt/blob/main/glindt/Packages/GlindtIOS/Sources/GlindtIOS/SSHExecACPChannel.swift) | GlindtIOS | A Citadel SSH exec channel (no Foundation `Process` available on iOS) — bidirectional stdin/stdout streams over the SSH transport. |
...
- **Remote:** the transport returns `/usr/bin/ssh -T <opts> host -- hermes acp`. `SSH_AUTH_SOCK` is inherited so the GUI-launched Glindt reaches the user's ssh-agent. `TERM` is removed.
...
---
_Last updated: 2026-04-25 — Glindt v2.5.0 (GlindtCore extraction + ACPChannel abstraction + iOS SSHExecACPChannel section)_