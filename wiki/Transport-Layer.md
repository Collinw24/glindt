...
type: note
permalink: glindt-wiki/transport-layer
---
...
- **`LocalTransport`** — direct `FileManager` + `Process` against the local disk (`glindt/Packages/GlindtCore/Sources/GlindtCore/Transport/LocalTransport.swift`).
- **`SSHTransport`** — OpenSSH-driven, multiplexed via ControlMaster. **Mac only**; iOS doesn't ship the `/usr/bin/ssh` binary (`glindt/Packages/GlindtCore/Sources/GlindtCore/Transport/SSHTransport.swift`).
- **`CitadelServerTransport`** — pure-Swift SSH via Citadel + NIO. **iOS only**, used by GlindtGo for every remote primitive (`glindt/Packages/GlindtIOS/Sources/GlindtIOS/CitadelServerTransport.swift`).

All three implement the same protocol, so services in [GlindtCore](GlindtCore-Package) can consume any of them without `#if os(...)` shims.

## Protocol surface

[`ServerTransport.swift`](https://github.com/awizemann/glindt/blob/main/glindt/Packages/GlindtCore/Sources/GlindtCore/Transport/ServerTransport.swift) exposes:

**Identity**
- `contextID: ServerID` — UUID; namespaces caches under `~/Library/Caches/glindt/snapshots/<id>/`.
...
[`TransportErrors.swift`](https://github.com/awizemann/glindt/blob/main/glindt/Packages/GlindtCore/Sources/GlindtCore/Transport/TransportErrors.swift) defines `TransportError`:
...
[`LocalTransport.swift`](https://github.com/awizemann/glindt/blob/main/glindt/Packages/GlindtCore/Sources/GlindtCore/Transport/LocalTransport.swift) — a thin wrapper around `FileManager`, `Process`, and `DispatchSourceFileSystemObject`.

- **Atomic writes:** writes to `<path>.glindt.tmp`, sets `0600` if the filename suggests a secret, then `replaceItemAt` (existing) or `moveItem` (new).
...
[`SSHTransport.swift`](https://github.com/awizemann/glindt/blob/main/glindt/Packages/GlindtCore/Sources/GlindtCore/Transport/SSHTransport.swift) — every primitive becomes an `ssh`/`scp`/`sftp` invocation, multiplexed over a single ControlMaster connection.
...
-o ControlPath=/tmp/glindt-ssh-<uid>/%C
...
- `writeFile`: scp to `<path>.glindt.tmp`, then remote `mv` — atomic; cleans the orphan on failure.
...
```
sqlite3 "$HOME/.hermes/state.db" ".backup '/tmp/glindt-snapshot-XYZ.db'" && \
sqlite3 '/tmp/glindt-snapshot-XYZ.db' "PRAGMA journal_mode=DELETE;"
```

`.backup` is WAL-safe — it captures a consistent snapshot without blocking writers. The `PRAGMA journal_mode=DELETE` strips WAL mode so the snapshot is self-contained (no `-wal`/`-shm` sidecars). `scp` pulls it to `~/Library/Caches/glindt/snapshots/<id>/state.db`. The remote temp is removed.
...
The iOS app can't shell out to `/usr/bin/ssh` — there's no such binary in the iOS sandbox. Instead, GlindtGo drives [Citadel](https://github.com/orlandos-nl/Citadel), a pure-Swift SSH/SFTP/exec implementation built on SwiftNIO. `CitadelServerTransport` wraps it behind the same `ServerTransport` protocol so all of GlindtCore consumes one shape.
...
- Same atomic-write convention (`<path>.glindt.tmp` → SFTP `rename`).
...
- **Keychain-backed SSH key.** Each configured server holds its own Ed25519 keypair in the iOS Keychain (`com.glindt.ssh-key` service, `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly`, account `server-key:<UUID>`). Mac uses the system ssh-agent + `~/.ssh/config`; iOS keys never leave the device.
- **Watch via mtime polling.** Same 3-second cadence as `SSHTransport` — Citadel doesn't have an equivalent of inotify-over-SSH.
- **No streamed exec yet.** `streamLines` is a stub on iOS; log tailing in GlindtGo uses periodic refreshes instead. Future work — Citadel exposes the raw exec channel, just hasn't been wired up.

### Connection holder + reuse

Citadel's `SSHClient.connect(...)` handshake costs ~500ms on a warm network. GlindtGo keeps a long-lived per-server `CitadelConnectionHolder` so subsequent calls reuse the same TCP+crypto session — same idea as Mac ControlMaster, different mechanism. The holder is cached per-`ServerID` so two configured remotes don't contend on a single channel pool.

See [GlindtGo Onboarding](GlindtGo-Onboarding) for user-side setup and [GlindtCore Package](GlindtCore-Package) for why `KeychainSSHKeyStore` lives in `GlindtIOS` and not `GlindtCore`.

---
_Last updated: 2026-04-29 — Glindt v2.5.2 (snapshot fallback via `cachedSnapshotPath`)_