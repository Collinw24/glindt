...
type: note
permalink: glindt-wiki/wiki-maintenance
---

# Wiki Maintenance

This wiki is the public reference for Glindt. It is a separate git repo (`glindt.wiki.git`) at <https://github.com/awizemann/glindt/wiki>. This page documents how it is edited, what should and should not go in it, and how it stays in sync with the code.
...
```bash
git clone git@github.com:awizemann/glindt.wiki.git .wiki-worktree
```
...
- Every page **ends with**:
  ```
  ---
  _Last updated: YYYY-MM-DD — Glindt v<current>_
  ```
...
- **TestFlight + App Store metadata** stay in `releases/v<ver>/TESTFLIGHT_CHECKLIST.md` and `APP_STORE_METADATA.md` on `main`. The wiki's [GlindtGo](GlindtGo) page links to the live TestFlight URL but doesn't duplicate Apple-side metadata.
- **Privacy Policy** has two copies on purpose: the canonical one at `awizemann.github.io/glindt/privacy/` (linked from the iOS Info.plist + App Store Connect), plus a wiki mirror at [Privacy Policy](Privacy-Policy) for in-wiki readability. The wiki copy is updated alongside major releases.
- **Internal dev notes** (PRD, Hermes API discovery, raw architecture) live in `glindt/docs/` in the main repo. The wiki carries the public-relevant parts in distilled form, not full duplicates.
...
- **Larger changes:** clone `git@github.com:awizemann/glindt.wiki.git` directly, or open an issue describing the proposed change and we'll work it in.

---
_Last updated: 2026-04-25 — Glindt v2.5.0 (added TestFlight + App Store metadata + privacy-mirror source-of-truth rules)_