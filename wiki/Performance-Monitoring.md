...
type: note
permalink: glindt-wiki/performance-monitoring
---

# Performance Monitoring (GlindtMon)

Glindt ships an always-on, opt-in performance instrumentation harness called **GlindtMon**. It records timing samples and event counts at known hot spots so users hitting "feels slow" can capture a baseline, share it with maintainers, and have a concrete signal to act on instead of a vague report.
...
- **It's open source.** All the plumbing lives in [`GlindtCore/Diagnostics/`](https://github.com/awizemann/glindt/tree/main/glindt/Packages/GlindtCore/Sources/GlindtCore/Diagnostics).
...
| **Signpost only** (default) | `os_signpost` events to the `com.glindt.mon` subsystem | Zero outside Instruments |
...
`Settings → Diagnostics → Performance`. Same three-mode picker, same panel layout, same `Copy as JSON` action. The mode is persisted across launches in `UserDefaults` under key `GlindtMonMode`.

### From a Terminal (any mode)

```bash
log stream --predicate 'subsystem == "com.glindt.mon"' --info --debug
```
...
|---|---|---|---|
| `chatRender` | `mac.ChatView.body` | [Mac ChatView](https://github.com/awizemann/glindt/tree/main/glindt/glindt/Features/Chat/Views/ChatView.swift) | Full chat tab body re-eval count |
| `chatRender` | `mac.RichChatMessageList.body` | [RichChatMessageList](https://github.com/awizemann/glindt/tree/main/glindt/glindt/Features/Chat/Views/RichChatMessageList.swift) | Whether the message-list `ForEach` is re-issuing |
| `chatRender` | `mac.RichMessageBubble.body` | [RichMessageBubble](https://github.com/awizemann/glindt/tree/main/glindt/glindt/Features/Chat/Views/RichMessageBubble.swift) | Per-bubble re-evals — divide by `acpEvent` count to spot wasted re-renders |
| `chatRender` | `ios.ChatView.body` / `ios.MessageBubble.body` | iOS `ChatView.swift` | Same signal on iOS |
| `chatStream` | `mac.sendViaACP` | [Mac ChatViewModel](https://github.com/awizemann/glindt/tree/main/glindt/glindt/Features/Chat/ViewModels/ChatViewModel.swift) | User tap → first prompt write (carries prompt byte count) |
...
| `chatStream` | `firstByte` / `firstThoughtByte` | [RichChatViewModel](https://github.com/awizemann/glindt/tree/main/glindt/Packages/GlindtCore/Sources/GlindtCore/ViewModels/RichChatViewModel.swift) | Time-to-first-token. Splits Hermes "thinking" from streaming render |
...
| `sessionLoad` | `mac.fetchSkeletonMessages` / `.rows` / `.transportError` | [HermesDataService](https://github.com/awizemann/glindt/tree/main/glindt/Packages/GlindtCore/Sources/GlindtCore/Services/HermesDataService.swift) | Phase 1 of v2.8 two-phase chat loader — user+assistant rows only, ~few KB on the wire regardless of tool_calls blob size |
...
| `sessionLoad` | `mac.loadRecentSessions.coalesced` | [Mac ChatViewModel](https://github.com/awizemann/glindt/tree/main/glindt/glindt/Features/Chat/ViewModels/ChatViewModel.swift) | A second caller awaited the in-flight load instead of spawning a parallel SSH round-trip |
| `sqlite` | `sqlite.query` / `sqlite.queryBatch` | [RemoteSQLiteBackend](https://github.com/awizemann/glindt/tree/main/glindt/Packages/GlindtCore/Sources/GlindtCore/Services/Backends/RemoteSQLiteBackend.swift) | Per-call latency over SSH (carries row count + stdout bytes) |
| `transport` | `ssh.streamScript` (iOS) / `ssh.run` (Mac) | [CitadelServerTransport](https://github.com/awizemann/glindt/tree/main/glindt/Packages/GlindtIOS/Sources/GlindtIOS/CitadelServerTransport.swift), [SSHScriptRunner](https://github.com/awizemann/glindt/tree/main/glindt/Packages/GlindtCore/Sources/GlindtCore/Transport/SSHScriptRunner.swift) | SSH round-trip time |
| `transport` | `ssh.cancelled` | [SSHScriptRunner](https://github.com/awizemann/glindt/tree/main/glindt/Packages/GlindtCore/Sources/GlindtCore/Transport/SSHScriptRunner.swift) | Parent task cancellation reached the ssh subprocess (v2.8) — terminated within 100ms instead of running to its 30s deadline |
| `sessionLoad` | `mac.fetchSessionPreviews` / `.rows` / `.transportError` | [HermesDataService](https://github.com/awizemann/glindt/tree/main/glindt/Packages/GlindtCore/Sources/GlindtCore/Services/HermesDataService.swift) | Sidebar preview fetch — already substr-bounded, instrumented in v2.8 for visibility |
| `diskIO` | `loadConfig` / `loadCronJobs` | [HermesFileService](https://github.com/awizemann/glindt/tree/main/glindt/glindt/Core/Services/HermesFileService.swift) | Hot disk reads. `loadConfig` also logs caller stack frames in Full mode |
| `diskIO` | `memory.load` / `.bytes` | [MemoryViewModel](https://github.com/awizemann/glindt/tree/main/glindt/glindt/Features/Memory/ViewModels/MemoryViewModel.swift) | Memory tab open — 4 sequential SFTP reads on remote (config + profiles + memory + user). v2.8 instrumented. |
| `diskIO` | `cron.load` / `.jobs` | [CronViewModel](https://github.com/awizemann/glindt/tree/main/glindt/glindt/Features/Cron/ViewModels/CronViewModel.swift) | Cron tab open — jobs.json + skills walk + selected job's output. v2.8 instrumented. |
| `diskIO` | `skills.load` / `.count` | [SkillsViewModel](https://github.com/awizemann/glindt/tree/main/glindt/Packages/GlindtCore/Sources/GlindtCore/ViewModels/SkillsViewModel.swift) | Skills tab open — full SkillsScanner walk (one stat + SKILL.md read per skill dir). v2.8 instrumented. |
| `diskIO` | `curator.load` / `.bytes` | [CuratorViewModel](https://github.com/awizemann/glindt/tree/main/glindt/Packages/GlindtCore/Sources/GlindtCore/ViewModels/CuratorViewModel.swift) | Curator tab open — `hermes curator status` CLI + state file + REPORT.md. v2.8 instrumented. |
...
3. Optionally, in another terminal: `log stream --predicate 'subsystem == "com.glindt.mon"'`.
...
7. Paste the JSON in a [GitHub issue](https://github.com/awizemann/glindt/issues) or feedback thread, with one sentence describing what you were doing.

The maintainers will use the same JSON shape to grep for outliers.

## Privacy

GlindtMon is privacy-conscious by construction:
...
- **Subsystem isolation.** All output uses subsystem `com.glindt.mon`, so users can grep / filter / disable independently of Glindt's general logs.
...
GlindtMon.measure(.diskIO, "loadX") {
    // your work
}

// Async interval — same shape
try await GlindtMon.measureAsync(.sqlite, "query") {
    try await actualQuery()
}

// One-shot event — count + optional payload size
GlindtMon.event(.chatStream, "firstByte", count: 1, bytes: chunk.utf8.count)
```

### Picking a category

Categories are a fixed enum in [`GlindtMon.swift`](https://github.com/awizemann/glindt/tree/main/glindt/Packages/GlindtCore/Sources/GlindtCore/Diagnostics/GlindtMon.swift):
...
Adding a new category is a one-line case in `GlindtMon.Category` plus a row in this page.
...
```swift
var body: some View {
    let _: Void = GlindtMon.event(.chatRender, "mac.ChatView.body")
    return VStack { … }
}
```
...
let t = ContinuousClock().measure {
    for _ in 0..<n {
        GlindtMon.measure(.other, "noop") { 1 + 1 }
    }
}
...
```
┌─────────────────────────────────────────────────┐
│ GlindtMon.measure / measureAsync / event         │
│   ↓ (one os_unfair_lock per emit, no allocs)   │
├─────────────────────────────────────────────────┤
│ GlindtMonSignpostBackend  (always on)            │
│ GlindtMonRingBuffer       (Full mode only)       │
│ GlindtMonLoggerBackend    (Full mode only)       │
└─────────────────────────────────────────────────┘
```

Boot wiring lives in [`GlindtMonBoot.swift`](https://github.com/awizemann/glindt/tree/main/glindt/Packages/GlindtCore/Sources/GlindtCore/Diagnostics/GlindtMonBoot.swift) — `GlindtMonBoot.configure(mode:)` is called once at app launch from both `GlindtApp.init` (Mac) and `GlindtIOSApp.init` (iOS).
...
Tests live at [`GlindtMonTests.swift`](https://github.com/awizemann/glindt/tree/main/glindt/Packages/GlindtCore/Tests/GlindtCoreTests/GlindtMonTests.swift) — 11 cases covering ring ordering / wrap / reset, summary aggregation, percentiles, install / isActive transitions, the throw path on `measureAsync`, boot mode switches, and JSON export round-trips.
...
The in-app panel is for "I want a quick number to share." For deep work, attach Instruments and pick the **Time Profiler** template — every `GlindtMon.measure(...)` call shows up as a signpost event in the **Points of Interest** track. Filter the track by `subsystem: com.glindt.mon`. Pair with Time Profiler in the same trace and you'll see exactly which Swift functions are hot at the moment GlindtMon recorded a slow `sqlite.query`.

## Roadmap

GlindtMon currently covers the chat hot path, transport, SQLite, and a slice of disk I/O. Next planned coverage areas:
...
- [Architecture Overview](Architecture-Overview) — where GlindtCore + Diagnostics fits in the stack
- [Core Services](Core-Services) — the services GlindtMon instruments today
